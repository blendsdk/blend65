...
| RD-07 | 6502 codegen & structured `Instr` model | IL → `Instr` list; calls platform codegen hooks (AR-18) | RD-06, RD-10 | A | 🟡 Candidate |
| RD-08 | Peephole optimizer | Optimizer 2 over `Instr` list (passthrough v1) | RD-07 | B | 🟡 Candidate |
| RD-09 | ACME emitter & assembler integration | `Instr` → ACME text; invoke ACME → binary; binary format via platform hook | RD-07 | A | 🟡 Candidate |
| RD-10 | **Platform plugin system** | Platform = data + codegen-strategy hooks (AR-18); profiles for c64/cx16/a800xl/a7800/c64u; **platform packages contribute T4 intrinsic registry entries + ship `.asm` runtime (AR-28/29/30)**; ZP arg-block size declared per profile (AR-34) | RD-01 | A | 🟡 Candidate (scope ✅ resolved) |
| RD-11 | Diagnostics & resource reporting | **Scope ✅ FULLY resolved (AR-70..AR-85).** *Diagnostics engine in `@blend65/core`:* namespace partition (Ch 14 `E10xxx`/`W10xxx` user codes + `E9xxxx` ICE band, AR-70); structured `Diagnostic` record (AR-71); interned-`SourceId`+byte-offset span model w/ on-demand UTF-16 (AR-72); accumulating `DiagnosticBag` (AR-73); recovery architecture stable / rule-coverage provisional + mandatory cascade-suppression (AR-74); central severity-policy layer (AR-75); multi-renderer terminal caret + JSON emitter (AR-76); library-first API returning `Diagnostic[]` (AR-77); LSP keep-ready posture (AR-78). *Build summary / resource reporting:* dedicated `ResourceReport` aggregator in core (AR-79); data-source ownership SFA/ACME/profile/plugin (AR-80); ZP/RAM budget diagnostics pre-ACME, binary-size post-ACME (AR-81); multi-renderer table + JSON (AR-82); on by default (AR-83); staged MVP scope, shape defined now (AR-84); folded here, warnings via AR-75 (AR-85). | RD-01 | A | 🟡 Candidate (scope ✅ resolved) |
| RD-12 | Test harness & emulator verification | 3-tier taxonomy (unit/golden/emulator, AR-22); `@blend65/test-harness` published pkg w/ `EmulatorDriver` (VICE x64sc binary monitor, AR-23/24); register/memory assertions + screenshot artifacts (AR-25); run strategies label/frames/sentinel (AR-26); CI policy (AR-27) | RD-01 | A | 🟡 Candidate (scope ✅ resolved) |
| RD-13 | Non-functional requirements | Performance, portability, error UX, maintainability | — | A | 🟡 Candidate |
| RD-14 | VS Code extension & Language Server | Full LSP (AR-14): highlighting + diagnostics + completion + hover + go-to-def; depends on error-tolerant frontend (AR-15) | RD-03, RD-04 | B | 🟡 Candidate |
| RD-15 | Programmatic + CLI API | Compiler usable as a library AND via CLI (like `tsc` + TS compiler API); yargs + conditional chalk (AR-16/17) | RD-01 | A | 🟡 Candidate |
| RD-16 | Compiler configuration (`blend65.json`) | JSONC project config (AR-13): optimization flags, target platform, paths, etc. | RD-01 | A | 🟡 Candidate |
| RD-17 | Intrinsic functions & runtime-routine ABI | 4-tier taxonomy T1–T4 (AR-28); typed descriptor registry (AR-29); hybrid body strategy — TS Instr-emit + hand-written `.asm` runtime (AR-30); ambient vs imported + reserved names (AR-31); CPU/platform conditioning → compile-time error (AR-32); runtime-routine calling convention / ABI (AR-33); ZP arg-block min floor (AR-34); "crazy asm" = registered routine only (AR-35); end-user `extern` deferred (AR-36) | RD-04, RD-10 | A | 🟡 Candidate (scope ✅ resolved) |
