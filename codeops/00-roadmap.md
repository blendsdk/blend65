...
|---------|---------|---------------|----------|--------|--------------|
| blend65-ri | [→](features/blend65-ri/00-roadmap.md) | **RD-18 Slice 7a ✅ COMPLETE (2026-07-12, exec_plan 64/64)** — the aggregate DIRECT-addressing surface (arrays/structs/enums) ships end-to-end; **3-part bar GREEN on real VICE 3.10 FIRST RUN**: two-file `examples/slice7/` → `$C000..$C009 = 0E 2A 08 02 06 02 01 14 0B 03` (indexed loop sum, nested member write, scaled struct-array element, enum-dispatch switch, query folds 6/2/1, cross-module `Gfx.TABLE[i]` from the in-image data section, whole-struct COPY proof, enum→word cast); 184-line golden minted post-VICE, seven prior goldens byte-exact NO re-mint. Highlights: array literals parse everywhere legal (50→51 kinds; `parseConstDecl` flag gap fixed; dotted `Mod.Type`); module-keyed FQN tables — the bare-name struct-collision DEFECT fixed + regression-tested; the unified lazy const/type engine (constants ⇄ layouts ⇄ enum values; ONE path-carrying E10165/E10194 per cycle; engine-folded `sizeof`/`offsetof`/`length`, representability typing); full aggregate typing (E10097 field order, switch-on-enum E10077, W10140/W10141, E10157, permanent E10093/E10120 returns, loud param rejection pending 7b); `lowerPlace` + const-data `!byte` streams + tier-1 `abs,X` framings under per-arm state obligations (X-mirror cleared pre-LDX, byte-load homing, loud word-store rejection, `protectA()` ZP spills). Ledger R18/R35/R37/R38/R56/R57/R60/R62/R64/R68/R69/R76/R88–94/R101–105 advanced; RD-18 item 6 → "7a complete; closes at 7b". Full verify green ×7 phase boundaries; `spec/` clean. **Next: Slice 7b (pointer surface) make_plan; then Slice 8; RD-13/RD-14 queued.** Prior: **RD-18 Slice 7a 🔄 Executing (2026-07-11, exec_plan)** — all 13 accepted preflight fixes APPLIED to the plan docs first (the "apply fixes → exec_plan" sequence): register → 26 rows (+AR-25 `length` representability spec-drift ≤255 byte/≥256 word; +AR-26 mint **E10157 ExpressionStatementNotACall** for statement-head aggregate literals + string-initialiser loud Slice-8 ICE mechanism — the two pins PF-005/PF-007 left open), **E10164→E10097** rippled through register/03-04/03-07/ST-35/task-1.2.1/01-requirements/00-index, task 1.2.3 widened to `parseConstDecl` (PF-001), 03-06 §1 rewritten with the prescan def/read fixes + per-arm state-obligations block + stash-before-LDX (PF-002/PF-004), scaling-warning attribution corrected to ASL+W10172 pow-2 / `__rt_mul`+W10170 non-pow-2 (PF-003), new ST rows 6a/6b · 26a/26b · 44a/44b · 51a/51b · 53a · 54a, golden path + line cites fixed (PF-010), PF-011/012/013 recon notes folded in. Execution begins at Phase 1 (codes, AST & array-literal parser); 8 phases / 64 tasks; per-phase commit+push. Prior: **RD-18 Slice 7a 🔬 Plan Preflighted (2026-07-11, preflight)** — iteration 1: 0 crit / 5 major / 5 minor / 3 obs, **ALL 13 resolved per recommendation** (✅ PASSED; report `features/blend65-ri/plans/rd-18-slice-7-aggregates/00-preflight-report.md`; ~55 file:line refs verified by 3 recon agents + lead-context registry/spec reads; independent challenger converged on all five majors). Majors (all stale-claim plan amendments, no design rework): **PF-001** `parseConstDecl` (`parse-decl.ts:355`) parses flag-FALSE — const aggregate literals (incl. the fixture's own `const TABLE`) unparseable as tasked (the "let/const" claim was let-only; grammar-mandated fix: `const_expression = expression`) → task 1.2.3 widened + const-form parser STs; **PF-002** translate prescan NOT ready for the load ops despite "already enumerates" (`destTempId` misses `value`-as-dest; `readOperands` counts the load's dest as a read) + the byte-load "bind A" framing leaves its result un-homed — `sum = sum + a[i]` (the fixture's $C000 shape) dies in the E90001 backstop → Phase-6 prescan fixes + stash-to-home rule; **PF-003** power-of-two byte scaling emits **ASL + W10172** (`translate.ts:1224-1242`), NOT `__rt_mul`/W10170 as plan/AR-15/AR-17 claimed (2-byte `Point` = the canonical case) → prose/register corrected before it poisons the Phase-6/7 spec-test oracle, ASM-tier W10172 row added; **PF-004** the word-store framing's `LDX <idx>` destroys an A:X-resident source's high byte BEFORE it's read (X mirrors word hi bytes, `translate.ts:166`; one executor branch silently stores the index as data) → 03-06 §1 gains a per-arm state-obligations block (def visible to prescan, result homed, regA/regX truthful at exit — shared remedy with PF-002) + stash-before-LDX + a live-word-across-indexed-access test row; **PF-005** frozen Ch 08 §9 "length ≤256 elements → byte" unrepresentable at exactly 256 (`byte[256]` legal tier-1) → representability pin (≤255 → byte, ≥256 → word; TS-21's own shape) + spec-drift register row + `byte[255]`/`byte[256]` boundary STs. Minors: THIRD bare-name-table consumer (`intrinsic-validation.ts:188-209` via `ctx.tables`) added to the FQN migration; the AR-2/AR-18 promised loud rejections (statement-head aggregate literals — NO callable-statement check exists, `statement-typing.ts:317`; string array-initialisers) now tasked + STs; **E10164 mint retired → wire Ch 07's own free E10097** (the one outlier from the reuse-free-chapter-numbers policy); AR-10 row marked resolved-by-AR-4; golden path corrected (`packages/test-harness/test/golden/`, not root `test/golden/`) + five line-cite fixes. Observations: `needsDataInit` verified DEAD (zero consumers); `ConstDataEntry.type` carries `"embed"`; dotted-type `parse-type.ts` extension confirmed needed; `SymbolKind` already declares struct/enum/enumMember. **Next: apply fixes → exec_plan; then 7b make_plan.** Prior: **RD-18 Slice 7a 📋 Plan Created (2026-07-11, make_plan) — aggregates (arrays/structs/enums) through the DIRECT-addressing surface.** `features/blend65-ri/plans/rd-18-slice-7-aggregates/` (8 phases / 64 tasks), Zero-Ambiguity Gate ✅ PASSED (24 items — 20 at the gate + AR-21…24 authoring-surfaced; independent challenger converged on all 7 high-stakes picks). Slice 7 splits **7a/7b along the addressing-mode seam** (AR-1): 7a = decls/typing/tier-1 `abs,X` indexing/member access/enums complete/unified const evaluator/const data/intrinsic folds (aggregate returns = permanent E10093/E10120; aggregate params + >256 B arrays loudly rejected); 7b = by-ref params + const params + tier-2 `(ZP),Y` + unsized params (own plan; **RD-18 item 6 ticks at 7b**). Key decisions: **array literals are a NEW parser surface** (`[1,2,3]` does not parse — new `ArrayLitExpr`, 50→51 kinds; Ch-08 fill semantics `[vals; fill]`, grammar-appendix drift recorded); **chapters-beat-registry** (E10133 exhaustiveness / E10142 dup-values / E10141 cap stay unwired per Ch 09 EN-5/§8); ONE **lazy memoized const/type engine** (array sizes ⇄ struct layouts ⇄ module consts; exact path-carrying cycles — pure-struct → new E10165, const-involved → E10194; Parked Q1 = "no depth limit"); **cross-module type surface in 7a** + the verified bare-name declaration-table collision DEFECT (`passes.ts:35` — two modules' `struct Point` last-write-wins) fixed via FQN tables; strict tier index rules (E10117/E10118); lowering owns index scaling; value-dependent `sizeof`/`offsetof`/`length` typing; struct-literal declaration order (mint E10164); single-step enum casts. Codes: wire E10110–15/E10160–63/E10140+43 + E10077 first emission; register E10093/E10117/E10118/E10119/E10120/E10121/E10230; mint E10126/E10156/E10164/E10165 + W10140/W10141; `--bounds-check` deferred (no trap ABI/config key). Fixture: two-file `examples/slice7/` (Main + Gfx), `$C000..$C009` incl. const-expr-sized const table + scaled struct-array indexing + enum dispatch + cross-module reads. **Next: preflight → exec_plan; then 7b make_plan.** Prior: **RD-18 Slice 6 ✅ COMPLETE (2026-07-11, exec_plan 52/52) — full expressions & mixed width end-to-end, closes RD-18 AC-5.** Full operator matrix + TS-4 mixed-width promotion under ONE `isAssignableTo` rule (5a strict-arg interim superseded per spec §5.3/TS-4); unary `- ! ~` (E10087) / FR-40 casts (E10086/E10155) / ternary (E10134/E10088) / TS-17 compound assignment; width-aware const-eval (two's-complement folds, LAZY `&&`/`\|\|` + selected-arm ternary); short-circuit GUARANTEE lowered as CFG diamonds over synthetic `0sc<N>` SFA frame slots (preorder count + byte-size parity guards; `__init` pseudo-frame); comparisons carry the promoted OPERAND type at all three emission sites + translate grew all four byte/word × unsigned/signed framings — **fixing the latent DEF-1 word-compare low-bytes-only miscompile**; signed `/`/`%` → loud lowering rejection; non-const `lo`/`hi`; word + variable-count shifts; zext = zero-cost fold; the pinned branch-free sext; W10160/W10161/W10101/W10174 live. **3-part bar GREEN on real VICE 3.10**: `examples/slice6/main.blend` → `$C000..$C008 = E7 04 DA 05 07 00 01 44 00` incl. the short-circuit suppression proof ($C005=$00 — the side-effecting helper provably not called on two short-circuit paths; $C006=$01 — then ran exactly once); 293-line byte-exact golden (EIGHT `0sc` frame equates); all six prior goldens byte-exact NO re-mint; nine negatives/advisories via compile()/emitIl(). Bonus latent-defect fix: ACME accumulator-mode rendering (`ASL A` → bare `ASL`; the `A` parsed as an undefined symbol — exposed by the first real assembly of an accumulator op; zero golden re-mints). SR-2: 30 B RAM data at $2000 (incl. 8 B synthetic slots), ZP 10 B unchanged, binary 535 B, no new runtime routines. RD-04 ledger R31–R33/R40–R43/R49–R55 advanced. Full verify green; `spec/` clean. **Next: Slice 7 (arrays/structs/enums) needs `make_plan`; then Slice 8; RD-13/RD-14 queued.** Prior: **RD-18 Slice 6 🔬 Plan Preflighted (2026-07-11, preflight)** — iteration 1: 1 crit / 3 major / 6 minor / 3 obs, **all 13 resolved & applied** (✅ PASSED; report `features/blend65-ri/plans/rd-18-slice-6-expressions/00-preflight-report.md`; 60+ file:line refs verified, every 🔴/🟠 hardened by an independent challenger). 🔴 **PF-001** the DEF-1 operand-type fix covered only `lowerBinary` — `compareCounter` (`lower.ts:583`, the for-loop predicate = the plan's OWN motivating repro) and `lowerSwitch`'s dispatch `eq` (`lower.ts:517`; word/sword discriminants legal) also stamp `IL_BYTE`; the plan now stamps ALL THREE sites (byte operands unchanged → prior goldens byte-exact) + ST-23 widened to all three shapes. 🟠 the AR-3 supersession glob missed the load-bearing core pins (`type-utils.spec.test.ts:89/:107`, value-shaped, no E-code to grep — task 1.2.6 now names them); 🟠 ST-17 + ST-18's W10101 case were gated a phase ahead of the Phase-2 width folds they consume (moved to the Phase-2 spec set/gates; W10174 stays Phase 1); 🟠 `hi()`'s shr-by-8 lowering contradicted translate's word-fold ICE rule — respecified as a direct high-byte read (location +1, sign-correct for `sword`; computed-word/`sbyte` args = loud ICE). Minors: slot guard gains a size-parity check; switch-discriminant-with-slot-site over-claim documented as a loud-ICE limitation + witness; `FrameVar` snippet + poisoned-site placeholder type corrected; unary `~` context rule pinned (no-context per TS-9); lo/hi boolean arg → E10171; "unassigned in spec" wording fixed (spec DOES assign E10086 + the W-codes — why AR-10 picked them). **Next: exec_plan.** Prior: **RD-18 Slice 6 📋 Plan Created (2026-07-11, make_plan) — full expressions & mixed width, closes RD-18 AC-5 on completion.** `features/blend65-ri/plans/rd-18-slice-6-expressions/` (6 phases / 52 tasks), Zero-Ambiguity Gate ✅ PASSED (AR-1…AR-14; AR-14 authoring-surfaced — cast syntax = the shipped RD-03 FR-40 `<type>(expr)` prefix form; TS-11/grammar `as`-form drift recorded for the errata pass). Scope: the full binary matrix (comparisons TS-7 / logical / bitwise / shifts join arithmetic), TS-4 mixed-width promotion with ONE `isAssignableTo` rule everywhere (arguments/returns too — supersedes 5a's strict-arg interim, per-test documented), unary `- ! ~` (**E10087** mint), FR-40 casts (**E10086** mint; E10155 first emission), ternary (**E10088** mint; E10134 condition), TS-17 compound assignment, `&&`/`\|\|` short-circuit **guarantee** as CFG diamonds over synthetic `0sc<N>` SFA frame slots (source-illegal leading digit; preorder count-parity ICE guard; `__init` pseudo-frame for initializer expressions), signed relational comparisons (N⊕V byte+word) + word comparisons — fixing latent **DEF-1** (comparisons stamped `IL_BYTE` → translate compares LOW BYTES ONLY; a `word` loop bound miscompiles silently today), signed `/`/`%` → loud lowering ICE (signed `*` stays — truncated multiply is bit-exact), word + variable-count shifts, non-const `lo`/`hi`, width-aware const-eval (optional per-node type lookup + two's-complement helpers), warnings **W10160/W10161/W10101/W10174** minted (W10100/W10173 explicitly out); shift-amount signedness = never-emitted **E10083** key-renamed. Fixture: single-file `examples/slice6/main.blend` → VICE `$C000..$C008 = E7 04 DA 05 07 00 01 44 00` incl. the short-circuit suppression witness (`$C005==$00`, then one real call → `$C006==$01`); all six prior goldens + both assemble goldens byte-exact, NO re-mint. **Next: preflight → exec_plan.** Prior: **RD-18 Slice 5b ✅ COMPLETE (2026-07-11, exec_plan 42/42) — module system end-to-end, closes RD-18 AC-4 (Slice 5 fully ✅)** — merging (name-keyed shared scopes, cross-file dup → E10003 incl. a NEW duplicate-function guard, 5a dup-module ICE removed), full qualified-access value surface (value-first ladder, E10100/E10012 reuse, one shared callee ladder over both shapes, E10191 const writes, fn-as-value → ICE until Slice 8, SFA alias guard), call-free initializers (local-let parity; any call → loud ICE), per-variable init order (import-edge Kahn + stable topo; ONE E10194 per cycle + path), scalar const completion (VAR-6 order-independent eval, E10193, inlining — closes the verified `__frame_*` mis-lowering hole), `__init` stream (initCode + initTempCount realized, serialized FIRST, conditional `JSR __init` after banking via additive `hasInitCode` through five plugins; six prior goldens + both assemble goldens byte-exact NO re-mint; bare startup user-owned). **3-part bar GREEN on real VICE 3.10**: `examples/slice5b/` → `$C000..$C006 = 05/08/07/02/01/03/01`; 94-line golden; six negatives. SR-2: 7 B data + ~28 B init machinery, ZP unchanged. RD-04 ledger advanced (R17/R20/R21/R23, R64 scalar, R13, AC-09/AC-16; E10192 parser-owned; deviations + named deferrals recorded once); **RD-18 AC-4 CLOSED**. Full verify green; `spec/` clean. **Next: Slice 6** (expressions) needs `make_plan`. Prior: **Phase 3 ✅ (2026-07-11, init codegen + lowering arms, 32/42)** — 6 STs red→green: `lowerFieldAccess`/const inlining (closes the verified `__frame_*` mis-lowering hole), qualified assign/call arms, `initCode` realized (`__init` stream over `initOrder`, `moduleInit` ICE guard, NEW `initTempCount` + 13-literal sweep, printIL section), `generateInstr` translates `__init` FIRST, additive `hasInitCode` → conditional `JSR __init` after banking through the shared shim + five plugins (bare stays user-owned). 20 impl tests; six goldens + both assemble goldens byte-exact NO re-mint; full verify green. Next: Phase 4 (acceptance). Prior: **Phase 2 ✅ (2026-07-11, initializers/consts/init-order, 22/42)** — 11 STs red→green first run: `typeModuleLet` (local-`let` parity + call-free loud ICE incl. builtin intrinsics except `lo`/`hi`), const compile-time eval (VAR-6 order-independent, resolver-aware `evalConst` + shared range check → `constValues`), const/let cycles → ONE E10194 + path (Tarjan reuse), NEW `init-order.ts` two-level order (import-edge Kahn + stable per-variable topo, alias-skipped ordinals) → `initOrder`. 6 impl tests; frontend 433 green; full verify green. Next: Phase 3 (init codegen). Prior: **Phase 1 ✅ (2026-07-11, module merging + qualified access, 12/42)** — ST-1…ST-11 red→green: name-keyed shared module scopes + new duplicate-FUNCTION E10003 guard (stale "existing guard" claim), E90001 dup-module ICE dropped + `importEdges` recorded, `resolveQualified` ladder (value-first, E10100/E10012, platform ids excluded), `typeFieldAccess`/shared `typeCall` ladder/`typeAssign` arms (fn-as-value → ICE, const write → E10191), SFA `userCalleeOf` FieldAccess arm + `modelToModuleVars` alias guard (ONE `__var_*` slot for imported vars). 9 impl tests; full verify green; `spec/` clean. Next: Phase 2 (initializers, consts, init order). Prior: **RD-18 Slice 5b 🔬 Plan Preflighted (2026-07-11, preflight)** — iteration 1: 1 crit / 3 major / 5 minor / 1 obs, **all 10 resolved & applied** (✅ PASSED; report `features/blend65-ri/plans/rd-18-slice-5b-module-system/00-preflight-report.md`; ~50 file:line refs verified against the codebase, 48 exact; 1 independent challenger converged on all four high-stakes findings). 🔴 **PF-001** the acceptance fixture was written with a nonexistent `fn` keyword (the language keyword is `function`) — fixed across fixture + ST scenario tables; 🟠 **PF-002** the AR-4 call-rejection walk covered only `CallExpr` — builtins are their own `IntrinsicCallExpr` node kind, so `peek`/`peekw` would have silently lowered into `__init`; rule amended (reject both kinds except `lo`/`hi`, recursing lo/hi args) + new **ST-15b**; 🟠 **PF-003** ST-3/ST-7/ST-10 pinned function-local (top-level lets are untyped until Phase 2; a module-level ST-3 would be the AR-4 ICE, never "typed byte"); 🟠 **PF-004** `modelToModuleVars` alias guard added (an imported module variable double-projects a phantom `__var_<Importer>_*` slot — latent since 5a) + the AR-5 declIdx rule pinned to the declaring scope (aliases skipped). Minors: typeAssign lives in expression-typing.ts (task 1.2.5 retargeted); SIX existing goldens not five; AR-4 allowed surface corrected (unary is neither typed nor lowered today); ST-23 re-cited to AR-8 + §5.4:197-199 (spec §5.3 prescribes fall-through — the shipped `JSR _main` shim is the pre-existing RD-07c deviation, ledger row queued); `hasInitCode` optional-with-default so the five `emitStartupShim` delegations stay valid; sanitize() doc-comment staleness noted. No gate decision changed; the register carries a preflight-corrections note. **Next: exec_plan.** Prior: **RD-18 Slice 5b 📋 Plan Created (2026-07-10, make_plan)** — module-system completion, **closes RD-18 AC-4 on completion**. `features/blend65-ri/plans/rd-18-slice-5b-module-system/` (5 phases / 42 tasks), Zero-Ambiguity Gate ✅ PASSED (AR-1…AR-13 + imported I-1…I-3; 3 recon agents + 1 independent challenger — converged on all five high-stakes picks, surfaced the `--startup bare` hole). Scope: module merging (R20, name-keyed shared scopes, cross-file dup → E10003, replaces the 5a dup-module ICE); **full qualified-access value surface** (`Math.fn()` + `Math.v` reads + writes, exported-only, value-first heads, E10100/E10012 reuse, call-graph/SFA parity rider, `Math.fn`-as-value → ICE until Slice 8 `&fn`); **call-free module-var initializers** (local-`let` parity typing; call → loud ICE) + ONE global per-variable init graph, spec-literal two-level order (import-edge Kahn then stable per-variable topo), ONE E10194 per cycle with path (Tarjan reuse); **scalar const completion** (const-eval + `constValues` + E10193 + use-site inlining — closes a VERIFIED latent hole: module-const refs mis-lower to an unallocated byte-defaulted `__frame_*` symbol); `ILProgram.initCode` realized as a synthetic `__init` stream + additive `PreambleOptions.hasInitCode` → conditional `JSR __init` (initializer-free programs byte-identical — all five prior goldens stay minted; bare startup documented user-owned). Fixture: 3 files (both math files `module Math`; Main discovered first, Math inits first — import-edge order load-bearing); VICE `$C000..$C006 = 05/08/07/02/01/03/01`. **Next: preflight → exec_plan.** Prior: **RD-18 Slice 5a ✅ COMPLETE (2026-07-10, exec_plan 46/46)** — user functions/params/calls/recursion/imports ship end-to-end; **3-part bar GREEN on real VICE 3.10**: two-module `examples/slice5a/` → `$C000==$11` add(10,7) / `$C001==$84`+`$C002==$03` triple(300)=$0384 / `$C003==$10` combo(5); byte-exact golden. **Phase 0 retired the 13-byte data ceiling** (`ramStart` `$0800`→`$2000` + mandatory plan-keyed post-ACME overlap check on the E10033 band; five goldens re-minted equate-only + VICE re-verified; closes 3b AR-1/SR-3). Frontend: param collection (E10003, FN-13→E10101), `typeCall` ladder (E10100→E10051→E10023→E10175; E10170/E10171 strict same-type + E10084), return completion (E10172 + assignment family with return wording), Pass-3 edges + iterative Tarjan → **ONE E10174 per cycle with the full path** + a NEW `hasErrors`→skip-`planAllocation` driver gate, `import-resolution.ts` (E10012, user-module-wins, same-Symbol aliasing, dup-module-name unsupported-ICE until 5b); registry E10175→`NotCallable` + E10051 minted. SFA: params-first frames, sorted callee FQNs, **argument-window interference** (`main → f(1, g())`, `g → h` compiles correctly — f/h frames disjoint). Codegen: `lowerCall` store-per-arg + bare `call` with the AR-3 residual ICE; translate `call` = `JSR Module_function` + A/A:X bind with the AR-4 remaining-use-ledger guard (`f()+g()` → "value live across a call" ICE; `f(g(1),2)` compiles). SR-2: 9 B data at `$2000` (2 B saved by frame sharing), ZP 10 B unchanged, `__rt_mul16` embedded. RD-04 ledger advanced (R58/R22/R66/R80/R81/R84–R87 + AC-07/AC-15; R10/R13/R65 partial) + deviations recorded once (E10175 rename incl. Ch 06 §10 vs Ch 14 inconsistency; FN-11 no-param-limit; E1017x chapter drift; JSR-startup scoped; AR-3/AR-4/AR-13 named deferrals). RD-18 AC-4 "5a partial ✅; closes at 5b". Full verify green; `spec/` clean. **Next: Slice 5b** (module merging + `Module.fn` qualified access + call-free initializers/E10194) needs `make_plan`. Prior: **RD-18 Slice 5a 🔬 Plan Preflighted (2026-07-10, preflight)** — iteration 1: 0 crit / 2 major / 4 minor / 1 obs, **all 7 resolved & applied** (iteration 2: 0 new findings; report `features/blend65-ri/plans/rd-18-slice-5a-functions-calls/00-preflight-report.md`; 3 recon agents + 1 independent challenger, converged on both MAJORs). Both MAJORs = stale existing-code claims at never-miscompile seams: **PF-001** AR-4 live-temp guard → NEW **separate** remaining-use map (the static prescan `useCount` provably cannot distinguish must-compile `f(g(1),2)` from must-ICE `f()+g()`; per-consumed-operand-occurrence decrement, never mutate `useCount`); **PF-002** the `hasErrors`→skip-`planAllocation` gate does NOT exist → new driver-level gate in `run-frontend` guarding the whole call expression (inline `modelToFunctionInfo` + its AR-3 `reach()` DFS included), all new reachability DFS walks visited-set-bounded. MINORs: PRG load-address read-back must be built, size = header-excluded `binarySize` (PF-003); E10175 rename divergence vs the canonical Ch 14 registry recorded (PF-004); duplicate module name across files → explicit unsupported ICE until 5b merging (PF-005); `lowerCall` unresolvable-callee ICE fallback (PF-006); phantom helper name fixed (PF-007). No gate decision changed; the register carries a preflight-amendment note. **Next: exec_plan.** Prior: **RD-18 Slice 5a 📋 Plan Created (2026-07-10, make_plan)** — user functions/params/calls; Slice 5 split 5a/5b at the gate (AR-1). `plans/rd-18-slice-5a-functions-calls/` (6 phases / 46 tasks), Zero-Ambiguity Gate ✅ PASSED (AR-1…AR-16; 3 recon agents + independent challenger). 5a = the calling-convention vertical: param collection, call typing (E10170/E10171 + **E10175 repurposed `NotCallable`** + new mint **E10051** interrupt-call + E10023), return completion (E10172 + assignment family), call graph + one **E10174** per cycle (Tarjan, pre-SFA poison), imported calls (E10012), SFA feed (params/callees + challenger-hardened **argument-window interference**, AR-3), `lowerCall` store-per-arg + translate `call` (JSR + A/A:X bind) with two never-miscompile ICE guards (same-callee-in-later-arg AR-3; live-temp-across-call AR-4). **Phase 0 retires the 13-byte data ceiling** (`$0800`→`$2000` + mandatory post-ACME overlap check; closes 3b AR-1). 5b (merging + `Module.fn` + call-free initializers/init order E10194) closes RD-18 AC-4. Fixture: two-module `examples/slice5a/` → VICE `$C000==$11`/`$C001==$84`/`$C002==$03`/`$C003==$10`. **Next: 5a plan preflight → exec_plan.** Prior: **RD-18 Slice 4b ✅ COMPLETE (2026-07-07, exec_plan 26/26)** — the `switch`/`case`/`default`/`fallthrough` sub-machine ships end-to-end, **closing RD-18 AC-3**. **3-part bar GREEN on real VICE 3.10**: `examples/slice4b/main.blend` (multi-value case + `fallthrough` + auto-break + `default`) → `$C000==$19` (25) / `$C001==$07` (7); byte-exact ASM golden; negatives reject via `compile()` (E10071/E10132/E10075). Semantics: `typeSwitch` (`statement-typing.ts`) — E10075 operand / E10071 non-const / E10084 range / E10077 bespoke type-match (wired, emission→Slice 7) / E10132 duplicate / E10074 fallthrough-position / E10073 no-effect warning; case-body locals collected flat; break/continue switch-transparent (AR-6). Codegen: `lowerSwitch` (`lower.ts`) — a `brcond` **compare-chain** over the 4a CFG keystone (multi-value shared body, `fallthrough`→`br(next body)`, auto-break→`br(join)`, default tail); **zero** new IL terminator / translate work (AR-1). Five codes registered additively (E10071/E10073/E10074/E10075 + new mint E10077); **E10076** duplicate-`default` deferred parser-owned (unreachable at semantics — parser silently overwrites, PF-001). RD-04 ledger R75/R79 advanced; R76 exhaustive-enum switch (E10133) stays deferred → Slice 7 (AR-2). Full workspace verify green; `spec/` clean. **Next: Slice 5** (functions/params/calls) needs `make_plan`. Prior: **RD-18 Slice 4b 🔬 Plan Preflighted (2026-07-07, preflight)** — iter 1: 0 crit / 3 major / 2 minor / 1 obs, **all 6 resolved & applied** (3 recon agents + 1 challenger; report `plans/rd-18-slice-4b-switch/00-preflight-report.md`). Contract changes: **E10076** duplicate-`default` **dropped** (unreachable at semantics — parser silently overwrites; deferred parser-owned, PF-001); **E10077** case-type-match kept registered+wired but emission **deferred to Slice 7**, precedence E10071→E10084→E10077 ("TS-4 auto-promotion" was fictional, PF-002); out-of-switch `fallthrough` ICE fixture **kept** (not repointed) + gap deferred (PF-003); mint corrected to **five** codes (PF-004) + spec cites fixed (PF-005). Next: exec_plan. Prior: **RD-18 Slice 4b 📋 Plan Created (2026-07-07, make_plan)** — the `switch`/`case`/`default`/`fallthrough` sub-machine (deferred from 4a, AR-1). `plans/rd-18-slice-4b-switch/` (4 phases / 26 tasks), Zero-Ambiguity Gate ✅ PASSED (AR-1…AR-14). Recon: switch is fully parsed — 4b is semantics + one `lowerSwitch` case (a `brcond` **compare-chain** over the 4a CFG keystone; no parser/translate/new-IL-terminator work). Gate decisions: compare-chain (jump-table→Phase B); integer-only, enums+E10133→Slice 7; fallthrough-in-default = E10073 warning (dissolves Parked-Q4); case-type-mismatch → new **E10077** (E10072 taken by parser `MissingDefaultClause`). Closes RD-18 AC-3. Prior: **RD-18 Slice 4a ✅ COMPLETE (2026-07-07, exec_plan 35/35 + DEF-1)** — conditionals + loops + the first **multi-block CFG codegen keystone** ship end-to-end (`if`/`else`/`while`/`do-while`/`for` `to`/`downto`/`step`). **3-part bar GREEN on real VICE 3.10**: `examples/slice4a/main.blend` (for-loop with `break`/`continue` + a while + a two-armed if/else) computes `21` — `$C000==$15`, `$C001==$01`; byte-exact golden (labels `Main_main_L0..L13`, Pattern-A `le`/`add`, break→L3/continue→L2); a non-void function missing a return → **E10102** via `compile()`. Phase 1 semantics (`function-collection.ts` flat body-local recursion; `statement-typing.ts` `loopDepth`+`typeCondition`+`typeFor`; `post-check.ts` all-paths `checkAllPathsReturn`), Phase 2 `lower.ts` loop-context CFG lowering, Phase 3 `translate.ts` multi-block loop (`prescanAll()`+per-block `resetBlockState()`, PF-001). New codes E10134/E10061/E10065/E10102 (additive, AR-11); break/continue E10130/E10131. **DEF-1/AR-16**: latent RD-07b `translateComparison` Z-flag clobber → `eq`/`ne` always 0; **fixed eq/ne only** (branch after `CMP`), goldens re-minted + regression added. Phase 5: RD-04 ledger R71–R74/R77/R78/R80/R11 + AC-12/AC-17 advanced; RD-18 AC-3 "4a partial ✅; closes at 4b (switch)". Full verify green; `spec/` clean. **Next: Slice 4b** (`switch`) needs `make_plan`. Prior: **RD-18 Slice 4a 🔬 Plan Preflighted (2026-07-07)** — preflight iter 1: 0 crit / 1 major / 3 minor / 0 obs, all 4 resolved (PF-001 keystone block-state fix; PF-002 new E10065). Prior: **RD-18 Slice 4a 📋 Plan Created (2026-07-06)**. Prior: **RD-18 Slice 3b ✅ COMPLETE (2026-07-06, exec_plan 45/45)** — scalar type/scope engine end-to-end, **3-part acceptance bar GREEN on real VICE**: Phase 1 scalar type engine (Pass 3 `type-check/*`, same-type `isAssignableTo`/`commonType`, Pass 4 `post-check.ts` main() E10020/E10021/E10022, + registered E10084/E10022, AR-11); Phase 2 module scalars (`module-variable-collection.ts` + `modelToModuleVars`); Phase 3 width-aware lowering (word literals `i16u`/`word*word`→`__rt_mul16`; module scalars→`__var_*`); Phase 4 acceptance — `examples/slice3b/main.blend` assembles clean (real ACME) to a loadable PRG, byte-exact golden (`__var_Main_accB=$0800`, `__rt_mul8/16`; footprint 10B ≤ 13B shadow, AR-1), **real VICE 3.10 `$C000==$11`/`$C001==$58`/`$C002==$02`**, mixed-sign E10081 negative via `compile()`, gate/slice3a goldens unchanged; Phase 5 bookkeeping (RD-04 AC-02/03/04/06/08/13/14 scalar-subset ticked, **PF-004** AC-05→E10081 correction, RD-18 AC-2 ✅, SR-2/SR-3). Full verify green (frontend 87, compiler 82, codegen 335, test-harness 78, R15 boundary); `spec/` clean. Runtime AR-12 + AR-13 resolved. **Next: Slice 4** (control flow) needs `make_plan`. Prior: 🔬 Plan Preflighted (2026-07-05) — `plans/rd-18-slice-3b-scalar-type-engine/`: 5 phases / 45 tasks, Zero-Ambiguity Gate ✅ PASSED (AR-1..AR-11); **preflight 2 crit / 3 major / 4 minor / 1 obs, all 10 resolved & applied (iter 2)** — root cause diagnostic-code drift (codes taken from stale frozen-spec §5.3 + false "all codes exist" claim), fixed by **AR-11 code-reconciliation** (register E10084 + E10022 per RD-18 AR-115; realign boolean-arith→E10080 / narrowing→E10154 / cross-sign→E10153 / boolean-assign→E10152; drop E10086); structural build verified sound; report `00-preflight-report.md`. The scalar **type engine**: real RD-04 Pass 3 (expression/literal typing → `typeMap`/`symbolMap`, real `isAssignableTo`/`commonType`, poison) + Pass 4 (`main()`) + minimal const-eval; module-level scalar allocation (`__var_*`); width-aware lowering (thread `typeMap` → `__rt_mul16`). Recon (3 agents): codegen already lowers the surface — the build is semantics + wiring. Gate: same-type only (AR-3), module-var initializers deferred (AR-2), `$0800` region within the 13-byte BASIC-stub shadow (`>13-byte` collision = documented deferred fix, AR-1). Next: exec_plan. Prior: Slice 3a ✅ COMPLETE (2026-07-05, 21/21) — `modelToFunctionInfo` seam closed, VICE `$D020==0xF5`. Earlier: RD-12 ✅ COMPLETE (44/44); RD-15 ✅ COMPLETE (50/50) | 18/20 | 🔄 | 2026-07-11 |
