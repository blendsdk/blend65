...
| asm-parity | [→](features/asm-parity/00-roadmap.md) | **RD-01 ✅ done** (parity infra) · **RD-02 ✅ done** (#61 — 14-pair twin corpus, twin tier, committed SCOREBOARD.md baseline 4.83×/6.51×, routed audit, CI freshness gate) · **RD-04 🔄 executing** (#50 — `brcmp` fusion, 5 phases/43 tasks; phases 1–3 done — terminator + dangling-target ICE, all five framings fused behind one compare core, and the `guards` acceptance pair with its pre-fusion baseline at 2.71×/2.68×; still zero golden diffs) — next: phase 4, the atomic flip | 2/14 RDs | 🔄 | 2026-07-19 |
