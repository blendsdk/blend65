...
| `C64-AUDIO-*` | Hash-pinned GoatTracker 2.77 release members and player source, HVSC format boundary, C64 framework audio path, Oscar64 SFX-only comparison, SID Factory II candidate docs, and reSID/VICE sources | Player-neutral contract design; music-only/integrated/SFX-only/custom paths; exact init/tick/SFX ABI, cadence, writable state, arbitration and resource ownership; register-level behavior and emulator/revision boundaries. Candidate evidence never grants a callable ABI. |
