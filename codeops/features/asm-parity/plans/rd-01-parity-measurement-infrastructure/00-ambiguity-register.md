...
| 12 | Technical (runtime) | **Loop-shaped window slicing** (surfaced executing Phase 4): balloon's `frameUpdate` window has no runtime-arriving label after its final pokes — the static text bound (`Main_main_L2`, after the back edge) never executes, while the measured arrival bound (`Main_main_L0`, the loop head) sits address-wise BEFORE the body, so one label pair cannot drive a naive `[from, to)` address slice for both metrics | Single viable path implemented: static slice for `to ≤ from` runs from the body start through the first back-edge transfer targeting the end label — the SAME label pair (`Main_main_L5` → `Main_main_L0`) serves the static text sum and the measured arrival window, keeping plan-AR #3's schema intact. Named rejected: extending the schema with a `measuredToLabel` field (deviates from the user-approved shape); a degenerate `staticMaxCycles: 0` row on an empty slice (games ST-14/ST-19's intent) | ✅ Resolved — single viable path, per the grounded-options directive; flagged in the session summary for review | ✅ Resolved |
| 13 | Technical (runtime) | **slice8b measured window is not externally quiesce-able** (surfaced executing Phase 4, live-measured): the VIC latches badline-enable when DEN is set at raster line $30, so a mid-frame display blank cannot suppress the CURRENT frame's badlines — and slice8b's one-shot copy window always runs inside the boot frame. Measured distribution across launches: 568 + k×43 cycles, k ∈ {0,1,2} (0–2 phase-dependent badlines) | Single viable path: drop `measuredMaxCycles` from slice8b's copyLoop (schema keeps it optional); the exact static budget stays, and the measured guarantees are carried by balloon's frameUpdate (deterministic 162, ST-19) + the IRQ demo. Named rejected: seeding the observed max 654 (a ±86-cycle gate flakes on real regressions); parking the PC in scratch RAM to skip a frame before measuring (novel C64-specific choreography, disproportionate for a second data point) | ✅ Resolved — single viable path; flagged in the session summary | ✅ Resolved |
