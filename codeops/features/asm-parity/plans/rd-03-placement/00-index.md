...
That is not ceremony. The **RD-stage** preflight's most dangerous finding (its PF-001 — the
plan-stage report numbers independently) was that `&X` and an ordinary **by-reference array
argument** emit the *same* IL `addrOf` operand — an implementation
