...
> **Status**: In Progress
> **Created**: 2026-07-17
> **Last Updated**: 2026-07-21 (**RD-15 ✅ DONE** — alignment granularity shipped in 3 phases; the deliverable is the bound, not the bytes: `balloon-color` −128 B in a program with no budget, corpus total unmoved, every block image's pad now under 64.), gate passed on
> AR #101–#110 + #111–#112 surfaced during authoring (12 items). The open design question RD-03 left behind is settled **without touching frozen
> `spec/`**: the boundary is derived from the divisor the source already writes, so `lo(&X / 64)`
> demands 64 and a bare `&X` keeps 256, combined **coarsest-wins** — which keeps `hi(&X) * 4` correct
> *by construction* rather than by caution, the same discipline as AR #87. No `@align(n)` attribute
> (FUT-014, D3-blocked), no consumption inference (it re-imports the IL-scan mistake RD-03 rejected
> for cause). The demand set is an **allowlist of hardware granularities, not the divisor**:
> `lo(&X / 16384)` is a legitimate VIC-bank *read*, not a request to 16 KB-align an image, and
> honouring it as one would insert up to 16 KB of padding — 256 is not a VIC granularity at all, it
> is only RD-03's default. **#69's headline figures were stale** (filed pre-RD-13) and were
> re-measured across all four address-taken programs at HEAD: `balloon-color` is 188 of 584 B, and
> **60 B at 64-byte, not the 1 B #69 claims** — that 1 B was a lucky landing under pre-RD-13 code
> sizes, which is precisely the volatility the RD exists to bound. **The corpus byte delta is 0 and
> the RD says so in its own acceptance criteria**: `balloon` is the only budgeted address-taken
> program and it pads 19 bytes under either boundary, so AC asserts invariants and a bound
> (pad < 64), never a byte count — a criterion phrased as "corpus total decreases" would fail on a
> correct implementation, which is exactly how RD-13's AC-8 was lost. That bound *is* the
> deliverable: it caps byte-ratchet noise at 63 instead of 255, the noise that swallowed RD-13's
> real 13-byte win whole. An independent challenger on a different model family, given the five
> candidate designs **without** the author's preference, converged on the same one and returned
> three corrections that were each verified against the tree before adoption — the page-alignment
> oracle inventory was **six, not four** (ST-13f fails deterministically; **ST-C15 and ST-13j pass
> by luck**, `$08ED`→`$0900` and `$0AFF`→`$0B00` rounding identically under both boundaries, so CI
> would never flag them and they would surface later disguised as a regression in whatever changed
> the code size); generalized divisor inference is *unsound* rather than merely unclamped; and
> **RD-13:157-159 predicts this RD makes `hi(&X) * 4` incorrect** — false under the maximum rule,
> and contradicted by RD-13:439 on the same page, so it is corrected here before someone implements
> uniform-64 to fulfil it. Three of the twelve register items exist only because of that challenger.
> Next: `preflight` the RD. — prior: **RD-13 ✅ CLOSED** — 54/54 tasks across 5 phases. The sprite-pointer
> idiom went **18 B / 24 cyc → 5 B / 6 cyc** and `balloon`'s pointer store is now
> instruction-for-instruction its hand-written twin, that divergence row **deleted** rather than
> re-routed. **The corpus byte total did not move**: the image is `!align 256`, so all 13 bytes
> removed from balloon's code were absorbed by padding growing 6 → 19 — recorded as AC-8 failing on
> bytes and passing on cycles (4237 → **4219**) rather than reconciled away. The obvious consolation
> — that 64-byte alignment would recover them — is **false and recorded as false**: balloon lands on
> `$0900` either way, so its pad is 19 under both. 16 rows re-routed off #58 with a structural check
> that no row carries it. AR #100 at runtime; **[#73](https://github.com/blendsdk/blend65/issues/73)**
> spun off. Earlier, **plan preflighted** — two iterations, a 5-cluster fan-out
...
| RD-15 | Alignment granularity: 64-byte sprite blocks ([#69](https://github.com/blendsdk/blend65/issues/69)) | [RD](requirements/RD-15-alignment-granularity.md) | [Plan](plans/rd-15-alignment-granularity/00-index.md) | Done | ✅ | 2026-07-21 | **✅ DONE 2026-07-21 — 3 phases / 24 tasks, all 15 acceptance criteria + P-1/P-2/P-3 discharged** ([closeout](plans/rd-15-alignment-granularity/08-closeout.md)). A const image's boundary now follows the arithmetic the source writes: normalized shift 6 demands 64, everything else 256, coarsest-wins — so `hi(&X) * 4` stays correct structurally, with no caution anywhere. An allowlist of one, minted at the single `lowerAddressOf` call site that knows a unit; `/ 16384` is a VIC-bank *read* and is declined by design rather than by accident. **The deliverable is the bound, not the bytes**: `balloon-color` recovers 128 B (582 → 454) in a program carrying no budget, `balloon` recovers nothing and stays 318 B, and the corpus total does not move — what changes is that a block image's pad is bounded under 64 (measured 19, 60, 1) instead of under 256, a measurement-hygiene property every later size RD inherits. Phase 1 landed behaviour-neutral and proved it; Phase 2 review returned no findings. Every guard that was already green before the implementation was perturbed once and watched to fail — this feature manufactures unfailable guards, since 64 and 256 coincide on most real addresses. VICE 3.10 green on `balloon` and `boing-ball`; `balloon-color`, the one image that moves, got a recorded one-off look — image at `$0980`, pointer 38, 63 bytes matching the committed asset. Prior stages: **EXECUTING — Phases 1–2/3 complete 2026-07-21.** The 64 demand is live: `boundaryOfShift` is an allowlist of one (normalized shift 6 → 64, everything else → 256) minted at the **single** `lowerAddressOf` call site that knows a unit, so the other eight still register a page and `hi(&X) * 4` stays correct structurally. `balloon-color` moves `$0A00 → $0980` and drops 582 → 454 B; `balloon` recovers nothing and stays 318 B, `boing-ball` stays at `$0B00`, and the corpus total does not move — the deliverable is the bound, every 64-demand pad now under 64 (19, 60, 1). Seven new end-to-end criteria (ST-15a…g) and three re-derived oracles, each pinning `!align 63, 0, 0` alongside the address because a multiple of 256 is already a multiple of 64. Every pre-green guard was perturbed and watched to fail before being trusted. Phase 2 review returned no findings. Prior:  The mark is now a value: `ConstDataEntry.pageAligned: boolean` → `boundary?: AlignBoundary` (`64 | 256`), `LowerCtx.addressTakenConsts` → `alignmentDemands: Map<string, AlignBoundary>`, `lowerAddressOf` carries a defaulted demand, and `instr-program`'s `const PAGE = 256` is gone rather than moved. Sixteen test sites reshaped, with `false` becoming an **absent** field rather than `boundary === 256` — the distinction the by-reference membership rule rests on. Behaviour-neutral and proven so: 14 goldens byte-identical, no budget/ratchet/scoreboard movement, and the three fold-form oracles still assert `% 256`, untouched and green. Post-phase review returned 2 minor findings (a missing barrel re-export, and Prettier drift the rename itself introduced), both applied. Next: Phase 2 mints the 64 demand and `balloon-color` moves to `$0980`.Prior: plan preflighted — 10 findings (0 critical / 1 major / 6 minor / 3 observations), all resolved same-day; no design, phase or test-inventory change.** The major: the red-phase table filed `ST-13f` as going RED on both clauses, but `balloon-color` sits at `$0A00` today and `$0A00 % 64 === 0` — so the `% 64` clause of *all three* re-derived oracles is green pre-implementation, and all three had fallen outside the perturb-and-restore set the plan mandates for exactly this reason. Three guards on the property the feature establishes would have shipped unproven. Also caught: `PAGE_BOUNDARY` was consumed in Phase 1 but created in Phase 2 (Phase 1 would not have compiled), `RD-15:271`'s `Math.max` insert does not typecheck under `AlignBoundary` and was missing from the back-propagation list, and the `lowerAddressOf` call-site count was nine, not ten. Four blind auditors on a different model family; the miscount was found independently by three and the ST-13f defect by two. Everything structural held — the 16-site sweep is complete, no snapshot or golden captures the IL shape, a demand can never orphan, and every measurement reproduced. **Plan created same day — 3 phases / 24 tasks, gate passed on AR #113–#121 (9 items).** Phase 1 lands the data-shape migration **behaviour-neutral** — every demand still 256, sixteen `pageAligned` sites reshaped, goldens byte-identical — so the one behavioural commit (Phase 2) is bisectably separate from twelve spec-oracle edits; Phase 3 is ledgers and closeout. Two gate decisions moved off the RD's own text and are back-propagated in Phase 3: **AR #113** passes the *derived boundary* rather than the normalized shift and closes the value set at the type (`AlignBoundary = 64 | 256`) on the parameter, the demand map **and** `ConstDataEntry.boundary`, so a non-power-of-two `!align` — which assembles cleanly and aligns nothing — is unrepresentable at every producer rather than guarded by a comment at one (a blind challenger raised this option and moved the lead off keeping the allowlist at the mark site); **AR #121**, surfaced while authoring the test strategy and measured against the real compiler, found AC-5's `lo(&X / 65536)` clause untestable — `65536` exceeds the 16-bit maximum, so the lexer rejects it with `E10216` before lowering runs and there is no boundary to keep — clause dropped, the k=0 and k=15 extremes retained. Also recorded: the red-phase table separating the assertions that genuinely fail today from the pre-green guards, since 64 and 256 coincide on most real addresses; and AR #111/#112's ledger edits were already applied during RD authoring, so Phase 3 verifies rather than re-applies them. Earlier: **RD PREFLIGHTED 2026-07-21 — 18 findings (0 critical / 2 major / 14 minor / 2 observation), all resolved same-day; every recommendation accepted, fix application queued.** Five-cluster fan-out plus a blind challenger that converged on both majors: (1) the fold shape is legal on **mutable** arrays and functions, where no alignment path exists and the Security section's “removed by construction” over-reached — closed with a Won't-Have, one M1 sentence, a numbered AC pinning the new shift parameter inert outside the const branch, and the undiagnosed hazard filed as [#74](https://github.com/blendsdk/blend65/issues/74); (2) the three re-derived `% 64` oracles are **unfailable against a 256-regression** — every current image lands ≡0 mod 64 under both boundaries — closed with ST-C11-style directive-text assertions in all three example suites, the discriminator AR #108 itself names as the only deterministic one. Also caught: AC-5's claimed k=15 extreme unreached by any listed divisor (found independently by all five clusters), a 16th `pageAligned` site (blast radius 16, not 15), the Should-Have's platform allowlist cited at the wrong pipeline seam (`LowerInput` has no platform field), and AC-11 mixing `.prg` file size with the budget convention (456 → 454). [Report](requirements/00-preflight-report-rd-15.md). Earlier: **Unblocked by RD-13 ✅.** RD drafted 2026-07-21, gate passed on AR #101–#110 + #111–#112 surfaced during authoring (12 items). The open design question is **settled without touching frozen `spec/`**: the boundary is derived from the divisor the source already writes (`lo(&X / 64)` → 64, bare `&X` → 256), combined **coarsest-wins**, which keeps `hi(&X) * 4` correct by construction rather than by caution — no `@align(n)` attribute (FUT-014, D3-blocked), no consumption inference. The demand set is an allowlist of hardware granularities, **not** the divisor: `lo(&X / 16384)` is a VIC-bank read, not a request for 16 KB alignment. **#69's figures were stale** (filed pre-RD-13) and are re-measured at HEAD: `balloon-color` 188 of 584 B, and **60 B at 64-byte, not 1**. **Corpus byte delta is 0** — `balloon` is the only budgeted address-taken program and it pads 19 either way, so acceptance is invariants + the bound (pad < 64), never a byte count; that bound is the real deliverable, capping ratchet noise at 63 instead of 255 (the noise that made RD-13's 13-byte win invisible). Blast radius found by an independent challenger: **six** page-alignment oracles, not four — ST-13f fails deterministically, ST-C15 and ST-13j pass **by luck** ($08ED→$0900, $0AFF→$0B00 round the same under both). Also corrects RD-13:157-159, which predicts this RD makes `hi(&X)*4` incorrect (false, and it contradicts RD-13:439) |
...
| T-03 | Retire the dead byte-select scratch slots ([#73](https://github.com/blendsdk/blend65/issues/73)) | — | — | Backlog | ⬜ | 2026-07-21 | **Filed 2026-07-21 from RD-13 Phase 2's review.** Every `lo(&X)`/`hi(&X)` still claims a 2-byte word scratch slot that the byte select no longer reads or writes — 4 B of dead frame RAM per pair, zero binary bytes. The claim was kept deliberately: it is what keeps the **positional** slot counter aligned between the SFA planner and lowering, and retiring it on one side alone would shift every later slot index and silently mis-home an unrelated value. The fix stops counting byte-select sites in the slot preorder on **both** sides in one change — the SFA adapter and `claimResultSlot` — after which ST-9b's trailing homing site correctly names `_0sc0` rather than `_0sc2`, re-derived deliberately. Small in isolation and unbounded in the wrong direction: the waste grows with exactly the idiom the compiler now encourages |
