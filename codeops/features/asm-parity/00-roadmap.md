...
**The next pick is open.** RD-13 (#58, const-fold — whose groundwork RD-05 deliberately built as a
separately schedulable pass) now owns both divergences RD-03 measured and filed, which makes it the
natural follow-on; RD-06 (#52, INC/DEC peephole) is the smaller entry and owns the +61-byte
code-stream half of balloon's residual. Note also that **#59 is now the top divergence by breadth**
(17 rows across 13 of 14 pairs) while the roadmap still tiers it B3 — that tiering no longer matches
the data and is worth revisiting.
