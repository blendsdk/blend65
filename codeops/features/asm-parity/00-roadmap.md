...
> **Status**: In Progress
> **Created**: 2026-07-17
> **Last Updated**: 2026-07-19 (RD-04 🔄 executing — phases 1–4/5 done (40/43 tasks). **Phase 4 = the flip, landed**: every condition now branches on its comparison's own flags — `lowerCondition` fuses comparisons, makes `!` a pure label swap, turns `&&`/`||` into CFG edges claiming no frame slot, and folds boolean literals; the SFA slot rule moved in step (position-dependent, still no codegen import). 8 of 14 goldens regenerated and hand-reviewed; `slice6` — all value-position — did not move a byte, which is the corpus-level proof fusion stayed in condition position. Corpus 4172→3896 bytes, 5340→5023 static cycles; guards 2.71×→2.05× / 2.68×→2.02×; rasterpoll poll path 12 cycles (AC-1 met); guards compound guard 43→24. The #50 routing rows are gone from all three pairs that carried them — that disappearance is the evidenced fix. Review 2🟡: stale doc comment fixed; a PRE-EXISTING switch-on-ternary ICE filed as #66. Next: phase 5 — closeout)
> **Progress**: 2 / 14 (14%)
