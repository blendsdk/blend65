...
| AR-037 | Scope / M1 product and asset provenance | Does M1 remain a single-sprite interaction or become a bounded Invaders-style game, and when must authentic SpritePad 3.80 evidence exist? | Keep the single-sprite loop / bounded original-art Invaders-style microgame using raw bytes now and SpritePad later / faithful full game recreation | Use the bounded original-art eight-sprite Invaders-style M1 with no multiplexer, audio, loader, IRQ callback, or optimizer. RD-03 uses one checked-in 512-byte raw asset through the existing `embed(path)` contract and proves final placement, bytes and runtime behavior without a native-format parser. Authentic SpritePad 3.80 import and qualification move to RD-06 after the correct unoptimized compiler; missing producer evidence does not block RD-03 or RD-04. | ✅ Resolved |
...
### AR-037 — Bounded Invaders-style M1 and deferred SpritePad handoff
...
arithmetic, source-level collision, eight native sprite records, joystick input, frame updates,
...
**Initial user decision:** The user approved the bounded Invaders-style M1 and initially confirmed
that the SpritePad project would be supplied before the format-dependent work began.
...
**Sequencing amendment, 2026-09-20:** The user confirmed that SpritePad is unavailable and approved
deferring its native import until after the compiler is complete. RD-03 now consumes one checked-in
512-byte raw file through Specification 4's existing `embed(path)` contract. The file contains eight
64-byte native sprite records and has an independent generation recipe, expected bytes and SHA-256.
RD-03 still proves one-copy placement, VIC visibility, ACME output, PRG bytes and VICE behavior.
It adds no substitute format, parser, plugin seam or source restriction. RD-06 is the first owner of
the `.spd` handler and authentic SpritePad 3.80 producer qualification. This amendment supersedes
only the earlier RD-03 handoff timing; it preserves the native-format evidence gate in full.
