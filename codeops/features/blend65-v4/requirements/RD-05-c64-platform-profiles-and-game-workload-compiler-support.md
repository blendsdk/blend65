# RD-05: C64 Platform Profiles and Game-Workload Compiler Support

> **Document**: RD-05-c64-platform-profiles-and-game-workload-compiler-support.md
> **Status**: Draft
> **Created**: 2026-09-10
> **Project**: Blend65 v4
...
The following boundary is normative for RD-05:

| Area | Blend65 compiler/platform owns | Application source owns |
|---|---|---|
| Game loop | Profile frame facts, timing/resource reports, interrupts, and local timing contracts | Simulation cadence, update/render order, frame policy, and overrun behavior |
| Sprites | Typed VIC-II operations, placed image pointers, interrupt correctness, and atomic publication primitives | Logical sprites, pools, culling, sorting, drop policy, and the multiplexer algorithm |
| Scrolling | Fine-scroll/base operations, placement and visibility proof, and correct/optimized lowering | Map representation, edge generation, movement policy, and renderer structure |
| Buffering | Ordinary fixed storage, placement, visibility, and atomic-publication operations/proof | Which state is doubled, when it is prepared, and its lifecycle |
| Audio | Direct SID operations and exact hash-bound player/import adapters | Tick call sites, cue policy, and arbitration unless the selected player contract defines it |
...
> AR-020 through AR-024, AR-026 through AR-030, AR-033, AR-035, AR-037, and AR-038.
...
- [ ] **R5.9 — Complete the selected-profile memory map.** Reserve `$0000/$0001`, page one, active
  ...
  objects, and every selected hardware operation, adapter, or user object. Reject overlap,
  invisibility, an invalid zero-page pair, or an unprovable stack peak before assembly. SFA
...
- [ ] **R5.23 — Compute complete frame budgets.** For each representative game-workload slice, begin
  with model-specific CPU opportunities, subtract the union of actual badline and sprite-DMA bus denial,
...
#### Input and direct VIC-II operations — complexity L
...
- [ ] **R5.25 — Implement keyboard and combined-input scanning.** Provide named-key tests and a
  ...
  restoration, ghosting behavior, and joystick coexistence policy. Debounce and key-repeat policy
  remain ordinary application source; typed publication/critical-section operations expose their
  storage and cycle costs when needed. `RESTORE` remains an NMI source. (AR-003, AR-013, AR-038)
- [ ] **R5.26 — Implement all eight hardware sprites.** Provide typed operations for 9-bit X,
  ...
  operations lower directly. The platform library defines no logical-sprite object, lifecycle,
  dispatch loop, or copied-image policy. (AR-003, AR-007, AR-013, AR-038)
- [ ] **R5.27 — Preserve collision-latch semantics.** Sprite/sprite and sprite/background collision
  ...
  Higher-level collision remains ordinary application source. (AR-002, AR-013, AR-038)
...
#### User-authored sprite, scrolling, and buffer workloads — complexity XL

- [ ] **R5.29 — Compile user-authored fixed-capacity sprite multiplexers.** Ordinary Blend65 source
  can declare its logical sprite pool, culling and sorting policy, deterministic tie order,
  capacity, overflow/drop result, inactive schedule, and publication point. The platform library
  supplies only the necessary typed VIC-II, raster, interrupt, and atomic-publication operations;
  it supplies no logical-sprite type, pool, sorter, scheduler, or multiplexer engine. (AR-002,
  AR-012, AR-013, AR-038)
- [ ] **R5.30 — Preserve user-authored sprite policy.** Under `optimization: none`, culling, sorting,
  accepted/drop order, and scheduling follow the source exactly. RD-08 may specialize a proved
  bounded implementation or substitute an equivalent credible algorithm only when the behavior
  contract permits it and complete costs improve. The compiler never invents game policy merely
  because source resembles a multiplexer. (AR-023, AR-033, AR-038)
- [ ] **R5.31 — Qualify multiplexing against the frozen expert workload.** Reproduce the Q-P13
  ...
  raster lines. Q-P13 remains a source fixture and evidence oracle; it is not a public multiplexer
  module or prescribed game architecture. (AR-008, AR-013, AR-024, AR-038)
- [ ] **R5.32 — Compile user-authored fine/coarse scrolling.** Typed platform operations expose fine
  VIC fields, screen/charset/bitmap bases, Color RAM, and safe publication. Ordinary source owns
  coarse row/column or tile updates, off-screen preparation, object-coordinate adjustment,
  movement policy, and renderer structure. Both directions and character-cell/map-width crossings
  compile without byte-index truncation or an injected scrolling engine. (AR-002, AR-013, AR-038)
- [ ] **R5.33 — Support explicit user-owned double buffering.** Ordinary fixed arrays, placed
  objects, and typed base/pointer/publication operations let source identify what is doubled and
  when inactive state becomes visible. The compiler proves placement, visibility, atomicity, and
  ownership; it introduces no buffer declaration construct, lifecycle manager, implicit copy, or
  renderer. Different evolving buffers are distinct state, while identical static replication
  still needs a necessity/cost proof. (AR-007, AR-013, AR-020, AR-038)
- [ ] **R5.34 — Qualify scrolling and buffering through real slices.** Provide at least one standard
  ...
  its PAL and NTSC profile. The slices are self-contained user programs, not linked support
  modules. (AR-008, AR-024, AR-033, AR-038)
...
- [ ] **R5.37 — Support all four deliberate audio paths.** The platform contract can select music
  ...
  reentrancy, and complete resources. Arbitration or resume policy is adapter-owned only when the
  exact selected player defines it; otherwise application source owns that policy. The compiler
  does not supply a mixer, scheduler, or general audio engine. Missing fields remain unavailable
  rather than guessed. (AR-008, AR-013, AR-024, AR-038)
- [ ] **R5.38 — Keep audio scheduling source-owned.** `audioTick()` performs exactly one update at
  ...
  reports its cost. The compiler does not choose the game's tick placement or cue policy. (AR-002,
  AR-018, AR-038)
...
#### Representative game-state compiler qualification — complexity L

- [ ] **R5.41 — Compile user-authored bounded fixed pools.** Ordinary fixed arrays, structs, active
  counts, bitsets, and free lists are sufficient to express application-selected pool policies.
  Pools larger than 255 use word-capable ordinals automatically. Blend65 supplies no entity type,
  pool module, lifecycle policy, allocator, hidden handle table, or dynamic array. (AR-002, AR-012,
  AR-014, AR-038)
- [ ] **R5.42 — Preserve source-level data layout semantics.** Ordinary AoS, SoA, and hybrid layouts
  ...
  AR-023)
- [ ] **R5.43 — Compile user-authored bounded collision algorithms.** Ordinary source can implement
  fixed-capacity broad-phase lists, grids/buckets, or sorted intervals and narrow-phase point,
  half-open AABB, mask, tile, or other application-selected tests. The application defines result
  order and overflow behavior; the compiler preserves them, selects arithmetic width from proven
  ranges, and never injects a collision system or silently overwrites memory. Direct VIC-II
  collision-latch access remains the distinct hardware operation in R5.27. (AR-002, AR-014,
  AR-038)
- [ ] **R5.44 — Compile user-authored deterministic state dispatch.** Ordinary switch/compare
  chains, tables, and finite typed function values preserve source-declared state structure, update
  order, and the complete call/SFA domain. Blend65 supplies no state-machine type, transition model,
  event dispatcher, or dispatcher runtime. RD-08 may specialize a proven finite target set or
  select an equivalent table only when complete costs improve for the selected goal. (AR-018,
  AR-023, AR-038)
- [ ] **R5.45 — Qualify the 320-slot expert workload.** Run the frozen Q-P16 workload with 320
  ...
  record candidate costs without pretending RD-08 has selected an optimized layout. Q-P16 is a
  self-contained qualification program, not a public pool, collision, or state module. (AR-008,
  AR-023, AR-033, AR-038)
- [ ] **R5.46 — Inject no workload support by recognition alone.** Compiling source that resembles a
  game loop, multiplexer, scroll routine, buffer handoff, fixed pool, collision pass, or state
  machine emits only the program's reachable semantics. Only explicitly invoked hardware
  operations or selected exact adapters may add platform code/data, and their complete costs remain
  visible. A qualification fixture or example cannot become an implicit dependency. (AR-002,
  AR-012, AR-038)
...
- [ ] **R5.48 — Make platform failures actionable.** Diagnostics name the exact profile, source
  ...
  (AR-003, AR-008)
- [ ] **R5.49 — Use independent game-workload behavior oracles.** Derive expected sprites, cells,
...
- [ ] **R5.52 — Close the resident C64 platform boundary honestly.** At closeout, all eight PRG
  identities and every RD-05 platform capability are either directly qualified or absent from the
  public surface. Every claimed game-workload capability is backed by a self-contained source
  fixture, never by a public engine module. RD-06 native assets/composition, RD-07 disk loading,
  RD-08 optional optimization, RD-09 complete tooling, and C64U remain explicitly incomplete. No profile,
...
- [ ] **R5.54 — Provide small modern examples — complexity M.** Add focused, runnable examples for
  ...
  dispatch. Examples are self-contained user-authored programs that show game intent and source
  ownership rather than raw register folklore; they are not installed support modules or prescribed
  architectures. (AR-003, AR-033, AR-038)
...
- Any built-in or separately shipped game engine, framework, or gameplay library, including game
  loops, entity/fixed-pool modules, collision systems, state machines/dispatchers, renderers, scene
  graphs, sprite-multiplexing or scrolling engines, double-buffer managers, audio mixers/schedulers,
  task systems, or callback registries. This exclusion applies even when a facility would be
  opt-in, zero-cost when unused, or implemented as ordinary Blend65 source. Narrow hardware
  operations and exact external adapters remain in scope. (AR-002, AR-012, AR-038)
...
### Hardware/workload ownership and cost envelope — complexity XL

Every selected hardware integration and every representative game workload supplies this minimum
evidence record:

| Field | Required result |
|---|---|
| Intent | The source-visible hardware operation or user-authored workload behavior |
...
runtime registry, game-system interface, public gameplay API, or requirement for one class per row.

### Frame/publication qualification pattern — complexity L

One representative qualification source pattern is explicit and bounded:
...
silently move user work into an IRQ, serialize a shared update by masking interrupts, or ship this
pattern as an engine/module. User programs need not adopt this structure. (AR-038)
...
- Supplies correct user-authored game-workload paths, complete costs, effects, ranges, layout, and
  independent oracles. RD-08 may choose proved equivalent implementations but cannot invent game
  policy or repair missing RD-05 semantics/device ownership. (AR-038)
...
| Platform structure | Public plugins / built-in qualified hardware components | Explicit built-in hardware components | Keeps target contracts testable without a speculative framework. | AR-012 |
| Game-workload boundary | Reusable game systems / compiler plus narrow platform integration | User-authored game algorithms; compiler/platform enable and qualify them | Keeps Blend65 a language/compiler while preserving modern source and expert output. | AR-038 |
...
23. [ ] **AC-23 — User-authored multiplexer oracle:** The self-contained 24-sprite Q-P13 PAL source
    case produces exact accepted sprites/order, schedule tables, channel reuse, selector
    publication, raster events, late/drop outcome, MMIO trace, and complete cost without importing
    a logical-sprite, sorter, scheduler, or multiplexer module. Nine-overlap, page-cross, banking,
    NMI, and expanded-sprite counterexamples reject that specialized contract.
...
25. [ ] **AC-25 — User-authored scrolling:** Self-contained horizontal and vertical standard-text
    programs cross fine/coarse, direction, character-cell, and map-ordinal boundaries with exact
    screen and Color RAM results, no byte-truncated index, no torn edge, and a complete PAL/NTSC
    frame budget. No scrolling or renderer module is imported or injected.
26. [ ] **AC-26 — User-owned double buffering:** Screen-matrix and logical-schedule programs use
    ordinary fixed objects, write only the inactive object, publish once atomically, flip only after
    completion, and never perform a full copy under the name “flip.” Reports count both evolving
    buffers and all alignment/padding; no buffer-manager API or runtime is present.
...
29. [ ] **AC-29 — Audio contract behavior:** Integrated music+SFX tests prove the exact selected
    player's priority, replacement, same-frame request order, selected voice, and resume behavior;
    otherwise those choices remain explicit application source. Concurrent nonreentrant calls and
    incompatible PAL/NTSC or 6581/8580 contracts fail before emission. No compiler mixer or
    scheduler supplies missing policy.
...
31. [ ] **AC-31 — User-authored fixed pools:** A self-contained program's empty, full,
    release/reuse, and overflow cases for a pool larger than 255 retain exact object identity and
    word ordinals without a heap, hidden tables, memory overwrite, or any imported/injected entity
    or pool module.
32. [ ] **AC-32 — User-authored collision/state workload:** The self-contained Q-P16 program
    updates all 40 active entries including slot 270, dispatches five instances of each of eight
    states, emits the exact 20 ordered half-open AABB pairs, and reports overflow at pair 97 in the
    clustered case without publishing a 97th result. Its source owns the pool, collision algorithm,
    state model, ordering, and overflow policy.
33. [ ] **AC-33 — No implicit workload support:** A source program that does not invoke a hardware
    operation or exact adapter gains zero related platform code, data, RAM, ZP, stack, startup work,
    and cycles merely because its shape resembles a game technique. Each explicitly used platform
    integration lists every linked owner separately.
...
39. [ ] **AC-39 — Honest support statement:** Public status names all eight resident PRG profiles,
    exact hardware/platform operations, exact adapters, and separately identified game workloads as
    qualified. It never reports a game engine or gameplay module. Native asset handlers/composition,
    D64/loading, optional optimization, complete tooling, C64U, and hardware-unverified claims remain
...
43. [ ] **AC-43 — Compiler-only product surface:** Static inspection of public modules, generated
