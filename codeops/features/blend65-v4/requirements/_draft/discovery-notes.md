...
| Specification transition | Apply all accepted language changes once to Specification 4.0, including ordinary nested lexical shadowing with stable declaration identity, run the Language Guard, version/qualify one expert skill, freeze both identities, then begin semantic implementation. | Covered by AR-014; no implementation-only override or parallel live specification is allowed. |
...
| Configuration | Nearest manifest wins; duplicate/unknown keys, invalid schema/profile/options, output escape, invalid literal artifact basename, and nested-project ambiguity are diagnosed. Source and asset filenames remain exact host input identities and never pass through the output-basename validator. | AR-022 |
...
| Compile-time evaluation | No runtime/MMIO/host-nondeterministic effects; exact integer math is specified byte-for-byte; fixed `comptime-budget-v1` provides 16,777,216 compilation-wide steps, 16 MiB live logical values, and 512 active calls; exhaustion fails before the exceeding operation and produces no partial result. | AR-019; accounting units and E10269/E10270/E10271 are frozen before Specification 4 activation |
...
| Tool and artifact failure | Missing/wrong ACME or VICE, invalid tools schema, assembler error, timeout, hash mismatch, invalid/aliased/over-limit artifact component, occupied symlink/device/directory, exclusive-create collision, no-space, or packaging failure retains distinct status and cannot publish/run a stale mixed set. Project data never enters ACME source; direct arguments own its outputs. Publication is the no-return point; later run cancellation preserves the committed build. | AR-009/AR-022 and artifact evidence contract |
