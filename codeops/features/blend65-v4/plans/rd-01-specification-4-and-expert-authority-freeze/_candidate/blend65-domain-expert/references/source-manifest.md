...
### BLEND65-SPEC-P3-4bf8a989 — Current reconciled content identity

- **Authority/status**: Current reconciled identity after every accepted product ruling through
...
| Q-C02, Q-C04 | BLEND65-SPEC-P3-4bf8a989 `spec/02-type-system.md`, “Comparison operators” and signed integer ranges; MOS-PGM-1976 §4.2.1 and §§4.1.2–4.1.3 | Blend65 requires mathematical signed order while MOS supplies the exact flag effects. Sign normalization, sign split, or controlled subtraction is valid only when its stated preconditions reproduce that order. A word comparison settles a differing signed high byte before the low byte. |
| Q-C05, Q-C06 | BLEND65-SPEC-P3-4bf8a989 `spec/02-type-system.md`, “Deterministic integer wraparound”; MOS-PGM-1976 §§2.2.1–2.2.3 (`ADC`/`SBC`, multi-precision, carry/overflow) and §§3.0–3.0.2 (`C`, `SEC`, `CLC`) | Multi-byte addition must establish a source-independent initial carry and propagate it; subtraction uses carry as no-borrow and begins with `SEC` for ordinary `a-b`. |
...
| Q-C13 | BLEND65-SPEC-P3-4bf8a989 `spec/04-expressions-operators.md` and `spec/02-type-system.md`, wide-shift rules; MOS-PGM-1976 Chapter 10 §§10.0–10.4 (`LSR`, `ASL`, `ROR`, `ROL`); CC65-2.19 `libsrc/runtime/asr.s` (comparative sign-propagating implementation only) | Signed arithmetic `>>` preserves sign extension for counts at least the width: negative operands produce `-1`, and non-negative operands produce `0`. Unsigned `>>` and every `<<` produce `0` at those counts. The 6502 practice check confirms that signed shifts explicitly propagate sign through carry; cc65's different C rule supplies comparison only and does not decide Blend65 semantics. |
| Q-C14 | BLEND65-SPEC-P3-4bf8a989 `spec/04-expressions-operators.md:70-104` and `spec/02-type-system.md:269-300,441-456,485-489`; MOS-PGM-1976 Chapter 10 shift/rotate effects; `evidence-parity-and-recovery.md#equivalent-work-accounting` | Fold/identity/shift-add/table/helper selection is compiler synthesis. It must preserve width/signedness/wrap semantics and win after complete attributable cost; no comparative compiler is authority for the choice. |
| Q-C15 | BLEND65-SPEC-P3-4bf8a989 `spec/04-expressions-operators.md:106-147` (truncation-toward-zero quotient, signed-remainder identity, and zero boundary); MOS-PGM-1976 Chapter 10 shift/rotate effects; `evidence-parity-and-recovery.md#transformation-proof` | A signed negative odd dividend distinguishes truncation-toward-zero division from arithmetic shift and dividend-signed remainder from an unsigned mask. `-3 / 2 == -1` and `-5 % 2 == -1`; a shift/mask replacement requires a proof or sign-aware correction. |
| Q-C16 | BLEND65-SPEC-P3-4bf8a989 `spec/02-type-system.md:181-185,228-237` (comparison produces Boolean); MOS-PGM-1976 §4.2.1 and §§4.1.2–4.1.3; `evidence-parity-and-recovery.md#equivalent-work-accounting` | Direct branching and delayed materialization are project/compiler synthesis. The escaping Boolean remains materialized exactly where demanded by the use graph. |
...
| Q-C21 (**project-policy oracle**) | `evidence-parity-and-recovery.md#transformation-proof`; BLEND65-SPEC-P3-4bf8a989 and MOS-PGM-1976 are permitted raw semantic/hardware evidence for the isolated case | Changed assembly requires an independent expected-behavior oracle and a separate intended shape/cost expectation; comparing two implementations alone cannot prove both correct. |
...
SRC-009/SC-005 and SC-131–SC-155 are bound to `BLEND65-SPEC-P3-4bf8a989`. Earlier comprehensive
