...
### BLEND65-SPEC-4-5c6bac04 — Current frozen Specification 4 identity

- **Authority/status**: sole current Blend65 language and C64-profile authority for expert baseline
...
- **Scope**: syntax, types, evaluation, effects, modules, storage, intrinsics, diagnostics, and the
  nine active C64 profiles. Future-target comparisons are non-normative pressure only.
- **Dependent sections**: every current specification reference in this candidate.
- **Known issues**: none in the frozen authority. Compiler code and tests remain audit subjects.
- **Verification**: reproduce the digest and exact 45-path inventory before qualification.

### Historical Specification 3 identities

The remaining `BLEND65-SPEC-P3-*` entries preserve review provenance only. They are invalid as
current semantic authority and must not be cited as the active baseline.

### BLEND65-SPEC-P3-4bf8a989 — Former Specification 3 reconciled identity

- **Authority/status**: Historical Specification 3 identity after every accepted product ruling through
  AR-P43 and every consistency repair through SC-155. It includes the final
  ...
  queries, representable fixed-object domain, updated diagnostic inventory, corrected Atari 7800
  timing summary, enumerated-conversion axiom, count-free future-register link, and narrow F001
  evidence boundary. The final exception scan additionally corrects aggregate-return debt,
  warning scopes, per-activation SFA costs, address-workaround bounds, migration disclosure, and
  C64 unchecked indexing. Earlier runs remain historical supporting evidence only. These edits
  followed the first complete 107/107 candidate run, so that transcript cannot qualify the changed
  cases by itself. The final identity requires deterministic gates, a complete blind coverage
  sample, focused correction evidence for affected and dependency-traced cases, and independent
  grading with zero unresolved material defect. Any resulting specification edit invalidates this
  identity and requires a replacement digest plus review and reruns for its traced blast radius.
- **Version**: exact GNU SHA-256 record digest
  `4bf8a98934df6282febe013c31a16d89dd73f8e8bbbce9e87b6678e1b75013fe`.
...
- **Verification**: digest and 50-path equality are mechanically reproducible. SC-148..SC-155 are
  content-complete; changed-surface review remains required. Definitive evaluation and grading of
  this exact identity are pending; no result from an earlier identity is relabelled as current
  evidence.

### BLEND65-SPEC-P3-8983563e — Invalidated pre-provenance-review candidate

- **Authority/status**: Invalidated historical identity. It contained the accepted aggregate-return
  direction, but FUT-010's source metadata and navigation summary still named only struct returns
  and F011. Independent changed-surface review found that the fixed-array E10120/F014 provenance was
  missing.
- **Version**: exact GNU SHA-256 record digest
  `8983563e8658aeffdac1755fc63b5b45752e8673e8bf390e86949439c52075ab`.
- **Digest algorithm**: the exact 50-path GNU SHA-256 record algorithm defined by the current
  identity above.
- **Known issue and impact**: no evaluator attempt for this identity qualifies the replacement.
  FUT-010 now names aggregate returns and depends on both F011 and F014 under
  `BLEND65-SPEC-P3-4bf8a989`.

### BLEND65-SPEC-P3-6f5a1734 — Invalidated pre-final-HLE-scan candidate

- **Authority/status**: Invalidated historical identity. Its complete isolated 107-case run and
  correction grades reached 107/107, but the mandatory final exception scan then found missing
  HLE-002/HLE-005 discrimination, stale range and per-function wording, false universal address
  workarounds, absent migration/C64 disclosures, and aggregate-return debt misclassified as a
  settled language restriction.
- **Version**: exact GNU SHA-256 record digest
  `6f5a17341fa2b67f9783a875bf791d8cf3ad2bbe838a929981023ffa6e503c81`.
- **Digest algorithm**: the exact 50-path GNU SHA-256 record algorithm defined by the current
  identity above.
- **Known issue and impact**: no output from this identity qualifies the replacement. SC-152..
  SC-155 and strengthened Q-L20/Q-L23/Q-A08 require current focused evidence under
  `BLEND65-SPEC-P3-4bf8a989`; unrelated results remain historical coverage evidence only.

### BLEND65-SPEC-P3-400777d0 — Invalidated partial AR-P42 candidate

- **Authority/status**: Invalidated historical identity. It corrected the first AR-P42 future
  summary in Chapter 00, but independent review found that the feature index still duplicated the
  `FUT-019` endpoint and resolved subset. The candidate crosswalk repeated that contradiction.
- **Version**: exact GNU SHA-256 record digest
  `400777d0a61b6612ef28e0f569ba5e1ad050a30fb19cd18d96a9e850334c0125`.
- **Digest algorithm**: the exact 50-path GNU SHA-256 record algorithm defined by the current
  identity above.
- **Known issue and impact**: SC-150 was incomplete. No evaluator or review result for this
  identity qualifies the replacement; `BLEND65-SPEC-P3-6f5a1734` was the first count-free candidate
  and is now separately invalidated above.
...
- **Authority/status**: Invalidated candidate identity retained as historical evaluation evidence.
  ...
  replacement is `BLEND65-SPEC-P3-4bf8a989`. Chapters 00–15 still govern language semantics;
...
### BLEND65-PROJECT-POLICY-P3-28627e0c — Durable compiler and workflow directives
...
- **Version**: repository `AGENTS.md` SHA-256
  `28627e0ca180846727c3425fae6955dcb48fd1eb36da14eaebaa446508b33d7a`.
- **Location**: repository `AGENTS.md`, headings `PRIME DIRECTIVE — expert assembly game developer`,
  ...
  Architecture bullet `Aggregate-return direction` and the `Project-specific` bullets
  `Skill/implementation independence` and `C64 verification authority`.
- **Scope**: modern-language input, expert-assembly output, measured parity debt and authorized
  ...
  caller-owned destination passing as the aggregate-return redesign direction, commit/push policy,
  and no hidden runtime or support-framework authority.
...
- **Authority/status**: Commodore manufacturer manual and recovered DOS 2.6 source, independently
  cross-checked against VICE 3.10 host-image mapping.
- **Edition/version**: _VIC-1541 Floppy Drive User's Manual_ SHA-256
  `b98f28916579d63c5f18ded32c04ed07d3e87c9fb3155abc988c2fdd41b3fd97`;
  `mist64/cbmsrc` commit `01bd60f162ef92212ef0cb67546ae8f42be34168` with `romtblsf.src`
  `f0938ecdee9d3d8dd39443a2d106f6714ebbc3b3fcec643d13c62ab491448b1c`, `tst2.src`
  `9e18edc1f3ccb3b8c383b3d4f47f780c716c9bf9daefe26f953ac736332711f4`, and `tst4.src`
  `e6ca75b5f84317269585945ff6c1d0f4c66353d15f742779436000efb6952a46`; VICE tag
  `3.10.0`, commit `4d283a2e7dd59b7e378524878e81ecc7826b700c`, with `diskimage.h`
  `f611ef0b3059edd57859d0ef39231ee0421825a470b4cc32df447def92c64588`, `diskimage.c`
  `ddc53b5ebc6bf181dda9f1088b26a760aca4df9f5cba138a9cfa730cea42e251`, and
  `fsimage-check.c` `ffdcac825ed79140885fd8e1d872d762fa65f4ffdcd1cd92711f0108164153c3`.
- **Location**:
...
- **Known limit**: geometry and interpretation only; the packager owns measured allocation and
  interleave policy.

### CBM-C64-KERNAL-LOAD-03 — KERNAL 901227-03 loader ABI

- **Authority/status**: Commodore programmer reference, cross-checked against the reconstructed
  revision-03 KERNAL source.
- **Edition/version**: _Commodore 64 Programmer's Reference Guide_ Chapter 5 scan SHA-256
  `58262099fc7ef9e95e0b6d2cd9f50de12ed48f0a30625456269bcda7f8a6436a`;
  `mist64/cbmsrc` commit `01bd60f162ef92212ef0cb67546ae8f42be34168`, README
  `db3e44cdcd3a9e99f53e5e352128c5ea67dabccc8732b99288e3bc7b5c677d6c`, `init`
  `a19581404b6a7bddaac322c06f3bc5c7f5b442b81175c50acdfd6c48a0dcd180`, `load`
  `c4ee7c1abb358a6caa4e91f30937e7d71b099f621a8f7e8388cd4e10c37ad6c7`, `vectors`
  `7e5a1954d592b4ff48ace641f9432795a6a9a37c1f3b97d4b52333f5324ef714`, `declare`
  `d7ea21632b3a4e2d2282bf12a284325267a3eca39f6d24f2759e78a14b182fe2`, and `errorhandler`
  `5282c6bffaa68db16105858ca8437c969294a1271e8255eeed5a5f14d63a0db1`.
- **Location**:
...
- **Precision**: manual Appendix B entries B-15, B-28, and B-30; reconstructed `SETNAM`, `SETLFS`,
  `LOADSP/NLOAD`, `LD25`–`LD90`, `LD180`–`LD190`, jump table, and workspace/error paths.
...
- **Precision**: Codebase64 “Koala Painter 2” address table; Retropixels `cli/README.md` output
  table. The Codebase64 raw record has SHA-256
  `e544700ddff4288239260d14a5c5be3ee5fa956e0132cbdf345ff02f0b1077e0`; the Retropixels README
  has SHA-256 `ef1cd543b019f972286bcc6b872a7038d68ec92427aaaf62e87eb52fdd605945`.
...
- **Local extraction**: exact 10,003-byte length and order: two-byte little-endian `$6000`, 8,000
  bitmap bytes, 1,000 screen bytes, 1,000 Color RAM bytes, and one background byte. Blend65 project
  policy preserves every input byte; only the low nibble of Color RAM and background bytes has
  VIC-II color meaning. The sources do not establish an unused-high-nibble rule.
...
|---|---|---|
| Q-L08 (**project-policy oracle**) | `sfa-and-abi.md#hardware-stack-duties`; selected platform profile raw stack capacity and reserve; CPU/platform interrupt-entry sources when applicable | Measure every feasible path without reserve adjustment, then decide `measuredPeak <= rawCapacity - reserve`. Reserve reduces available capacity only; it is never subtracted from, added to, or counted inside the measured peak. Report all three values and diagnose E10238 when the inequality fails. |
| Q-L20 (**project-policy/reconciled-spec oracle**) | `BLEND65-SPEC-4-5c6bac04`: Chapters 02, 04, 14, and 15; F017; C64 appendix; HLE-002 and optional-safety sections | Constant evaluation keeps full precision while runtime operations use semantic operand width. Constant zero is E10160. Default runtime zero terminates with an unspecified valid-width result and declared effects only; it is neither a specified fallback nor optimizer undefined and adds no check/runtime. Default-off `--division-zero-check` evaluates once and fails before division; C64 uses only the source-labelled `SEI` plus self-loop terminal block, with all ROM/SFA/register/success-path/layout/timing costs reported. |
| Q-L23 (**project-policy/reconciled-spec oracle**) | `BLEND65-SPEC-4-5c6bac04`: Chapters 03, 08, 10, and 14; F003/F014/F019; HLE-005 | Module initializers execute once in dependency/effect order with a stable fully-qualified-name tie break. Omitted mutable initialization emits no clear and preserves valid-width stored bits without optimizer UB. W10190 applies only to function-local maybe-read-before-first-assignment; W10141 independently applies to every nonzero uninitialized mutable array, including module arrays, so both may apply to one function-local array. |
| Q-A08 (**project-policy/reconciled-spec oracle**) | `BLEND65-SPEC-4-5c6bac04`: Chapters 06–08 and 11; F011/F014; `evidence-parity-and-recovery.md#expressiveness-is-outside-the-ratio` | Fixed struct and array returns are current language behavior. Lowering uses a caller-owned hidden destination closed through SFA, with direct construction/copy elision under proof, no heap/runtime, and complete alias/lifetime/nested-call/IRQ/effect/cost closure. An implementation that still emits E10093, E10119, or E10120 for the supported forms has an expressiveness defect; it does not redefine the specification. |
| Q-L19 (**project-policy/reconciled-spec oracle**) | `BLEND65-SPEC-4-5c6bac04`: Chapters 02, 04–08, 11, 12, and 14; F008/F011/F014/F016/F018/F020; accepted AR-P35–AR-P41 | Stored arrays are fixed contiguous `T[N]`. `T[]` is only initializer extent inference or a whole-fixed-array any-size parameter; the latter carries a two-byte address plus a two-byte element count, while exact parameters carry only the address. `length()`, `sizeof()`, and `offsetof()` have stable semantic `word` types; fixed-array length and all valid size/offset queries fold, while any-size length reads the carried count. Extents and complete fixed array/struct byte sizes fit `0..65535`; E10264/E10265 reject larger forms, and E10266 rejects `sizeof(T[])`. Every unbarriered integer-producing operator inside `[]` uses the 16-bit-capable ordinal context, while explicit or earlier 8-bit barriers retain wrap. Proof may select byte-only work without changing source meaning. E10262 rejects only a proved finite-looking loop whose declared fixed-width counter cannot reach its invariant bound. No dynamic array, slice, span, view, helper, heap, or default runtime is introduced. |
| Q-L26 | Reconciled project contract; HVSC-SID-FORMAT-20260906 header fields; SPRITEPAD-380/C64LIB-RBT-79D5C0E/OPENSPRITE-F13B115 bounded SPD evidence; CHARPAD-388/C64LIB-CTM9-79D5C0E bounded CTM evidence; KOALA-NATIVE-003 classic layout evidence | Each handler has a literal key, exact accepted identity, selector types, malformed-input boundary, emitted representation, and placement/cost contract. Compile-time import and SFA/runtime-import code may be zero, but emitted bytes and final RAM/ROM/banked residency are separate; without a final map/loader plan residency and replication remain `Unknown`. CharPad `tile_width`, `tile_height`, and `tile_mode` remain available outside tile mode with exact values `1`, `1`, and `false`; only selectors explicitly declared tile-mode-only are absent. The SID handler accepts only the specified self-contained PSID v1–v4 subset, resolves the load/init fields exactly, rejects environment-dependent variants, validates exact selected video/model/topology compatibility with E10261, and places bytes at their effective load address without a runtime copy. Unknown metadata remains embed-only unless an exact player contract closes it. Producer-schema/fixture prerequisites remain fail-closed and cannot be replaced by comparative parser agreement. Koala preserves all 10,003 bytes; only low nibbles of its Color RAM/background bytes carry VIC-II color meaning. |
| Q-L27 | Reconciled project contract after AR-P14/AR-P16; SPRITEPAD-380 release record; C64LIB-RBT-79D5C0E and OPENSPRITE-F13B115 bounded parser evidence | The product contract pins 3.80/SPD v5, word counts, native 64-byte records, per-record attributes, complete optional components, and no hidden copies. Asset handlers add no SFA/runtime-import code, but final residency remains `Unknown` without a map. Fit is proved from actual bank-relative intervals: contiguous records occupy `[o, o + 64 × S)` inside `[0, $4000)`, with 64-byte-aligned `o`, exact live-object intervals, per-object constraints, and no unproved overlap. Final offsets already include padding. `256 × 64 = 16,384`, so 256 records consume the complete bank and cannot coexist with its active screen matrix; more than 256 necessarily exceeds it. Parser closure requires the `SPD380-MIN-V5`, `SPD380-ALL-V5`, and `SPD380-WIDE-V5` roles; an `SPD380-OPTIONALS-V5-*` present/absent matrix; `SPD-V4`; `SPD-V6`; and registered malformed-boundary fixtures. IDs label evidence but do not replace producer provenance, hashes, expected outputs, and exact boundary/EOF proof. The public producer record proves only application identity; comparative parsers omit or question tails, so complete tile/animation/expansion/overlay parsing remains unqualified until producer schema and fixture evidence settle every byte. |
...
| Q-L29 | Reconciled Specification 4; CBM-C64-PRG-1982 printed pages 308/311; CBM-C64-KERNAL-03 `irqfile::PULS/PULS1` and `editor.2::KPREND`; MOS-PGM-1976 Chapters 3/9; ZAKS-6502-1980 Chapter 3 p.3-13 | The 901227-03 KERNAL path saves A/X/Y but does not clear D before CINV and exposes its restore-only sub-tail at `$EA81`; its route report includes 16 existing-ROM PULS-to-CINV bytes and the applicable 6-byte existing-ROM tail, both as zero output bytes. Raw hardware entry also leaves NMOS D unchanged. Blend65 therefore establishes binary body entry while preserving interrupted/chained status, and `RTI` restores the interrupted status including D. One callback-only source handler uses sink-selected variants, and only reachable variants are emitted. Default chaining requires a reported two-byte saved-CINV link: `$xxFE` is valid and `$xxFF` is relocated or rejected. Specification 4 exposes no raw decimal-mode intrinsic. Exclusive takeover requires ownership of every enabled source; raw installation requires a profile-proven active/writable vector. Visible raw-entry installation at CINV is a compile-time ABI error, not a low-level escape. |
...
| Q-L31 | Reconciled project contract after AR-P32/AR-P38; `spec/05-statements-control-flow.md` and `spec/evaluations/F008-for-loop.md`; MOS-PGM-1976 Chapter 7 §§7.4/7.6 and Chapter 4 | One familiar three-clause loop uses ordinary evaluation, scope, mutation, exits, and fixed-width wrap. Generic CFG lowering is always correct. A proved canonical word induction may use byte `INX`/`DEX` wrap machinery when the semantic terminal state is unobservable. E10262 rejects the canonical finite-looking byte `i < 256` form because its counter repeats before the invariant condition can become false; intentional modular loops remain legal, and former range words are ordinary identifiers. |
...
| Q-L33 | Reconciled project contract after AR-P34; HVSC-SID-FORMAT-20260906 flag definitions; Chapters 13–15, F015, and the C64 appendix | Every active C64 profile selects an exact `video_standard` and one `$D400` SID endpoint/model. PSID v2NG–v4 clock and model bits retain all four meanings. A known mismatch is E10261 with no automatic retiming, retuning, or model conversion. Unknown is not Both: embedding remains legal, but callable audio requires an exact contract that closes unknown fields without contradicting specific metadata. C64 Ultimate remains future-target pressure, not an active profile. |
...
| Q-C02, Q-C04 | BLEND65-SPEC-4-5c6bac04 `spec/02-type-system.md`, “Comparison operators” and signed integer ranges; MOS-PGM-1976 §4.2.1 and §§4.1.2–4.1.3 | Blend65 requires mathematical signed order while MOS supplies the exact flag effects. Sign normalization, sign split, or controlled subtraction is valid only when its stated preconditions reproduce that order. A word comparison settles a differing signed high byte before the low byte. |
| Q-C05, Q-C06 | BLEND65-SPEC-4-5c6bac04 `spec/02-type-system.md`, “Deterministic integer wraparound”; MOS-PGM-1976 §§2.2.1–2.2.3 (`ADC`/`SBC`, multi-precision, carry/overflow) and §§3.0–3.0.2 (`C`, `SEC`, `CLC`) | Multi-byte addition must establish a source-independent initial carry and propagate it; subtraction uses carry as no-borrow and begins with `SEC` for ordinary `a-b`. |
...
| Q-C13 | BLEND65-SPEC-4-5c6bac04 `spec/04-expressions-operators.md` and `spec/02-type-system.md`, wide-shift rules; MOS-PGM-1976 Chapter 10 §§10.0–10.4 (`LSR`, `ASL`, `ROR`, `ROL`); CC65-2.19 `libsrc/runtime/asr.s` (comparative sign-propagating implementation only) | Signed arithmetic `>>` preserves sign extension for counts at least the width: negative operands produce `-1`, and non-negative operands produce `0`. Unsigned `>>` and every `<<` produce `0` at those counts. The 6502 practice check confirms that signed shifts explicitly propagate sign through carry; cc65's different C rule supplies comparison only and does not decide Blend65 semantics. |
| Q-C14 | BLEND65-SPEC-4-5c6bac04 `spec/04-expressions-operators.md:70-104` and `spec/02-type-system.md:269-300,441-456,485-489`; MOS-PGM-1976 Chapter 10 shift/rotate effects; `evidence-parity-and-recovery.md#equivalent-work-accounting` | Fold/identity/shift-add/table/helper selection is compiler synthesis. It must preserve width/signedness/wrap semantics and win after complete attributable cost; no comparative compiler is authority for the choice. |
| Q-C15 | BLEND65-SPEC-4-5c6bac04 `spec/04-expressions-operators.md:106-147` (truncation-toward-zero quotient, signed-remainder identity, and zero boundary); MOS-PGM-1976 Chapter 10 shift/rotate effects; `evidence-parity-and-recovery.md#transformation-proof` | A signed negative odd dividend distinguishes truncation-toward-zero division from arithmetic shift and dividend-signed remainder from an unsigned mask. `-3 / 2 == -1` and `-5 % 2 == -1`; a shift/mask replacement requires a proof or sign-aware correction. |
| Q-C16 | BLEND65-SPEC-4-5c6bac04 `spec/02-type-system.md:181-185,228-237` (comparison produces Boolean); MOS-PGM-1976 §4.2.1 and §§4.1.2–4.1.3; `evidence-parity-and-recovery.md#equivalent-work-accounting` | Direct branching and delayed materialization are project/compiler synthesis. The escaping Boolean remains materialized exactly where demanded by the use graph. |
...
| Q-C19 | Current reconciled specification after AR-P32/AR-P38, `spec/05-statements-control-flow.md` and `spec/evaluations/F008-for-loop.md`, three-clause/fixed-width/canonical-induction rules; MOS-PGM-1976 Chapter 7 §§7.4/7.6 (`INX`/`DEX`) and Chapter 4 branch rules | `for (let i: word = 0; i < 256; i += 1)` executes 256 iterations under ordinary expression semantics. A proof may lower it to one 8-bit induction register and a wrap exit when the word terminal state is unobservable. E10262 rejects the canonical finite-looking byte-typed form because its counter cannot reach the invariant bound; intentional modular byte loops remain legal. |
...
| Q-C21 (**project-policy oracle**) | `evidence-parity-and-recovery.md#transformation-proof`; BLEND65-SPEC-4-5c6bac04 and MOS-PGM-1976 are permitted raw semantic/hardware evidence for the isolated case | Changed assembly requires an independent expected-behavior oracle and a separate intended shape/cost expectation; comparing two implementations alone cannot prove both correct. |
...
| Q-P15 | INTEGRATOR-LEVY lines 124–136; INTEGRATOR-DIFRAIA-2012 lines 279–313; CBM-C64-PRG-1982 graphics/memory facts; `c64-game-engineering.md#machine-bound-workload-completion-gate` | Testimony supports elements/panels, masks, attributes, priority, reuse, and memory/speed tradeoffs; reconstruction supports concrete editor stages but retains explicit unknowns. User-authored compile-time Blend65 owns scene composition, masks, attributes, priority, conflict detection, and representation construction. The compiler/toolchain owns only asset ingestion, imported-format validation, typing, placement, packaging, correct lowering, and proved optimization; rendering and scene policy also remain user-authored. Resource accounting separates visible payload from reserved address space: two 1,024-byte screens plus one 2,048-byte charset plus 1,024-address Color RAM reserve 5,120 bytes, including 72 non-visible screen-tail/Color-RAM addresses. User-authored renderer code bytes are a separate term. Every introduced IRQ route individually reports vector/link storage, handler/wrapper bytes, entry/acknowledgement/restoration/exit and full-path cycles, IRQ-specific stack, ownership, visibility, and nesting; missing figures remain individually `Unknown`. |
| Q-P16 | HESSIAN-1.2 `actor.s::UpdateActors` and target/collision-list paths; C64-GAMEFRAME-C634F6F `actor.s::UpdateActors` and collision bounds/search paths; MOS-PGM-1976 Chapter 6 and Appendices B–C; `c64-game-engineering.md#machine-bound-workload-completion-gate` | SoA/AoS, pools, collision phases, and dispatch are selected from the fixed workload and SFA/IRQ consequences. No source establishes one universally best layout. Even a CPU-heavy comparison states exact machine/CPU/video/standard/banking/visibility, either no interrupts or every route's entry/source/ownership/cost, and loader bytes, load time, initialization-loading work, visibility and interference; unavailable terms are `Unknown`, never omitted. |
...
| Q-A17 (**project-policy oracle**) | Frozen five-status/target-boundary policy; future-target constraint sources are permitted only when the target is named | Return exactly one capability status for the inspected boundary. With no supplied or inspected target artifact, declaration, stub, delegation, implementation, or observation, the status is `Unknown`; constraint knowledge alone is not a scaffold. |
...
| SRC-010 | Chapter 05 and F008 first disagreed over the range-loop contract; later review found the whole range-only design unfamiliar and unnecessarily specialized. | SC-006 records the superseded range reconciliation. SC-131 closes AR-P32: one three-clause loop uses ordinary evaluation, scope, mutation, effects, exits, and fixed-width wrap; correct generic CFG lowering is always available, while proven canonical induction recovers expert patterns without a runtime or second syntax. SC-136 and AR-P38 supersede AR-P32's original byte-loop outcome: E10262 rejects a proved canonical finite-looking loop whose fixed-width counter repeats before its invariant condition can become false, while intentional modular loops remain legal. | Q-L31 and Q-C19 are rebound to the final Phase-3 specification identity and AR-P38/SC-136; every pre-AR-P38 result is historical. The later compiler audit must treat every existing range-only parser/AST/analyzer/lowering path as a recovery subject rather than authority. |
...
SRC-009/SC-005 and SC-131–SC-155 are bound to `BLEND65-SPEC-4-5c6bac04`. Earlier comprehensive
results remain historical supporting evidence only. The final exception-scan corrections require
changed-surface review plus the complete definitive suite and independent grading before this
identity may activate. A later source that materially contradicts an active invariant reopens only
the affected authority gate and its dependent knowledge/results; it never permits silent editing
of a frozen oracle.
