...
| ST-08 | Trigger each RD-04-applicable active Chapter-14 diagnostic once, plus legal shadowing and retired E10101 probes. | Every applicable active code has exact public fields/order; legal shadowing and E10101 emit nothing; no implementation-only code appears. Phase 1 proves the registry and its frontend-owned subset; the full case first goes green in Phase 8. | R4.10, AC-07; Spec 4 Ch 14 |
| ST-09 | Run CLI `check` and language-server analysis on equivalent invalid source snapshots, including embedded-source errors and an edited embedded asset path supplied as an editor overlay and as the CLI's on-disk source. | Canonical compiler diagnostic records are byte-for-byte equivalent before CLI text/LSP rendering for the effective snapshot; frontend and LSP import no machine/artifact/emulator code. The CLI uses the public compiler service. | R4.53, AC-39, AR-P7 |
| ST-10 | Build a project with one lexical, semantic, layout or ACME error in separate probes. | Each probe returns failure and publishes no current runnable generation or stale-success pointer; full pipeline-wide case first goes green in Phase 8. | R4.4, AC-03, AC-35 |
...
| ST-20 | Read a nonzero uninitialized local array on one path and assign only a may-alias range on another. In a focused internal semantic-flow case, store a synthetic conditional-effect result, test it later, and join paths with agreeing/disagreeing captured ranges. | W10141 appears at declaration, W10190 at possible read, and no blanket clear or false strong-update credit appears. Only a proved success with the same captured destination range gains initialization across the join. No real transfer is added. | R4.18, R4.38, AC-14; Spec 4 Ch 03, Ch 08, Ch 11 |
...
| ST-26 | Assign overlapping fixed arrays/structs and use the assignment as an expression. | Destination equals the original source value under overlap, result equals the assigned value, and each source effect occurs once; first green in Phase 4. | R4.25, AC-19; Spec 4 Ch 07, Ch 08, Ch 11 |
| ST-27 | Pass the same array to mutable/const exact and `T[]` parameters, forward it, query runtime length and attempt transitive writes. | Exact ABI carries two address bytes; unsized carries address plus word count; const forwarding is zero-cost and writes reject; scalar `const` rejects; first green in Phase 4. | R4.26, AC-20; Spec 4 Ch 08 |
...
| ST-42 | Use variable-address `peek/poke/peekw/pokew`, `lo/hi`, and `sizeof/offsetof/length`. | Addresses/values evaluate once, word bytes are low-first across wrap, and query widths are word. RD-05 owns source-visible selected-profile fact branches across PAL/NTSC variants. | R4.34, AC-28; Spec 4 Ch 04, Ch 12 |
...
| ST-48 | Build one complete-language example using familiar expressions, calls, aggregates and loops while exercising code, globals, user ZP, SFA, constants, asset, helper, placement and branch repair. | ACME bytes reconcile with one coherent PRG and complete deterministic memory/cost/debug/build evidence; no ranges overlap. Generated assembly has stable source-related routine/data labels and meaningful boundary comments; diagnostics name source problems and platform-forced limits without compiler-internal workarounds. | R4.47, R4.55, R4.56, AC-35 |
