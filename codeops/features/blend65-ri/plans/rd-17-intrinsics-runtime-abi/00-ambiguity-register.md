...
| AR-P17 (runtime) | Testing | AR-P4 defers *functional* math verification of the hand-written `__rt_*` routines to RD-12, so Phase 4 would ship division/multiply with syntax-only (ACME) verification — flagged as the plan's top correctness risk during execution | A: ship syntax-checked only per plan / B: add a minimal in-process 6502 interpreter test harness now and run the four routines against arithmetic vectors | **Option B (user-approved 2026-07-02)** — a test-only interpreter in `@blend65/test-harness` executes each routine's assembled bytes against exhaustive/edge vectors (`impl` tier; AC-14's emulator tier remains RD-12's). AR-P4's deferral of *emulator* verification stands; this closes the interim functional gap | ✅ Resolved |
