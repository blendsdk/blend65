...
| ST-4 | `switch (x: byte) { case 300: ... }` (value out of `byte` range / wrong type) | **E10077** | FR-3, AR-4 |
...
| ST-11 | two `default:` clauses in one switch | **E10076** *(if the AST retains both — else recorded parser-owned per 03-01 §2.4)* | FR-5, AR-7 |
...
| ST-18 | the repointed `unsupportedFixture` (a genuinely-unsupported node) | exactly one `E90001` ICE, never throws (RD-06 ST-L5 updated) | 03-02 §4 |
