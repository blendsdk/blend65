...
>
> **Last Updated**: 2026-07-12 (**RD-18 Slice 7b ✅ COMPLETE — exec_plan 58/58; SLICE 7 CLOSED,
> RD-18 acceptance item 6 TICKED.** The pointer surface ships end-to-end: by-ref struct/array
> params (FN-3 — caller stores the address into the callee's 2-byte frame home; SFA chain-max
> colors per-param `__zp_ptr_<Module_fn>_<param>` pairs over the dormant pool, sequential callees
> sharing addresses; one-time prologue frame→pair copies; dead/pass-through params skip both),
> const params CP-1..5 (E10122/E10123 via the one root-symbol predicate; scalar const params
> legal read-only), unsized array params (`ArrayType.size: number|null`, `T[N]→T[]` binding,
> both index widths, `length()`→E10080) + the AR-15 narrowed element-list size inference
> (E10126 owns fill/no-initializer), tier-2 >256-byte arrays via `(zp),Y` (E10118 emittable;
> W10112/W10142/W10143 minted; W10143 keyed to `targetProfile.maxRam`), runtime pointer
> formation through the conditional `__zp_ptr_scratch` (word-domain scaling; every word
> intermediate homed by the fused-store discipline), the IL `addr` operand (store source + ALU
> right operand per runtime AR-16 — `#<sym`/`#>sym` byte selects), the translate `(zp),Y`
> framings + regY mirror + INY-straddle guards, and both 7a E90001 rejections retired
> loud-never-silent (four pins rewritten by the retired-row protocol). **3-part bar GREEN on
> real VICE 3.10 FIRST RUN**: two-file `examples/slice7b/` → `$C000..$C006 = 00 2A 0F 1D 11 0B
> 16` incl. the runtime-word-index-260 formation proof; 212-line golden minted post-VICE; nine
> prior goldens byte-exact NO re-mint. Notable exec finds: ACME sizes symbols by equate digit
> count — pair equates serialize 2-digit via the additive `SymbolDefinition.zeroPage` flag;
> AR-16 pinned at runtime (formation vs fused-store conflict → addr as ALU right operand).
> Ledger R57/R70/R101/R104 closed. Full verify green ×6 phase boundaries; `spec/` clean.
> **Next: Slice 8 (hardware & advanced) needs `make_plan`; RD-13/RD-14 queued.**)
> Prior: 2026-07-12 (**RD-18 Slice 7b 🔬 Plan Preflighted** — preflight iteration 1:
> 0 critical / 7 major / 5 minor / 3 observations — **ALL 15 resolved per recommendation AND
> APPLIED to the plan docs same-day** (✅ PASSED; report `plans/rd-18-slice-7b-pointers/
> 00-preflight-report.md`; the full 🟠 batch challenged by ONE independent challenger:
> 4 converged, 1 diverged-and-reconciled, 2 challenger-originated). Application added register
> row **AR-15** (element-list-only unsized inference; E10126 owns both non-inferable forms),
> renumbered ST-24b→ST-40 (Phase-4 suite, filling the ST-40..44 gap with the preflight rows),
> and grew the plan 57→**58 tasks** (new 2.2.7 inference + 2.2.8 retired-row protocol). 🟠 PF-001 the fixture's tier-2
> observables were const-indexed — `lowerPlace` folds literals to absolute (`lower.ts:1407-1409`),
> so the (zp),Y formation path never ran on VICE; observable 4 rerouted through a runtime word
> index + a golden formation landmark. 🟠 PF-002 `const TABLE: byte[] = […]` does NOT compile —
> AR-5's "7a inference" was a phantom (unsized → size 0 → E10152); inference is half-shipped
> (`typeArrayLit`/`inferUnsizedArray` dead by check-order) and spec-✅ in Ch 02:74 + Ch 08:191 —
> user chose the NARROWED in-slice fix (element-list literals only; AR-5 amended). 🟠 PF-003 a
> word value at pair offset 255 wraps Y mid-value (INY $FF→$00) — fast-path predicate becomes
> offset+size−1 ≤ 255 (word-at-255 rides formation); ST-47's invariant reworded to "Y never wraps
> within one value access". 🟠 PF-004 Phase 2's green gate was unachievable — FOUR 7a pins
> (`aggregate-typing.spec.test.ts:86,:193`; `slice7-negatives.spec.test.ts:117,~:170`) break when
> the E90001s retire, and ST-24b (a Phase-4 lowering ICE) sat in the Phase-2 suite; the
> retired-row protocol now applies at Phase 2 and ST-24b moves to the Phase-4 band. 🟠 PF-005
> ST-47 contradicted 03-04 §6 — indexed compound-assign through a pair stays the loud 7a-class
> ICE. 🟠 PF-006 (challenger-found) scalar compound `e.hp += 1` through a pair would RMW the
> POINTER bytes (the 7a compound branch rewrites via `place.symbol`, `lower.ts:1319-1327`) —
> specified as load_indirect→ALU→store_indirect at the const offset + ST row. 🟠 PF-007
> (challenger-found) byte-index scaling is mod-256 — a `word[]` param aliases element 0 at
> in-bounds index 128; the byte fast path is now elemSize==1-only, multi-byte routes
> zext→formation + ST row. Minors: `finalizeSymbol` SKIPS params (`annotation-resolution.ts:
> 149-150`) and `lengthOf` needs a param arm (03-02 claimed both pre-existing); the §5 word
> add+store works ONLY fused (single-use dest + adjacent store, else ICE `translate.ts:578`) —
> invariant documented; ZERO existing Y-touching emitters exist (the 03-05 audit premise was
> vacuous — rescoped to the new 7b sequences); W10143 pinned to `targetProfile.maxRam` (26623 vs
> the 51199 raw span — 2× apart), skipped when the profile is absent; word-domain index scaling
> for word-element tier-2 arrays specified + ST row. Observations: five citation drifts (incl.
> Ch 07 §5.5-not-§5.6; zpSlot carries no offset → symbolRef Absolute+offset is the +1 mechanism);
> the ST-40..44 gap closes at renumbering; dormant `warnArraySize` recorded as W10142's
> threshold-source-or-removal decision. ~70 references verified (3 recon agents + 1 independent
> challenger with live compile probes); `Symbol.mutable` already ships; byte-contract arithmetic
> re-derived ✓. **Next: exec_plan (fixes already applied); then Slice 8 `make_plan`; RD-13/RD-14 queued.**)
> Prior: 2026-07-12 (**RD-18 Slice 7b 📋 Plan Created (make_plan)** — the pointer
> surface completing Slice 7: by-ref struct/array params + `const` params (CP-1..5,
> E10122/E10123), unsized array params (`size: null`, byte AND word indexes), tier-2 >256-byte
> arrays via `(zp),Y` (E10118 becomes emittable; W10142/W10143 + W10112 minted), and the
> `load_indirect`/`store_indirect` translate framings — retiring both 7a E90001 loud rejections.
> `plans/rd-18-slice-7b-pointers/` (7 phases / 57 tasks; 14-row gate PASSED). Load-bearing
> decisions: **AR-2 calling convention** (challenger-CONFIRMED): caller stores the address into
> the param's 2-byte FRAME slot (FN-3's normative sentence; `slotSize` already ships the rule),
> SFA colors per-param `__zp_ptr_<fq>_<param>` pairs onto the dormant pool
> (`computePeakPointers` lights up), callee entry block copies frame→pair once (dead/
> pass-through params skip pair + prologue); accepted cost-table drift recorded (~26 vs the
> sketches' ~10 cycles; Phase-B C→A peephole legal). AR-3: by-ref args = statically-addressable
> places + whole pass-through only (runtime-indexed + pair-relative → loud ICE until Slice 8
> `&`). AR-4: ONE conditional `__zp_ptr_scratch` pair under the challenger-hardened predicate
> (incl. const aggregates >256 B) + a translate ICE backstop — pointer-free programs keep a
> byte-identical ZP layout, so all 8 prior goldens stay unmixed. AR-12: new IL `addr` operand
> (store-source-only). Fixture `examples/slice7b/` (two files, six observables over
> $C000..$C006, incl. the index-260 high-byte proof). **RD-18 item 6 ticks when 7b lands.** **Next:
> preflight → exec_plan; then Slice 8 `make_plan`; RD-13/RD-14 queued.**)
> Prior: 2026-07-12 (**RD-18 Slice 7a ✅ COMPLETE (exec_plan 64/64)** — the aggregate
> DIRECT-addressing surface ships end-to-end: arrays/structs/enums through typing → SFA → IL →
> 6502 → in-image data → real VICE. **3-part bar GREEN on real VICE 3.10, first run**: two-file
> `examples/slice7/` → `$C000..$C009 = 0E 2A 08 02 06 02 01 14 0B 03` (indexed loop sum, nested
> member write, scaled struct-array element, enum-dispatch switch, length/sizeof/offsetof folds
> 6/2/1, cross-module `Gfx.TABLE[i]` read from the data section, whole-struct COPY proof,
> enum→word cast); 184-line golden minted post-VICE; seven prior goldens byte-exact NO re-mint.
> Frontend: array literals parse everywhere aggregate literals are legal (`ArrayLitExpr`, 50→51
> kinds; the `parseConstDecl` flag gap fixed; dotted `Mod.Type` annotations); module-keyed FQN
> declaration tables (the bare-name collision defect FIXED + regression-tested); the unified
> lazy memoised const/type engine (constants ⇄ struct layouts ⇄ enum values, ONE path-carrying
> E10165/E10194 per definition cycle, engine-folded `sizeof`/`offsetof`/`length` with
> representability typing ≤255→byte/≥256→word per AR-25); full aggregate expression typing
> (index/member/literal ladders, nominal enum semantics EN-8/9/10, switch-on-enum E10077,
> W10140/W10141, E10157 statement-head literals, aggregate returns E10093/E10120 permanent +
> params loudly rejected pending 7b). Codegen: `lowerPlace` (const offsets fold; runtime indexes
> scale via `mul` → ASL+W10172 pow-2 / `__rt_mul8`+W10170 else), literal init stores + fill
> unroll + per-byte struct copies (local, assignment, `__init` stream), const images →
> `constData` → labeled `!byte` data streams after code; translate prescan def/read fixes +
> tier-1 `abs,X` framings under the per-arm state obligations (X-mirror cleared before every
> LDX; byte loads home via store-fold/ZP spill — `sum = sum + a[i]` compiles; word stores need a
> memory-readable source else LOUD reject; new `protectA()` spills live unhomed A temps at
> mul/const). `ConstValue` gains an optional `bytes` image; `Symbol.type` deliberately mutable
> for the Pass-2 finalize-in-place. 15 facade negatives + advisories; ledger R18/R35/R37/R38/
> R56/R57/R60/R62/R64/R68/R69/R76/R88–R94/R101–R105 advanced; RD-18 item 6 = "7a complete;
> closes at 7b". Full verify green at every phase boundary; `spec/` clean. **Next: Slice 7b
> (pointer surface — by-ref/const params, tier-2 `(zp),Y`, unsized params) needs `make_plan`;
> then Slice 8; RD-13/RD-14 queued.**)
> Prior: 2026-07-11 (**RD-18 Slice 7a 🔄 Executing (exec_plan)** — all 13 accepted
> preflight fixes were APPLIED to the plan docs first (the roadmap's "apply fixes → exec_plan"
> sequence): register now 26 rows (+AR-25 `length` representability spec-drift — ≤255→byte,
> ≥256→word; +AR-26 the PF-007 code pin — mint **E10157 ExpressionStatementNotACall** for
> statement-head aggregate literals, verified free + band-adjacent to E10156, string
> array-initialisers → loud Slice-8 ICE mechanism); E10164→E10097 rippled through
> register/03-04/03-07/ST-35/task-1.2.1/01-requirements/00-index; 03-01 + task 1.2.3 widened to
> `parseConstDecl` (PF-001); 03-06 §1 rewritten — prescan def/read fixes + the per-arm
> state-obligations block + stash-before-LDX word store (PF-002/PF-004); 03-05/AR-15/AR-17
> scaling-warning attribution corrected to ASL+W10172 pow-2 / `__rt_mul`+W10170 non-pow-2
> (PF-003); new ST rows 6a/6b, 26a/26b, 44a/44b, 51a/51b, 53a, 54a; golden path + line cites
> corrected (PF-010); `needsDataInit` dead-flag + `"embed"` member + dotted-type-extension
> notes folded in (PF-011/012/013). Execution starts at Phase 1 (codes, AST & array-literal
> parser); 8 phases / 64 tasks; per-phase commit+push per standing preference.
> Plan: `plans/rd-18-slice-7-aggregates/99-execution-plan.md`.)
> Prior: 2026-07-11 (**RD-18 Slice 7a 🔬 Plan Preflighted** — preflight iteration 1:
> 0 critical / 5 major / 5 minor / 3 observations — **ALL 13 resolved per recommendation**
> (✅ PASSED; report `plans/rd-18-slice-7-aggregates/00-preflight-report.md`; **fixes NOT yet
> applied — apply before exec_plan**; every 🟠 hardened by an independent challenger, converged
> on all five). 🟠 PF-001 `parseConstDecl` (`parse-decl.ts:355`) parses flag-FALSE — const
> aggregate literals (incl. the fixture's own `const TABLE`) don't parse as tasked ("let/const"
> claim was let-only); task 1.2.3 widens to `parseConstDecl` + const-form parser ST rows
> (grammar-mandated: `const_expression = expression`). 🟠 PF-002 translate prescan NOT ready for
> the load ops despite the plan's "already enumerates" claim (`destTempId` misses `value`-as-dest
> for `load_indexed`/`load_indirect`; `readOperands` counts the load's dest as a read) + the
> byte-load "bind A" framing leaves its result un-homed — `sum = sum + a[i]` (the fixture's own
> $C000 shape) dies in the E90001 binder backstop; Phase 6 fixes both + a stash-to-home rule.
> 🟠 PF-003 power-of-two byte scaling emits **ASL + W10172** (`translate.ts:1224-1242`), NOT
> `__rt_mul`/W10170 as plan/AR-15/AR-17 claimed (2-byte `Point` = the canonical case) — prose
> corrected before it poisons the Phase-6/7 oracle; ASM-tier W10172 row added (W10170 only
> non-pow-2). 🟠 PF-004 the word-store framing's `LDX <idx>` destroys an A:X-resident source's
> high byte BEFORE it's read (X mirrors word hi bytes, `translate.ts:166`; one executor branch
> silently stores the index as data) — 03-06 §1 gains a per-arm state-obligations block (def
> visible to prescan, result homed, regA/regX truthful at exit — shared remedy with PF-002),
> stash-before-LDX, and a live-word-across-indexed-access test row. 🟠 PF-005 frozen Ch 08 §9
> "length ≤256 elements → byte" is unrepresentable at exactly 256 (`byte[256]` is legal tier-1)
> — pinned as representability (≤255 → byte, ≥256 → word; TS-21's own shape) + spec-drift
> register row + `byte[255]`/`byte[256]` boundary STs. Minors: THIRD bare-name-table consumer
> (`intrinsic-validation.ts:188-209` via `ctx.tables`) added to the FQN migration list; the
> AR-2/AR-18 promised loud rejections (statement-head aggregate literals — NO callable-statement
> check exists today, `statement-typing.ts:317`; string array-initialisers) now tasked + ST
> rows; **E10164 mint retired → wire Ch 07's own free E10097** (the one outlier from the
> reuse-free-chapter-numbers policy); AR-10 row marked resolved-by-AR-4; goldens live at
> `packages/test-harness/test/golden/` (not root `test/golden/`) + five line-cite corrections.
> Observations: `needsDataInit` verified DEAD (zero consumers — nothing to neutralize);
> `ConstDataEntry.type` carries an `"embed"` member the plan omits; the dotted-type
> `parse-type.ts` extension is confirmed needed (single-identifier `NamedType` today);
> `SymbolKind` already declares struct/enum/enumMember (unconstructed). ~55 file:line references
> verified (3 recon agents + registry/spec reads in lead context); fixture arithmetic re-derived
> ✓; all 13 new codes verified free ✓. **Next: apply fixes → exec_plan.**)
> Prior: 2026-07-11 (**RD-18 Slice 7a 📋 Plan Created (make_plan)** — aggregates through
> the DIRECT-addressing surface; `plans/rd-18-slice-7-aggregates/` (8 phases / 64 tasks). Slice 7
> splits 7a/7b along the addressing-mode seam (AR-1): 7a = decls/typing/tier-1 `abs,X` indexing/
> member access/enums complete/const evaluator/const data/intrinsic folds (aggregate returns =
> permanent E10093/E10120; aggregate params + >256 B arrays loudly rejected); 7b = by-ref params +
> const params + tier-2 `(ZP),Y` + unsized params (own plan later; **RD-18 item 6 ticks at 7b**).
> Zero-Ambiguity Gate ✅ PASSED — 24 items (20 at the gate + 4 authoring-surfaced); independent
> challenger converged on all 7 high-stakes picks. Load-bearing decisions: **array literals are a
> NEW parser surface** (`[1,2,3]` does not parse today — new `ArrayLitExpr`, 50→51 node kinds) with
> Ch-08 fill semantics (`[vals; fill]`; grammar-appendix drift recorded); **chapters-beat-registry
> policy** (E10133 enum-switch exhaustiveness / E10142 duplicate-enum-values / E10141 member cap
> stay registered-unwired — Ch 09 EN-5/§8 govern); ONE **unified lazy memoized const/type engine**
> (array sizes ⇄ struct layouts ⇄ module consts) with exact path-carrying cycles (pure struct-field
> cycle → new E10165; const-involved → E10194; Parked Q1 resolved "no depth limit — exact
> detection"); **cross-module type surface included** AND the verified bare-name declaration-table
> collision DEFECT (passes.ts:35 — two modules' `struct Point` silently last-write-wins) fixed via
> module-keyed/FQN tables; strict Ch-08 tier index rules (register E10117/E10118); lowering owns
> index scaling (`load_indexed` index = byte offset; translate stays arithmetic-free);
> value-dependent `sizeof`/`offsetof`/`length` result typing; struct-literal declaration order
> enforced (mint E10164); enum single-step casts (`<word>(enumVal)` legal). Codes: wire
> E10110–15/E10160–63/E10140+43 + E10077 first emission; register E10093/E10117/E10118/E10119/
> E10120/E10121/E10230 (spec-numbered, free); mint E10126/E10156/E10164/E10165 + W10140/W10141;
> runtime `--bounds-check` deferred (no trap ABI, no config key). Fixture: two-file
> `examples/slice7/` (Main + Gfx exporting struct/enum/const table), `$C000..$C009` band incl. a
> const-expression-sized const table (`byte[DIM + sizeof(Point)]`), scaled struct-array indexing,
> enum dispatch, cross-module reads. **Next: preflight → exec_plan; then Slice 7b `make_plan`.**)
> Prior: 2026-07-11 (**RD-18 Slice 6 ✅ COMPLETE (exec_plan 52/52)** — the full
