...
| 5 | RD-18 | Codegen language-feature completion (thin vertical-slice rollout; 100% working _unoptimized_ codegen for the whole frozen language) | RD-04, RD-05, RD-06, RD-07, RD-09, RD-12, RD-17 | ✏️ authored — needs `make_plan` (per slice) | A→B | ✏️ RD Drafted (2026-07-03 — thin reference-only rollout over RD-04/06/07 + `spec/`; 7 slices 3a→8, each grown across semantics→`modelToFunctionInfo`→IL lowering→IL→Instr and gated by CI assemble-clean + CI golden + local VICE; supersedes phantom `RD-04b` + the ledger's horizontal resume order; consumes `08-deferred-semantics-ledger.md` as backlog; drives RD-04/06/07 open ACs shut; optimizers excluded → Phase B. Decisions AR-110..114; re-check ran recommendation-hardening + 1 independent challenger → structure confirmed, 5 strengthening adjustments folded in. **Keystone = Slice 3a**: wire `modelToFunctionInfo` so the existing gate re-assembles through the real populated-model path.) |
