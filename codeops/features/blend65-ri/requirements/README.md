...
| RD-18 | [**Codegen language-feature completion**](RD-18-codegen-language-completion.md) | **Thin vertical-slice rollout RD** (references RD-04/06/07 + frozen `spec/`; restates no rule). Completes the walking skeleton to **100% working _unoptimized_ codegen** for the whole language via dependency-ordered slices — 3a (model→SFA seam proof) · 3b (scalar type engine) · 4 (control flow) · 5 (user functions & modules) · 6 (full expressions & mixed width) · 7 (aggregates) · 8 (hardware & advanced) — each grown across semantic analysis → `modelToFunctionInfo` → IL lowering → IL→Instr in lockstep, each with a 3-part gate (CI assemble-clean + CI golden + local VICE). Supersedes the phantom `RD-04b`; consumes RD-04's `08-deferred-semantics-ledger.md` as backlog; drives RD-04/06/07 open ACs shut. Optimizers **excluded** (Phase B). AR-110..114 | RD-04, RD-05, RD-06, RD-07, RD-09, RD-12, RD-17 | A→B | 🟢 Authored |
...
RD-18 (Codegen language completion) — rollout over RD-04/05/06/07 + RD-09/12/17;
        vertical slices grow semantics→lowering→translate→VICE; supersedes phantom RD-04b
```
...
| **A→B: Codegen language completion** | RD-18 (slices 3a→8) | Complete the walking skeleton to 100% working _unoptimized_ codegen for the whole frozen language, one VICE-verified vertical slice at a time. Spans the Phase-A/B boundary; **precedes** the two optimizers by design. |
| **B: Optimization, LSP & breadth** | RD-08 + RD-06 Phase B, RD-14, additional platforms | The two optimizers (peephole rules + IL passes) against a fully-lowered IL/Instr contract; VS Code extension/LSP; expand platform coverage. |
