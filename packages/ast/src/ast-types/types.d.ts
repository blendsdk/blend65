 ...
 */
export type TypeSystemNode = PrimitiveType | ArrayType | RecordType | RecordField | NamedType | FunctionType | PointerType | StorageQualifiedType | MemoryPlacement | TypeConstraint;
/**
 * Helper type for all valid type annotations
 */
export type Blend65Type = PrimitiveType | ArrayType | RecordType | NamedType | FunctionType | PointerType | StorageQualifiedType;
/**
...
export declare namespace TypeUtils {
     ...
     */
    function isPrimitive(type: TypeAnnotation): type is PrimitiveType;
    /**
     * Check if a type is an array type
     */
    function isArray(type: TypeAnnotation): type is ArrayType;
    /**
     * Check if a type is a record type
     */
    function isRecord(type: TypeAnnotation): type is RecordType;
    /**
     * Check if a type is storage-qualified
     */
    function hasStorageClass(type: TypeAnnotation): type is StorageQualifiedType;
    /**
     * Get the base type, stripping storage qualifiers
     */
    function getBaseType(type: TypeAnnotation): TypeAnnotation;
    /**
     * Get storage class if present
     */
    function getStorageClass(type: TypeAnnotation): StorageClass | null;
}
