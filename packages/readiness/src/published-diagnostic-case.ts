...
import type { GeneratedCase, PreparedCampaign } from "./campaign-model.js";
...
import {
  ...
  type PublishedCampaignCaseIntentV1,
...
/** Internal reduction facts retained behind one genuine diagnostic capability. */
export interface PublishedDiagnosticCaseReductionInputV1 {
  /** Exact generated invalid case, including baseline, transform and bindings. */
  readonly generatedCase: GeneratedCase;
  /** Defensive public diagnostic projection joined to selected oracle truth. */
  readonly projection: PublishedDiagnosticCaseProjectionV1;
  /** Genuine campaign retaining exact historical replay identity. */
  readonly campaign: PreparedCampaign;
  /** Original zero-based case ordinal. */
  readonly ordinal: number;
}
...
function joinPublishedDiagnosticCase(
  ...
  const modeled = generatedCase.modeledCase;
  if (
  ...
  const request = createPublishedDiagnosticOracleRequest(context, {
    ...
    ordinal: generatedCase.planItem.ordinal,
  ...
  const contentIdentity = deriveOracleSourceContentIdentity(generatedCase.sourceBytes);
  ...
  const projection: PublishedDiagnosticCaseProjectionV1 = Object.freeze({
    ...
    sourceBytes: generatedCase.sourceBytes.slice(),
...
export function createPublishedDiagnosticCaseV1(
  ...
  return joinPublishedDiagnosticCase(context, campaign, generated.value);
}

/**
 * Mints one invalid-source diagnostic authority solely from selected publication state.
 *
 * @param context Genuine selected oracle context.
 * @param intent Exact semantic case intent; executable dependencies are never caller supplied.
 * @returns Opaque diagnostic authority or a bounded validation failure.
 */
export function createPublishedDiagnosticCaseFromIntentV1(
  context: PublishedOracleContext,
  intent: unknown,
): OracleValidationResultV1<PublishedDiagnosticCaseV1> {
  const prepared = preparePublishedCampaignCaseV1(context, intent);
  if (!prepared.ok) return prepared;
  return joinPublishedDiagnosticCase(
    context,
    prepared.value.campaign,
    prepared.value.generatedCase,
  );
}

/** Public semantic intent accepted by publication-bound diagnostic construction. */
export type PublishedDiagnosticCaseIntentV1 = PublishedCampaignCaseIntentV1;

/**
 * Resolves reduction-only invalid-case facts from a genuine diagnostic authority.
 *
 * @param value Candidate diagnostic capability.
 * @returns Exact generated case and defensive projection, or `undefined` for a forgery.
 */
export function getPublishedDiagnosticCaseReductionInputV1(
  value: PublishedDiagnosticCaseV1,
): PublishedDiagnosticCaseReductionInputV1 | undefined {
  const state =
