/**
 * Config-file discovery: the upward walk for `blend65.json` (RD-16 R4).
 *
 * Pure and hermetically testable (AR-P7): the filesystem probe is an injected
...
/** The config file name RD-16 R1 fixes at the project root. */
export const CONFIG_FILE_NAME = "blend65.json";

/**
 * Walks from `startDir` toward the filesystem root, returning the first
 * `<dir>/blend65.json` for which `fileExists` returns true (R4 — the nearest
 ...
 * @param fileExists Probe for a candidate path (injected — AR-P7).
