...
import { createPrimitiveType, createArrayType, createVariableSymbol, createScope, } from '../types.js';
describe('TypeChecker', () => {
    ...
    beforeEach(() => {
        symbols.clear();
        mockSymbolLookup = (name) => symbols.get(name) || null;
    ...
    describe('AST Type Conversion', () => {
        it('should convert primitive types', () => {
            ...
            if (result.success) {
                ...
                expect(result.data.name).toBe('byte');
            }
        });
        ...
        it('should convert named types', () => {
            ...
            if (result.success) {
                ...
                expect(result.data.name).toBe('Player');
            }
        });
    });
    ...
    describe('Expression Type Inference', () => {
        it('should infer types for numeric literals', () => {
            ...
            if (result1.success) {
                ...
                expect(result1.data.name).toBe('byte');
            }
            // Word range literal
            const wordLiteral = {
                type: 'Literal',
            ...
            if (result2.success) {
                ...
                expect(result2.data.name).toBe('word');
            }
        });
        it('should infer type for boolean literals', () => {
            ...
            if (result.success) {
                expect(result.data.kind).toBe('primitive');
                expect(result.data.name).toBe('boolean');
            }
        });
        ...
        it('should type check identifiers through symbol lookup', () => {
            ...
            const varSymbol = createVariableSymbol('counter', createPrimitiveType('byte'), createScope('Global'), mockLocation);
    ...
    describe('Function Call Type Checking', () => {
        it('should validate function calls with correct arguments', () => {
            ...
            if (result.success) {
                expect(result.data.kind).toBe('primitive');
                expect(result.data.name).toBe('word');
            }
        });
 ...
 */
//# sourceMappingURL=type-system.test.js.map