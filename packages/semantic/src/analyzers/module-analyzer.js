...
export class ModuleAnalyzer {
    ...
    errors = [];
    warnings = [];
    dependencyGraph = new Map(); // module -> dependencies
    processingStack = new Set(); // for circular dependency detection
    moduleExports = new Map(); // module -> exported symbols
    programMap = new Map(); // module name -> program
    ...
    collectAllExports(programs) {
        for (const program of programs) {
            ...
            for (const declaration of program.body) {
                ...
                const isExported = declaration.exported === true;
                if (isExported) {
                    switch (declaration.type) {
                        case 'VariableDeclaration':
                            exports.add(declaration.name);
    ...
    detectCircularDependencyDFS(moduleName, visited, path) {
     ...
     */
    resolveImportsInOrder(programs) {
    ...
    resolveModuleImports(program, resolved) {
        const moduleName = this.qualifiedNameToString(program.module.name);
        if (resolved.has(moduleName))
            return;
     ...
     */
    resolveImportDeclaration(importDecl, importingModule) {
        const importedModuleName = this.qualifiedNameToString(importDecl.source);
     ...
     */
    addImportedSymbol(importingModule, localName, importedName, sourceModule) {
