 ...
 */
import { createVariableSymbol, isPrimitiveType, } from '../types.js';
/**
...
export class VariableAnalyzer {
     ...
     */
    analyzeVariableDeclaration(varDecl, currentScope) {
        const errors = [];
        ...
        if (varDecl.storageClass) {
            const storageResult = this.validateStorageClassUsage(varDecl.storageClass, variableType, varDecl.initializer !== null, currentScope, location);
        ...
        if (varDecl.initializer) {
            const initResult = this.validateInitialization(varDecl.initializer, variableType, varDecl.storageClass, location);
            if (!initResult.success) {
                errors.push(...initResult.errors);
            }
        }
        // 5. Check required initializations for storage classes
        const initRequiredResult = this.checkRequiredInitialization(varDecl.storageClass, varDecl.initializer, location);
     ...
     */
    validateStorageClassUsage(storageClass, variableType, hasInitializer, currentScope, location) {
        const errors = [];
        ...
        const storageResult = this.typeChecker.validateVariableStorageClass(storageClass, variableType, hasInitializer, location);
        return storageResult;
    }
    /**
     * Check for duplicate variable declarations in current scope
     */
    checkDuplicateDeclaration(variableName, location) {
     ...
     */
    validateInitialization(initializer, variableType, storageClass, location) {
        const errors = [];
        ...
        const compatResult = this.typeChecker.checkAssignmentCompatibility(variableType, initializerType, location);
     ...
     */
    checkRequiredInitialization(storageClass, initializer, location) {
     ...
     */
    validateConstantInitializer(initializer, location) {
     ...
     */
    collectVariableUsageMetadata(variables, expressionResults) {
     ...
     */
    analyzeZeroPageCandidates(variables) {
        const candidates = [];
        for (const variable of variables) {
     ...
     */
    analyzeZeroPageCandidate(variable) {
        const promotionFactors = [];
        const antiPromotionFactors = [];
     ...
     */
    analyzeRegisterCandidates(variables) {
        const candidates = [];
        for (const variable of variables) {
     ...
     */
    analyzeRegisterCandidate(variable) {
        let allocationScore = 0;
        let preferredRegister = 'memory';
        const alternativeRegisters = [];
     ...
     */
    analyzeVariableLifetimes(variables, _cfg) {
        const lifetimeInfos = [];
        for (const variable of variables) {
     ...
     */
    buildVariableOptimizationMetadata(variables, usageData) {
     ...
     */
    generate6502Hints(variable, usage, zeroPageInfo, _registerInfo) {
        return {
            addressingMode: zeroPageInfo.isCandidate ? 'zero_page' : 'absolute',
            memoryBank: variable.storageClass === 'zp'
                ? 'zero_page'
     ...
     */
    generateMemoryLayoutInfo(variable, usage, zeroPageInfo) {
        return {
     ...
     */
    calculateVariableSizeInBytes(varType) {
        switch (varType.kind) {
            case 'primitive':
                switch (varType.name) {
                    case 'byte':
 ...
 */
export function analyzeVariableDeclaration(varDecl, symbolTable, typeChecker, currentScope) {
...
//# sourceMappingURL=variable-analyzer.js.map