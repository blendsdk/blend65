 ...
 */
import { isPrimitiveType, isArrayType, isNamedType, isCallbackType, createPrimitiveType, createArrayType, createNamedType, createCallbackType, areTypesEqual, typeToString, validateStorageClassUsage, } from './types.js';
// ============================================================================
...
export class TypeChecker {
     ...
     */
    convertASTTypeToBlend65Type(astType, location) {
        switch (astType.type) {
            case 'PrimitiveType':
                return {
                    success: true,
                    data: createPrimitiveType(astType.name),
                };
            case 'ArrayType':
     ...
     */
    resolveNamedType(namedType, location, visited = new Set()) {
     ...
     */
    checkAssignmentCompatibility(target, source, location) {
        // Resolve named types
     ...
     */
    checkSpecificCompatibility(target, source, location) {
     ...
     */
    checkPrimitiveCompatibility(target, source, location) {
     ...
     */
    checkArrayCompatibility(target, source, location) {
     ...
     */
    checkCallbackCompatibility(target, source, location) {
        // Return types must be compatible
        const returnCompatibility = this.checkAssignmentCompatibility(target.returnType, source.returnType, location);
        ...
        for (let i = 0; i < target.parameterTypes.length; i++) {
            const paramCompatibility = this.checkAssignmentCompatibility(source.parameterTypes[i], // Note: contravariant
            target.parameterTypes[i], location);
     ...
     */
    validateVariableStorageClass(storageClass, varType, hasInitializer, location) {
        if (!storageClass) {
        ...
        const basicValidation = validateStorageClassUsage(storageClass, this.currentScopeType, hasInitializer);
     ...
     */
    validateStorageClassForType(storageClass, varType, location) {
        ...
        if (storageClass === 'io') {
            if (!isPrimitiveType(concreteType) ||
                ...
                default:
                    return {
                        ...
                        errors: [
                            {
                                errorType: 'InvalidOperation',
                                message: `Unknown primitive type: ${type.name}`,
                                location,
                            },
                        ],
                    };
     ...
     */
    checkArrayAccess(arrayType, indexExpression, location) {
     ...
     */
    checkArrayLiteralType(arrayLiteral, expectedType, location) {
        // Check element count
        ...
        for (let i = 0; i < arrayLiteral.elements.length; i++) {
            ...
            const compatibilityResult = this.checkAssignmentCompatibility(expectedType.elementType, elementResult.data, location);
     ...
     */
    validateFunctionSignature(functionDecl, location) {
        ...
        for (const param of functionDecl.params) {
            ...
            if (param.defaultValue) {
                ...
                const compatibilityResult = this.checkAssignmentCompatibility(paramTypeResult.data, defaultValueTypeResult.data, location);
                if (!compatibilityResult.success) {
                    return {
                        success: false,
                        errors: [
        ...
        if (functionDecl.callback) {
            ...
            const callbackValidation = this.validateCallbackFunction(parameters, returnTypeResult.data, location);
     ...
     */
    validateCallbackFunction(parameters, returnType, location) {
        // Callback functions should have minimal parameter complexity for 6502
        for (const param of parameters) {
            if (isArrayType(param.type)) {
                const arraySize = param.type.size;
                if (arraySize > 256) {
                    return {
                        success: false,
                        errors: [
     ...
     */
    checkFunctionCall(signature, argumentTypes, location) {
        ...
        for (let i = 0; i < argumentTypes.length; i++) {
            ...
            const compatibilityResult = this.checkAssignmentCompatibility(expectedType, actualType, location);
            if (!compatibilityResult.success) {
                return {
                    success: false,
                    errors: [
    ...
    checkExpressionType(expression) {
        switch (expression.type) {
            case 'Identifier':
                return this.checkIdentifierType(expression.name, expression.metadata?.start ?? { line: 0, column: 0, offset: 0 });
            case 'Literal':
     ...
     */
    checkIdentifierType(name, location) {
        ...
        if (symbol.symbolType === 'Function') {
            ...
            return {
                success: true,
                data: createCallbackType(funcSymbol.parameters.map(p => p.type), funcSymbol.returnType),
            };
        }
        return {
            success: false,
    ...
    checkBinaryExpressionType(expression) {
        ...
        switch (expression.operator) {
            ...
            case '/':
            case '%':
                return this.checkArithmeticOperation(leftType, rightType, expression.operator, expression.metadata?.start ?? { line: 0, column: 0, offset: 0 });
            case '==':
            case '!=':
            case '<':
            case '<=':
            case '>':
            case '>=':
                return this.checkComparisonOperation(leftType, rightType, expression.operator, expression.metadata?.start ?? { line: 0, column: 0, offset: 0 });
            case 'and':
            case 'or':
                return this.checkLogicalOperation(leftType, rightType, expression.operator, expression.metadata?.start ?? { line: 0, column: 0, offset: 0 });
            default:
                return {
                    success: false,
                    errors: [
     ...
     */
    checkArithmeticOperation(leftType, rightType, _operator, location) {
     ...
     */
    checkComparisonOperation(leftType, rightType, _operator, location) {
     ...
     */
    checkLogicalOperation(leftType, rightType, _operator, location) {
    ...
    checkCallExpressionType(expression) {
        ...
        const signature = {
            parameters: calleeType.parameterTypes.map((type, index) => ({
                name: `param${index}`,
                type,
        ...
        return this.checkFunctionCall(signature, argumentTypes, expression.metadata?.start ?? { line: 0, column: 0, offset: 0 });
    }
    /**
     * Check member expression type (simplified).
     */
    checkMemberExpressionType(expression) {
     ...
     */
    checkIndexExpressionType(expression) {
        ...
        return this.checkArrayAccess(objectType, expression.index, expression.metadata?.start ?? { line: 0, column: 0, offset: 0 });
    }
    /**
     * Check array literal expression type.
     */
    checkArrayLiteralExpressionType(expression) {
        ...
        for (let i = 1; i < expression.elements.length; i++) {
            ...
            const compatibilityResult = this.checkAssignmentCompatibility(elementType, elementResult.data, expression.metadata?.start ?? { line: 0, column: 0, offset: 0 });
            if (!compatibilityResult.success) {
                return {
                    success: false,
                    errors: [
