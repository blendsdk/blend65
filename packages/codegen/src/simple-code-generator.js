 ...
 */
import { ILInstructionType, isILConstant, isILVariable, isILLabel } from '@blend65/il';
/**
...
export class SimpleCodeGenerator {
    ...
    constructor(options) {
        ...
        this.platform = PLATFORM_SPECS[options.target];
        if (!this.platform) {
    ...
    generateBasicStub() {
        ...
        return [
            ...
            `* = $${basicStart.toString(16).toUpperCase()}`,
            '        .word next_line',
            '        .word 10        ; Line number',
            '        .byte $9E       ; SYS token',
            `        .text "${entryPoint}"`,
            ...
            `* = $${entryPoint.toString(16).toUpperCase()}`
        ];
    }
    /**
            ...
            case ILInstructionType.LOAD_MEMORY: {
                ...
                if (this.isILValue(source)) {
                    if (isILVariable(source)) {
                        ...
                        lines.push(`LDA $${source.value.toString(16).toUpperCase()}  ; Load from address`);
                    }
                }
                break;
            }
            case ILInstructionType.STORE_MEMORY: {
                ...
                if (this.isILValue(dest)) {
                    if (isILVariable(dest)) {
                        ...
                        lines.push(`STA $${dest.value.toString(16).toUpperCase()}  ; Store to address`);
                    }
                }
                break;
            }
