 ...
 *
 * Defines the `CpuVariant` primitive (D2 — the *only* profile fact 07a needs),
 * the inline ACME `AcmeDirective` union (R6), the `StreamEntry` discriminated
 * union (instr | label | directive, R5/R6), and the per-function `InstrStream`
 * container (R7), plus trivial constructors and type guards.
 *
...
/**
 * The CPU variant a stream targets (decision D2).
 *
 * This is the **only** platform-profile primitive RD-07a needs. When RD-10 lands
 * it will expose `profile.cpuVariant` and the RD-07b caller will read it; 07a's
 * signatures are unchanged — no fabricated `PlatformProfile`.
 */
