...
import { SimpleCodeGenerator, SimpleCodeGenOptions } from '../simple-code-generator.js';

describe('SimpleCodeGenerator', () => {
  const defaultOptions: SimpleCodeGenOptions = {
    target: 'c64',
    debug: false,
    autoRun: true
  };
  ...
  it('should handle arithmetic operations', async () => {
    ...
    func.instructions = [
      ...
      createILInstruction(
        ILInstructionType.LOAD_IMMEDIATE,
        [createILConstant({ name: 'byte' } as any, 10)],
        1
      ),
      // Add 5 (10 + 5)
      createILInstruction(
      ...
      createILInstruction(
        ...
        [
          createILConstant({ name: 'byte' } as any, 15),
          createILConstant({ name: 'byte' } as any, 3)
        ],
        3
      ),
