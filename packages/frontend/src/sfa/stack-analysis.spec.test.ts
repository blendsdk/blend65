/**
 * Specification tests for the SFA stack-depth analysis pass (RD-05 §4.9,
 * R37–R40; spec Ch 11 §5, Ch 06 §7.8).
 *
 * Expectations derive exclusively from plans/rd-05-sfa-frame-planner/
 * 07-testing-strategy.md (ST-S1..ST-S3) and 03-04-stack-and-budgets.md — NOT from
...
describe("Specification: SFA stack-depth analysis (R37–R40, §4.9)", () => {
  // ST-S1 — chain main→a→b (no irq): maxMainDepth 3; maxMainStackBytes 6;
  // irqOverhead 0.
  ...
  // ST-S2 — with one interrupt irq→x: irqOverhead 6; maxIrqStackBytes 4;
  // total = main + 6 + 4.
  ...
  // ST-S3 — total stack ≥ 75% budget → exceedsWarningThreshold true. Use a tiny
  ...
  // §4.9 — no main present → maxMainDepth 0, valid record (error tolerance, R60).
