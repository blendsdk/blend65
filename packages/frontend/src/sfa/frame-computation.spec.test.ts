/**
 * Specification tests for the SFA frame-computation pass (spec Ch 11 §3.3).
 *
 * Expectations derive exclusively from the language specification — NOT from
 * implementation logic. Immutable oracle (testing.md Rule 10).
 *
...
describe("Specification: SFA frame computation (R1–R6, §3.3)", () => {
  ...
  // Struct param passed by-ref → 2-byte pointer (struct byteSize 4).
  ...
  // computeFrame orders params before locals, offsets 0,2; totalSize 3.
  ...
  // Function with no params/locals → empty slots, totalSize 0.
  ...
  // A slot whose type is void contributes 0 and never throws.
  ...
  // The always-live flags are carried through to the frame.
