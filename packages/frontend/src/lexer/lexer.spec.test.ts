 ...
 * flag. String-valued `TokenKind` is what keeps this readable. A literal value
 * is JSON-encoded so raw escape text (`\n`, quotes) stays visible.
 */
 ...
 * Including the span and message pins both recovery ordering and the exact
 * message strings in a single golden snapshot.
 */
...
describe("lexer — Phase 6 behavioral spec & golden snapshots", () => {
  ...
  describe("ST-L39: public API surface", () => {
    ...
    it("re-exports the token vocabulary from @blend65/core", () => {
      // `TokenKind` is a string-valued const map; `Token`/`LexResult` are
      // type-only and erased at runtime, so we assert the value export that
    ...
    it("returns a LexResult carrying tokens and a lineMap", () => {
      ...
      // The lineMap is built once over the source; offset 0 is line 1.
      expect(result.lineMap.getLineCol(0)).toEqual({ line: 1, column: 1 });
    });
  });

  // ----- golden token list for a representative .blend -----
  ...
  // ----- golden tokens + ordered diagnostics -----
