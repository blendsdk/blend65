...
/**
 * The 32 reserved keywords, mapped to their token kinds.
 *
 * Contextual keywords (`until`, `to`, `downto`, `step`) are deliberately ABSENT
 * — the lexer always emits `Identifier` for them and the parser recognizes
 * them positionally. Reserved built-in identifiers (`peek`, `main`, …) are
 * likewise absent — the semantic analyzer enforces those, not the lexer.
 *
