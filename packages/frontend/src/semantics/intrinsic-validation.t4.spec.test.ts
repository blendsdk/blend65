 ...
 * implementation (IMMUTABLE ORACLE RULE). Authored as `.todo` in Phase 2; live as
 * of Phase 5 (task 5.1.1) with a test-local T4 fixture descriptor (AR-P2 allows a
 * test-local factory; the plugin-shaped fixture lives in `@blend65/platforms`).
 *
 * Expected behaviour (AR-P14 — Option A: distinct codes):
 ...
 * - Clean path (AC-05): import present on the owning platform → no E10046/E10043.
 */
...
/** Test-local T4 fixture descriptor (AR-P2): `fix_probe(): void`, call lowering. */
...
/** A canonical profile with the given platform id (availability keys on it — R25). */
