...
import { isSlotOperand, isImmediateOperand, isInlineContinuationLabel } from '../../il/guards.js';
...
export class ConstantPropPass implements OptimizationPass {
   ...
   *
   * **Inline continuation label exception:** Labels generated by the
   * function-inlining pass (pattern `_inline_*_cont`) are sequencing-only —
   * no external code jumps to them. Constants known before these labels
   * are still valid after, so we propagate through them safely.
   *
   * @param instr - Instruction to check
  ...
  protected clearsAllConstants(instr: ILInstruction): boolean {
    ...
    // Labels are merge points — EXCEPT inline continuation labels which
    // are sequencing-only and safe to propagate through
    if (opcode === ILOpcode.LABEL) {
      return !isInlineContinuationLabel(instr);
    }
