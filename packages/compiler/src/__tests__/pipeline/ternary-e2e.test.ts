 ...
 *
 * @module tests/pipeline/ternary-e2e
 */
 ...
 */
function compileSource(source: string, stopAfterPhase?: 'parse' | 'semantic' | 'il' | 'optimize' | 'codegen') {
...
describe('Ternary E2E - Semantic Phase', () => {
  describe('type checking success', () => {
    it('should type check ternary with byte branches', () => {
      ...
      expect(result.phases.semantic?.diagnostics.filter(d => d.severity === DiagnosticSeverity.ERROR).length).toBe(0);
    });
  ...
  describe('type checking errors', () => {
    it('should report error for non-boolean condition', () => {
      ...
      const errors = result.phases.semantic?.diagnostics.filter(d => d.severity === DiagnosticSeverity.ERROR) ?? [];
      expect(errors.length).toBeGreaterThan(0);
      expect(errors.some(e => e.message.toLowerCase().includes('bool') || e.message.toLowerCase().includes('condition'))).toBe(true);
    });
...
describe('Ternary E2E - Error Handling', () => {
  describe('parse errors', () => {
    it('should report error for incomplete ternary (missing colon)', () => {
      ...
      const hasIssue = !result.phases.parse?.success ||
                       (result.phases.parse?.diagnostics.length ?? 0) > 0;
  ...
  describe('semantic errors', () => {
    it('should report error for undefined identifier in condition', () => {
      ...
      const hasErrors = !result.phases.semantic?.success ||
