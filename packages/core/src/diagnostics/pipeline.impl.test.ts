 ...
 * (RD-11b Phase 4, task 4.3.2).
 *
 * Proves the R36–R38 flow end-to-end on the core side (AR-77): a producer
 * populates a real DiagnosticBag (errors, warnings, an ICE, and a
 * sentinel-span config diagnostic), the severity policy is applied exactly
 * once post-collection, and both renderers consume the policy-applied array —
 * with zero printing inside core (everything returns strings).
 */
...
describe("bag → policy → render chain (R36–R38)", () => {
  it("collects, applies the policy once, and renders both formats from the same array", () => {
    // --- Collect (R36): a realistic producer run over one interned file. ---
    ...
    // A config diagnostic on the RD-16 sentinel source (-2) — never interned.
    bag.addError(DiagCode.ConfigInvalidValue, makeSpan(-2, 0, 5), "Invalid value for 'maxErrors'");

    // --- Apply the policy exactly once, post-collection (R31/PF-005). ---
    ...
    // null-span ICE degrade to header-only (R51); promotion shows through.
    ...
    // JSON: all four records mirrored, sentinel span verbatim (R51/AR-Q10).
    ...
    // Zero printing inside core: both renderers returned strings; nothing on
    // stdout is asserted structurally by AC-14's audit (task 4.3.3).
