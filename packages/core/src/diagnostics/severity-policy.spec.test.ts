/**
 * Specification tests for the severity policy.
 *
 * Transcribed from the severity-policy requirements.
...
describe("severity policy (RD-11 R27–R31, R50)", () => {
  // Blanket promotion keeps the W-code string unchanged.
  it("ST-6: blanket warnAsError promotes warnings to errors with the code string unchanged", () => {
    ...
    // The warning became an error — but its code is STILL the W-code (it
    // renders as error[W10191] so the user can find the flag to disable).
    expect(result[1].severity).toBe("error");
    expect(result[1].code).toBe(DiagCode.UnusedVariable);
  });

  // Selective promotion touches only the listed codes.
  ...
  // Suppression removes the warning from the output entirely.
  ...
  // Suppression wins over promotion, selective AND blanket.
  ...
  // Errors/ICEs/sentinel pass through; order and inputs intact.
  it("ST-10: errors, ICEs, and the E10000 sentinel pass through untouched in input order without mutation", () => {
    ...
    // The promoted warning kept its position (input order preserved).
  ...
  // Promotion is exempt from the bag's --max-errors cap.
