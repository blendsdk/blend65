 ...
 */
function createMockOptimizationPattern(id, category, priority, improvementPercentage = 15) {
    return {
        id,
...
function createMockPatternRegistry() {
    ...
    return {
        getPatternsByCategory: vi.fn().mockImplementation((category) => {
 ...
 */
function createMockQualityAnalysis() {
 ...
 */
function createMockCFGAnalysis() {
    return {
        controlFlowGraph: {
            entryBlock: 0,
            exitBlocks: [3],
 ...
 */
function createMock6502Analysis() {
    return {
        platformValidation: {
...
describe('PatternReadinessAnalyzer', () => {
    let analyzer;
    let mockPatternRegistry;
    let mockProgram;
    let mockQualityAnalysis;
    let mockCFGAnalysis;
    let mock6502Analysis;
    beforeEach(() => {
        ...
        analyzer = new PatternReadinessAnalyzer(mockPatternRegistry);
    });
    describe('Pattern Applicability Analysis', () => {
        it('should analyze patterns from all categories', () => {
            const result = analyzer.analyzePatternReadiness(mockProgram, mockQualityAnalysis, mockCFGAnalysis, mock6502Analysis);
        ...
        it('should filter out patterns with low applicability scores', () => {
            ...
            mockPatternRegistry.getPatternsByCategory.mockImplementation((category) => {
                if (category === 'basic') {
    ...
    describe('Pattern Conflict Prediction', () => {
        ...
        it('should skip conflict prediction when disabled', () => {
            ...
            const analyzerWithOptions = new PatternReadinessAnalyzer(mockPatternRegistry, options);
            const result = analyzerWithOptions.analyzePatternReadiness(mockProgram, mockQualityAnalysis, mockCFGAnalysis, mock6502Analysis);
    ...
    describe('Optimization Strategy Generation', () => {
        ...
        it('should adapt strategy based on risk tolerance', () => {
            ...
            const conservativeAnalyzer = new PatternReadinessAnalyzer(mockPatternRegistry, conservativeOptions);
            const result = conservativeAnalyzer.analyzePatternReadiness(mockProgram, mockQualityAnalysis, mockCFGAnalysis, mock6502Analysis);
    ...
    describe('Application Sequencing', () => {
        ...
        it('should prioritize higher-ranked patterns in earlier phases', () => {
            ...
            if (result.applicablePatterns.length > 5 && result.applicationOrder.length > 1) {
                ...
                const firstPhaseRanks = firstPhasePatterns.map(id => result.applicablePatterns.find(p => p.pattern.id === id)?.overallRank || 0);
                const lastPhaseRanks = lastPhasePatterns.map(id => result.applicablePatterns.find(p => p.pattern.id === id)?.overallRank || 0);
    ...
    describe('Integration with Quality Metrics', () => {
        it('should use complexity metrics to influence pattern selection', () => {
            ...
            const result = analyzer.analyzePatternReadiness(mockProgram, highComplexityAnalysis, mockCFGAnalysis, mock6502Analysis);
            ...
            if (result.applicablePatterns.length > 0) {
                ...
                const avgSafety = result.applicablePatterns.reduce((sum, p) => sum + p.safetyScore, 0) /
        ...
        it('should use performance prediction to prioritize patterns', () => {
            ...
            const result = analyzer.analyzePatternReadiness(mockProgram, lowPerformanceAnalysis, mockCFGAnalysis, mock6502Analysis);
            // Low performance should increase impact scores
            if (result.applicablePatterns.length > 0) {
                const avgImpact = result.applicablePatterns.reduce((sum, p) => sum + p.impactScore, 0) /
                    result.applicablePatterns.length;
        ...
        it('should integrate with 6502 validation results', () => {
            ...
            const result = analyzer.analyzePatternReadiness(mockProgram, mockQualityAnalysis, mockCFGAnalysis, rich6502Analysis);
    ...
    describe('Pattern Weighting and Goals', () => {
        it('should weight patterns differently based on performance goals', () => {
            ...
            const performanceAnalyzer = new PatternReadinessAnalyzer(mockPatternRegistry, performanceOptions);
            const result = performanceAnalyzer.analyzePatternReadiness(mockProgram, mockQualityAnalysis, mockCFGAnalysis, mock6502Analysis);
            ...
            if (result.applicablePatterns.length > 0) {
                ...
                if (highImpactPatterns.length > 0 && lowImpactPatterns.length > 0) {
                    const avgHighImpactRank = highImpactPatterns.reduce((sum, p) => sum + p.overallRank, 0) /
        ...
        it('should weight patterns differently based on safety goals', () => {
            ...
            const safetyAnalyzer = new PatternReadinessAnalyzer(mockPatternRegistry, safetyOptions);
            const result = safetyAnalyzer.analyzePatternReadiness(mockProgram, mockQualityAnalysis, mockCFGAnalysis, mock6502Analysis);
    ...
    describe('Error Handling and Edge Cases', () => {
        it('should handle empty pattern registry gracefully', () => {
            ...
            const emptyAnalyzer = new PatternReadinessAnalyzer(emptyRegistry);
            const result = emptyAnalyzer.analyzePatternReadiness(mockProgram, mockQualityAnalysis, mockCFGAnalysis, mock6502Analysis);
        ...
        it('should handle minimal IL programs', () => {
            ...
            const result = analyzer.analyzePatternReadiness(minimalProgram, mockQualityAnalysis, mockCFGAnalysis, mock6502Analysis);
    ...
    describe('Performance and Scalability', () => {
        ...
        it('should scale metrics based on pattern count', () => {
            ...
            const largePatternSet = Array.from({ length: 20 }, (_, i) => createMockOptimizationPattern(`pattern_${i}`, 'mathematics', 2, 15));
            mockPatternRegistry.getPatternsByCategory.mockImplementation((category) => {
    ...
    describe('Real-world Integration Scenarios', () => {
        it('should handle complex optimization readiness data', () => {
            ...
            const result = analyzer.analyzePatternReadiness(mockProgram, complexAnalysis, mockCFGAnalysis, mock6502Analysis);
...
//# sourceMappingURL=pattern-readiness-analyzer.test.js.map