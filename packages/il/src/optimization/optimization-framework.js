 ...
 */
import { ILInstructionType, isILConstant, createILInstruction, createILConstant, } from '../il-types.js';
import { ILAnalyticsSuite } from '../analysis/il-analytics-suite.js';
import { OptimizationLevel, OptimizationCategory, OptimizationErrorType, DEFAULT_OPTIMIZATION_CONFIG, } from './types.js';
/**
...
export class ILOptimizationFramework {
     ...
     */
    async optimizeProgram(program, config = DEFAULT_OPTIMIZATION_CONFIG) {
        ...
        try {
            ...
            const finalMetrics = this.calculateFinalMetrics(originalProgram, program, passResults, sessionStartTime);
            const summary = this.generateOptimizationSummary(originalProgram, program, passResults, analytics);
            return {
     ...
     */
    async optimizeFunction(ilFunction, config = DEFAULT_OPTIMIZATION_CONFIG) {
        const program = {
     ...
     */
    createOptimizationContext(program, analytics, config) {
     ...
     */
    selectOptimizationPasses(context) {
     ...
     */
    async executeOptimizationPasses(program, passes, context) {
        const results = [];
     ...
     */
    calculateFinalMetrics(originalProgram, optimizedProgram, passResults, sessionStartTime) {
        const totalTime = Date.now() - sessionStartTime;
        const successfulPasses = passResults.filter(r => r.success);
        const totalCyclesSaved = passResults.reduce((sum, r) => sum + Math.abs(r.metricsChange.cyclesDelta), 0);
        const totalSizeChange = passResults.reduce((sum, r) => sum + r.metricsChange.sizeDelta, 0);
        const effectiveness = this.calculateEffectiveness(originalProgram, optimizedProgram, totalCyclesSaved);
        return {
            totalCyclesSaved,
            totalSizeChange,
            totalMemorySaved: Math.abs(passResults.reduce((sum, r) => sum + r.metricsChange.memoryDelta, 0)),
     ...
     */
    generateOptimizationSummary(originalProgram, optimizedProgram, passResults, analytics) {
            ...
            execute: (_program, _context) => {
                ...
                for (const module of _program.modules) {
                    for (const func of module.functions) {
                        ...
                        func.instructions = func.instructions.filter((inst) => inst.type !== ILInstructionType.NOP ||
     ...
     */
    createBasicConstantFoldingPass() {
        return {
                    ...
                    for (const func of module.functions) {
                        ...
                        for (let i = 0; i < func.instructions.length - 2; i++) {
                            ...
                            if (inst1.type === ILInstructionType.LOAD_IMMEDIATE &&
                                ...
                                const optimizedInstruction = createILInstruction(ILInstructionType.LOAD_IMMEDIATE, [createILConstant({ kind: 'primitive', name: 'byte' }, sum, 'decimal')], inst1.id, {
                                    metadata: {
    ...
    createBasicFunctionOptimizationPass() {
        return {
    ...
    createRegisterOptimizationPass() {
        return {
    ...
    createPeepholeOptimizationPass() {
        return {
    ...
    createSizeOptimizedConstantFoldingPass() {
        return this.createBasicConstantFoldingPass(); // Size-optimized version
    }
    createCodeSizeReductionPass() {
        return {
    ...
    calculateTimePerCategory(passResults) {
    ...
    generateDebugInfo(_passResults) {
        return {
            analyticsAccuracy: {
    ...
    calculateEffectiveness(originalProgram, optimizedProgram, cyclesSaved) {
    ...
    countInstructions(program) {
        return program.modules.reduce((total, module) => total +
            module.functions.reduce((funcTotal, func) => funcTotal + func.instructions.length, 0), 0);
    }
    estimateProgramComplexity(program) {
        return (this.countInstructions(program) +
            program.modules.length * 10 +
            program.modules.reduce((sum, m) => sum + m.functions.length, 0) * 5);
    }
    calculatePerformanceImprovement(passResults) {
        const totalCyclesSaved = passResults.reduce((sum, r) => sum + Math.abs(r.metricsChange.cyclesDelta), 0);
        return Math.min(100, totalCyclesSaved / 10); // Scale to percentage
    }
    calculateSizeChange(passResults) {
        return passResults.reduce((sum, r) => sum + r.metricsChange.sizeDelta, 0);
    }
    identifyTopPatterns(passResults) {
    ...
    generateRecommendations(analytics, passResults) {
 ...
 */
export async function optimizeIL(program, patternRegistry, config = DEFAULT_OPTIMIZATION_CONFIG) {
 ...
 */
export async function optimizeFunction(ilFunction, patternRegistry, config = DEFAULT_OPTIMIZATION_CONFIG) {
    const framework = new ILOptimizationFramework(patternRegistry);
    return framework.optimizeFunction(ilFunction, config);
}
//# sourceMappingURL=optimization-framework.js.map