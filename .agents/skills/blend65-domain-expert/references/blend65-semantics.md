...
> **Baseline version**: `1.0.0`. This module routes decisions to the reconciled specification;
> it does not replace it.
>
> **Current source identity**: `BLEND65-SPEC-P3-4bf8a989`, exact 50-path digest
> `4bf8a98934df6282febe013c31a16d89dd73f8e8bbbce9e87b6678e1b75013fe`. This baseline binds
> every accepted product ruling through AR-P43 and every consistency repair through SC-155,
> including the final no-view fixed-array, complete index-operator, loop-reachability,
> stable-query, representable-object, HLE-disclosure, and aggregate-return-debt contracts. Earlier
> grades are supporting evidence only.
> Definitive evaluation, independent grading, and changed-surface review are pending for this exact
> identity. Any specification edit invalidates it.
>
...
product rulings and derived-document repairs have closed recorded conflicts through SC-155 in the
current candidate. Any genuine
...
- inspect every live and resolved `FUT-NNN` entry, including aggregate-return debt and its
  `EXP-001` lineage, rather than trusting an aggregate count;
...
- bound manual address workarounds to a named addressable object's lifetime and require a
  separately passed address for a parameter field;
- include migration and target-HLE disclosures instead of treating them as non-semantic prose;
...
- for Atari 7800 timing, keep the 1.79 MHz nominal CPU rate separate from 1.19 MHz TIA/RIOT
  accesses and MARIA DMA stalls.
...
above. In particular, name the lifetime-bounded `&object + offsetof(...)` workaround and the
separately passed address required for a by-reference parameter field. Also name the C64 default:
...
|---|---|---|---|---|---|
| `spec/00-feature-index.md` | Discovery index and diagnostic mirror | axioms, feature ownership, feature-to-chapter routing, diagnostic assignments, and discovery of live `FUT-NNN` entries without a duplicated endpoint | use as discovery/checksum surface; preserve owning chapter meaning; Chapter 14 owns public diagnostic fields | [Authority](#authority-and-use); [Diagnostics](#diagnostic-doctrine) | None — adds navigation and registry guidance, but not independent semantics |
...
| `spec/future-considerations.md` | Future consideration | every live and resolved `FUT-NNN`, its source, dependency, rationale, and exact reconsideration trigger | audit the live register entry by entry rather than trusting a copied count; do not implement a live deferral as current behavior; re-open it when its stated reason expires | [Authority](#authority-and-use) | No current semantics. Hotspots: FUT-009 permits manual base-plus-offset only for a named addressable object within its lifetime and requires a separately passed address for a parameter field; FUT-010 records E10093/E10120 aggregate-return debt and the caller-owned hidden-destination/SFA direction; FUT-016 charges static frame and optional stack-free return-home storage per allocated activation, including bounded overlapping homes/entry variants |
...
| `spec/v2-to-v3-migration.md` | Migration context | removed/changed v2 constructs and replacement direction, including initializer/startup changes, every accepted HLE, aggregate-return debt, warning scopes, and bounded address workarounds | never preserve v2 behavior unless v3 normative text does; diagnose accidental legacy carryover; do not restore blanket BSS clearing or generic DATA copying; preserve W10190 only for function-local maybe-read-before-first-assignment and W10141 independently for every nonzero uninitialized mutable array; keep E10093/E10120 as redesign debt; disclose default unchecked modulo-65536 indexing and default-off checks | [Restriction triage](compiler-architecture.md#restriction-triage) | No independent current semantics. HLE-003 includes byte-by-byte `$FFFF` to `$0000` access under active banking/MMIO; a base-plus-offset workaround is valid only for a named addressable object within its lifetime, while a parameter field needs a separate passed address |
...
| functions | return annotation is mandatory; all value types parse so semantics can accept scalar/enum returns and diagnose current-v3 aggregate returns; interrupt functions declare `(): void`; read-only aggregate parameters use `name: const Type` and reject scalar/enum `const` | preserves every distinction, reports current v3 accurately, and treats the aggregate-return rejection as redesign debt rather than a hardware necessity | Reconciled current behavior; EXP-001 tracks the restriction |
...
- Initializer/update expression lists evaluate left to right. A header declaration is an ordinary
  local `let` or `const`; its scope covers condition, update, and body but not code after the loop.
  The body is a nested scope and ordinary no-shadowing applies. Mutation, conversion, call, MMIO,
  and fixed-width wrap rules remain ordinary language rules. The former range words `until`, `to`,
  `downto`, and `step` are ordinary identifiers.
...
omitted condition means `true`; the exact `let` and `const` initializer forms and their ordinary
mutability/initialization rules; left-to-right evaluation of every initializer and update
expression-list element; header-local scope and no-shadowing; `continue`/`break`/`return` update
edges; and that header commas delimit only these lists rather than adding a general comma operator.
...
Ordinary `+`, `-`, `+=`, and `-=` always use binary fixed-width semantics, regardless of the
processor's D or C flags. Packed decimal exists only through these four overloads:
...
Raw `asm_*()` calls are ordered opaque barriers with conservative A/X/Y/all-flag clobbers. A raw
`asm_sed()` path reaching ordinary arithmetic, address formation, a call, an exit, or a mismatched-D
join before `asm_cld()` is E10255; W10120 is retired.
...
This ledger records current specified restrictions that ordinary modern-language users will not
expect and that hardware does not force. These restrictions must be reported accurately during a
v3 audit, but the skill must never defend them as 6502 doctrine or preserve them merely because an
existing frontend, ABI, or lowering path is convenient.

| ID | Current v3 behavior | Why it is debt | Required redesign direction | Proof required before adoption |
|---|---|---|---|---|
| EXP-001 | E10093 rejects a fixed-struct return and E10120 rejects a fixed-array return. Callers must currently pass an aggregate destination explicitly. | The 6502 and SFA do not require this source restriction. A compiler can make the caller own the final storage without a heap, dynamic frame, or general runtime. | Use a compiler-managed hidden return destination: the caller supplies an SFA-known final address and the callee constructs directly into it. Permit direct construction/copy elision; do not mandate an intermediate callee-frame object or hidden byte copy. | Specify aliasing, lifetime, nested-call staging, mainline/IRQ/NMI overlap, destination ownership, error recovery, SFA closure, and complete ROM/RAM/ZP/stack/cycle costs. Preserve ordinary left-to-right source effects and prove every elided copy equivalent. |

EXP-001 is a future language/ABI change, not a claim that the current compiler already supports
aggregate returns. Audit results classify the current restriction as an expressiveness defect and
keep it outside any finite assembly-parity ratio.
...
|---|---|---|---|
| SC-001..SC-003 | feature index, Chapter 06, F018, Chapter 14 | Function codes retain E10170–E10176 and E10180–E10181 meanings; there is no parameter-count limit. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-004 | Chapter 10, F003, F019 | Module `let` initializers accept legal non-`void` expressions and run once before `main` in deterministic dependency/effect order. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-005 | Chapters 02 and 04 | Wide `<<` yields 0; wide signed-negative `>>` yields -1; other wide `>>` yields 0. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-006 | Chapter 05, F008, examples | The former range-loop direction/bound/step contract was reconciled at this checkpoint. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-007 | introduction, Chapter 06, F007/F018, future considerations | `interrupt function` is callback-only; recognized sinks select raw or firmware entry variants; ordinary helpers use `RTS`; overlapping invocation-private storage is disjoint; globals/assets/MMIO remain shared. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-008 | Chapter 13, C64 appendix, F015 | CharPad component meanings, placement, and VIC register derivation are separate and zero-copy. Canonical `"tiles"`/`"map"` use the smallest lossless `byte[]` or little-endian `word[]`; explicit forced-word, exact packed-12, and independently selected low/high planes provide all useful 8/12/16-bit C64 layouts; no representation truncates or silently emits another. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-009 | Chapter 12, F020 | A runtime-address memory intrinsic may require a compiler-owned two-byte ZP pointer, charged through SFA/resource accounting. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-010 | Chapters 04/05, F024, grammar | Assignment is a lowest-precedence, right-associative value expression with exactly-once target/RHS/store rules. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-011 | Chapters 06/08, F014/F018, grammar | Read-only aggregate parameters use `name: const Type`; constness is transitive and has no ABI cost. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-012 | Chapter 04, F017, Chapter 15 | Constant zero division is E10160; runtime zero has a bounded unspecified result with no default injected handling. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-013 | Chapter 08, F014, Chapter 15 | Default dynamic OOB indexing wraps the 16-bit effective address; optional checks are explicit inline instrumentation. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-014 | syntax-owning chapters, grammar, lexical/migration copies | The derived grammar now matches declarations, arrays, casts, literals, intrinsics, and bounded embed syntax. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-015 | Chapter 05, grammar | General expression statements are legal and do not reuse W10131. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-016 | owning chapters, feature index, Chapter 14 | Owners define predicates/consequences; Chapter 14 defines public fields; both active sets contain 177 codes (148 errors, 29 warnings), and collision/retirement history is durable. W10190 applies only to function-local definite assignment; W10121, range-only E10060/E10061/E10062/E10064/W10060, and array-width E10085/E10117/E10118/W10142 are retired. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; earlier grades are historical; definitive requalification pending |
| SC-017 | Chapters 06/11/15 and F018 | Stack overflow compares the simultaneous program peak with derived usable capacity; W10180 uses the explicit profile threshold or the defined 80%-rounded-down default. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-018 | Chapters 03/10 and all five target appendices | Target startup performs only selected device bootstrap, executes each scheduled module/zeropage initializer once, falls through into `main`, and applies the target return epilogue; it does not blanket-clear uninitialized storage or copy a generic data segment. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-019 | Chapters 07/08, F011, F014 | Arrays of structs are valid v3 source. SoA is an optional, measured layout choice; its possible cost advantage cannot become a language restriction. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-020 | Chapter 12, F012, F020, future considerations | Current memory access uses volatile `peek`/`poke` intrinsics. Separate `volatile_read`/`volatile_write` APIs remain deferred and cannot be recommended as current behavior. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-021 | feature index, future considerations | Deferred-feature discovery follows the complete live set of `FUT-NNN` entries; no summary duplicates its endpoint or resolved subset. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-022 | `AGENTS.md`, source manifest, compiler architecture, IL/optimization doctrine | ACME selection and automatic GitHub parity-debt recording are explicit product/process policy, not language, hardware, or optimizer semantics. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-023 | Chapters 03/10, target appendices, v2-to-v3 migration | The migration guide now reflects scheduled one-time `let` initialization, no emitted initialization for omitted initializers, and image-resident aggregate constants; it no longer restores obsolete blanket BSS clearing or generic DATA copying. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-024 | Chapter 03, Chapter 14, feature index, v2-to-v3 migration | Variable shadowing is E10101. E10031 remains exclusively the prohibition on `const` declarations inside `zeropage`. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-025 | Chapters 01/08/15, all target appendices, F014/F021, grammar, feature index, Chapter 14, migration | The full closed escape syntax is target-neutral; `\0`/`\xNN` are exact, every other escape and ordinary literal scalar uses the selected compile-time one-byte encoding/map, and E10249 rejects an unavailable mapping. C64/C64U use game-oriented screen-code defaults; unqualified future targets use the exact raw identity baseline until their specialized maps are qualified. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-034 | Chapter 13, F015/F016, grammar, feature index, Chapter 14, migration, future considerations | Format-aware import requires a literal path and, when supplied, a literal selector. The exact case-sensitive selector is handler-owned, may be file-derived, and creates no core query language. A registered handler resolves an explicit selector, its one documented default, or E10132; E10250 rejects a non-literal selector. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-035 | Chapter 13, C64 appendix, F015 | Initial handlers qualify against an exact latest-stable application release and accept only the file's pinned observable signature/version; an application release not encoded in the file is provenance, not a detectable rejection predicate. Unsupported, malformed, old, and new format versions are E10204. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-036 | Chapter 13, C64 appendix, F015 | SpritePad Pro 3.80 SPD v5 uses exact 64-byte sprite records and a word-sized count; sprite mode/color/expansion/overlay are per-record attributes, optional components remain typed, requested derived tables are costed, and VIC block values come from placement operations rather than asset selectors. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-037 | Chapter 04, F017 | For a nonzero divisor, quotient truncates toward zero and `r = a - q*b`; a nonzero signed remainder has the dividend's sign. Shift/mask strength reduction requires a proof of equivalent signed behavior. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-038 | Chapter 13, C64/C64U appendices, F015, future considerations | Classic 10,003-byte Koala files expose only bitmap, screen, color-RAM, and background components after exact load-address/layout/color validation. Bitmap/screen `$D018` fields derive from final common-bank placement; color-RAM transfer remains explicit and costed. Native decomposition is distinct from deferred modern-image conversion. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-039 | Atari appendices, F015 | Initial Atari 7800 and Atari 800XL profiles expose raw embedding only. PNG/TMX and FNT/RIP/RMT handlers reopen only with the relevant target's separately researched skill extension and exact source, format, selector, layout, placement, cost, failure, and fixture qualification. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-040 | Commander X16 appendix, F015 | Commander X16 remains a first-class planned target but its initial profile exposes raw embedding only. Its separately qualified platform-skill extension owns production W65C02/VERA/banking depth and reopens asset handlers, beginning with official ZSM revision 1; the stale `.zcm` identity is invalid. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-041 | Chapter 02, F010 | Same-size integer casts preserve bits and add no conversion sequence, but distinct source variables remain independent objects. Storage coalescing requires ordinary liveness, alias, volatility, and interference proof; representation equality never creates an alias. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-042 | Chapter 13, Chapter 14, F015 | E10140 applies to an explicit length mismatch for any embedded array element type. E10204 owns malformed or unsupported recognized-format input and remains part of F015's active diagnostic summary. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-043 | Chapter 06, v2-to-v3 migration, SFA/ABI reference | The v3 ABI uses caller-stored static-frame parameter homes, with scalar/enum values copied and aggregate base addresses passed by reference; scalar/enum returns use A or A/X. A different ABI needs a future versioned specification decision. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-044 | Chapter 04, F017 | The cost table now distinguishes unsigned/proven-nonnegative power-of-two division and remainder from signed operations that need sign-aware correction or general lowering. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-045 | Chapter 06, v2-to-v3 migration | Removing the v2 `callback` keyword does not turn every callback into an interrupt handler. Ordinary callbacks remain ordinary `RTS` functions referenced with `&name`; callback-only `interrupt function` declarations use the raw or firmware variant selected by a recognized platform sink. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
...
| SC-047 | Chapter 01, future considerations, v2-to-v3 migration | `type` is reserved but unusable in v3. Type aliases are rejected under REJ-001 and are not a provisional current feature. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-048 | Chapter 01, F021 | The declared token enumeration contains 32 operator tokens and 79 total token types. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-049 | Chapters 01/08/15, all target appendices, F014/F021, grammar, Chapter 14, feature index | Literal tokens preserve exact Unicode scalar values. Every current target encoding is a finite compile-time scalar-to-one-byte map; absent ordinary characters/symbolic escapes and non-single-byte character results are E10249. There is no normalization, replacement, UTF-8 target emission, or runtime conversion; `\0`/`\xNN` remain exact. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-050 | Chapter 15, target appendices, Chapter 08/F014 examples, Chapter 14, feature index | Every effective encoding binds to an immutable map identity. C64/C64U expose exact upper/graphics and lower/upper screen-code/PETSCII maps with a literal per-call override and no hardware change; A7800/A800XL/X16 use `ascii-raw-v1` until separately qualified target maps exist; custom charsets require explicit metadata. E10125/E10249/E10251 own map availability, scalar availability, and argument form. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-051 | Chapter 01, Chapter 08, F021, grammar | Character literals are valid non-ASCII literal contexts, and every grammar now accepts the same single scalar set: any scalar except quote, backslash, CR, and LF. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-052 | F015, Atari appendices | The stale PMG `base_page` selector example is removed; current Atari profiles remain raw-embed-only until their target extensions qualify handlers. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-053 | Chapter 06, SFA/ABI reference | A function has one logical frame layout and as many disjoint static instances as simultaneously live execution domains require; the total after coloring is compile-time known. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
...
| SC-055 | F016, F019, governing Chapters 02/03/10 | Evaluation documents explain rationale and rejected alternatives; they are not independent semantic authority and cannot call themselves the single source of truth. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-056 | F019, Chapters 03/10, target appendices | Startup uses the selected platform bootstrap. Scheduled initializer bodies may call ordinary functions. Only the final transition from completed initialization into `main` is guaranteed to use fallthrough rather than `JSR`/`JMP`. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-057 | Chapter 01, F021 | The lexical surface contains three literal categories: integer, character, and string. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-058 | Grammar definitions and production index | The historical P3-001b1331 grammar contained 92 definitions and index rows. The current SC-131 candidate adds four for-loop productions, giving 96 definitions and 96 unique index rows. A line comment ends at the lexical LF byte or EOF; it does not reference an undefined `newline` production. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-059 | Chapters 06/12/15, C64/C64U appendices, F007/F012/F018, architecture/IL/SFA references, source manifest, Q-L29/Q-P07 | NMOS interrupt entry establishes binary mode before Blend65 handler/helper execution. Default CINV chaining preserves entry flags with `PHP; CLD`/`PLP`; raw/exclusive variants use `CLD` and eventual `RTI` restoration. The two-byte saved chain link cannot begin at `$xxFF`; all bytes, cycles, and stack costs are reported and proof-based elision must preserve both boundaries. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-060 | Master grammar, Chapter 01, F021 | Lexical character sets use ISO special sequences rather than comment notation; line-comment terminators are explicit special sequences; and every operator production includes `?`. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-061 | Chapter 04, Chapters 05/06, architecture/IL/SFA references | Immediate expression subexpressions evaluate exactly once from left to right unless a more specific short-circuit, selected-arm, assignment, or call rule applies. Associativity changes grouping, not observable effect order. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
...
| SC-063 | Chapters 01/08, F014/F021, master grammar | Every lexical grammar surface spells reverse solidus, HT, CR, LF, and end-of-file as unambiguous ISO special sequences; no source-language escape convention is assumed by the EBNF metalanguage. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-064 | Migration guidance, Chapter 14, feature index | Migration guidance reports the same active diagnostic inventory as the governing registry. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-065 | Chapter 04, master grammar | `conditional_expr` always admits a plain logical-or expression and optionally adds the right-associative `? :` tail in every grammar fragment. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-066 | C64 appendix, F015 | SpritePad's governing target contract declares `"sprites"` as the default when the selector is omitted, matching the accepted evaluation. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-067 | Chapter 15, C64 appendix, F015 | The generic platform-profile example names only SpritePad's real `"sprites"` default and delegates the exhaustive selector set to the target profile; it does not advertise a fictitious `"colors"` selector. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-068 | Chapter 06, F018/F019, master grammar | Function and variable type fragments use one non-recursive array-element production with an optional extent, admitting supported unsized parameters while excluding deferred multidimensional arrays. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-069 | Chapter 04, master grammar | The expression fragment admits every current primary class, including primitive casts, aggregate literals, intrinsics, and embedding; it does not narrow the master grammar. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-070 | F005/F014/F019/F020 and governing Chapters 03/08/12 | Evaluation grammar fragments require a non-empty zeropage block, permit ordinary initializer and array-base expressions, accept any array-typed `length` expression, and apply semantic constant evaluation to the full expression grammar. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-071 | Chapters 01/08, F014/F021, master grammar | Every scalar-exclusion special sequence names quote, reverse solidus, CR, and LF by Unicode code point, avoiding source-escape or quote-spelling conventions inside EBNF prose. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-072 | Chapters 03/10 and target appendices | Chapter 03 governs declaration/initializer semantics, Chapter 10 governs cross-module startup scheduling, and the selected target appendix governs bootstrap/epilogue behavior; no chapter claims all three. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-073 | F024, Chapter 04, master grammar | F024's optional conditional tail uses the declared ISO concatenation commas and matches the governing right-associative expression production. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-074 | Chapter 15, target appendices, F018, Chapters 06/08/14 | The unsupported `warn_frame_size` profile surface is removed; exact frame sizes remain reported. Array thresholds use W10143, W10030 remains zero-page usage, W10180 remains hardware-stack peak, and W10191 remains unused-variable. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-075 | C64 appendix, Chapter 15, F015 | The initial SID handler has one exact current selector set: `"data"` (also the default), `"init_address"`, and `"play_address"`. Unsupported header-metadata/data-size selectors are not advertised. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-076 | C64 appendix, Chapter 15 | C64 documentation reports exact SFA frame/instance costs and total RAM fit without claiming the removed arbitrary single-frame warning. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-077 | Chapter 08, F014, master grammar | Value-list array literals do not accept a trailing comma; the derived master grammar cannot independently add that syntax. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-078 | Chapter 13, C64/C64U appendices, F015, source manifest | The initial SID handler is pinned to the official HVSC format-description snapshot and accepts one exact self-contained PSID v1–v4 subset with deterministic validation, address resolution, rejection, fixed-placement rules, and exact SC-133 profile/player compatibility. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-079 | Chapters 03/07/11/13/15, F005/F011, C64/C64U appendices | Every resource warning has an exact profile field or default formula, rounding rule, comparison, and advisory consequence. Frame-size warning policy remains removed. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-080 | Chapter 12, F012/F020 | CPU-control calls omit the statement semicolon in their expression grammar. All intrinsics are reserved built-in function identifiers with E10212 redeclaration. Its earlier depth-only explicit-stack conclusion is superseded by SC-128. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-081 | Chapters 03/07/08/09, F011/F014/F019 | Enum constants are scalar/inlined; enum fields and enum arrays are legal; aggregate summaries exclude `void`. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-082 | Chapter 08, F014, master grammar | Array fragments use the master production names and expression forms, with no undefined stale aliases. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-083 | Chapters 02/06/14, F018 | Return expressions use assignment compatibility and the matching conversion diagnostic. E10172 is exclusively argument mismatch. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-084 | Chapters 01/08/13/14, F014/F015, grammar, index, migration | Empty strings and zero-length arrays are valid zero-byte values with deterministic indexing/address behavior. E10111 is retired; E10131 remains an empty-raw-input policy error. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-085 | Chapter 15, C64/C64U appendices | `$0801`–`$CFFF` is consistently 51,199 bytes; overlapping code/data/mutable/SFA placements fit collectively rather than treating budget maxima as separate pools. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-086 | F014/F018 | Diagnostic-count summaries match their tables, and function resource accounting covers complete SFA closure, all static instances, and full simultaneous hardware-stack peak. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-087 | Chapter 15 | A present `warn_array_size` fires W10143 at `measured >= threshold`, must be in `1..65535`, and has no inferred default; omission disables only that advisory warning. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-088 | Chapter 15, C64 appendix | Every duplicate of the default C64 profile advertises the same complete initial format-handler set. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-089 | F020, master grammar | Memory-intrinsic grammar fragments use the declared `type` and `qualified_name` productions for `sizeof` and `offsetof`; no undefined `type_name` alias remains. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-090 | Chapter 08, F014 | Historical address-only any-size-parameter rule. Superseded by SC-134: an any-size parameter now carries the complete fixed argument's word element count, and `length(parameter)` reads it. | Superseded by SC-134 under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-091 | C64 appendix | The default PRG image starts at `$0801`; its 12-byte BASIC auto-start line occupies `$0801`–`$080C` and enters generated startup at `$080D`. No byte at unloaded `$0800` is part of the stub. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-092 | Chapter 12, F012 | W10120 is retired. E10255 rejects a raw `asm_sed()` path that reaches ordinary arithmetic, address formation, a call, a terminal, or a mismatched-D join before `asm_cld()`. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-093 | Chapters 03/05, master grammar | Variable fragments use `value_type`; local constants require `const_expression`; top-level `let`/`const` accept optional `export`, while local declarations do not. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-094 | F014, Chapter 06, Chapter 14 | A sized-array argument mismatch uses canonical argument diagnostic E10172; no placeholder diagnostic is published. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-095 | C64 appendix | The 35-byte default W10110 threshold is described as approximately, not at least, one quarter of the 142-byte allocatable zero-page budget. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-096 | Chapter 11, Chapter 15, C64 appendix | The default C64 memory example uses the canonical `$02`–`$8F` 142-byte zero-page budget and shared `$0801`–`$CFFF` program span. Its inclusive segment and total sizes are arithmetically exact. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-097 | Atari 7800 appendix | Placeable code and const data end at `$FFEF`. The packaging-owned `$FFF0`–`$FFFF` verification/vector trailer is reserved, and the `$8000`–`$FFEF` inclusive budget is exactly 32,752 bytes. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
...
| SC-099 | Chapters 13/15, target appendices | `embed_formats` contains signature/version-validating handlers only. Format-neutral one-argument raw-byte embedding, including `.bin` and `.prg`, is the universal fallback and is never registered as a `raw_binary` handler. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
...
| SC-101 | Chapter 15, C64/C64U appendices | The duplicated default C64 profile uses the selected PAL record's exact `0.985248` MHz derived CPU clock and names the NTSC record's exact `1.022730` MHz value; timing reports do not use a rounded value as machine identity. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-102 | Chapters 13–15, F015, feature index | W10150 measures embedded bytes against `max_binary_size`; its public message and rationale call this the binary-size budget, not an undefined data budget. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-103 | Chapters 03/11/15, C64 appendix, F019 | A contiguous load image serializes emitted bytes and internal padding as one prefix, then reserves mutable/SFA storage as a trailing non-emitted BSS suffix. Reports distinguish emitted payload from the complete shared-range footprint. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-104 | Historical Chapter 05/F008 range-loop design | The former range-loop declaration used `value_type` and imposed an integer-only counter rule. SC-131 supersedes that complete design. The current three-clause initializer is one ordinary local `let` or `const`; no parser- or SFA-driven integer-only clause restriction survives. | Historical only under SC-131; never apply to current `for` syntax |
| SC-105 | Chapters 02/14, F010/F016/F024, feature index | Diagnostic examples retain mandatory declaration annotations and distinct names so E10081/E10162 are isolated from E10150/redeclaration errors. E10150's public template covers variables, constants, and parameters; a missing return annotation remains E10170. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-106 | Chapter 01, Chapter 08, F021 | String examples use inferred extents, so `"HELLO WORLD"` owns exactly 11 bytes and `""` owns zero; they do not accidentally trigger const-array incompleteness. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-107 | Chapter 13, F015, target appendices | `embed()` dispatch is total: a registered extension always validates through its handler and uses the explicit selector, its default, or E10132; only an unregistered extension without a selector is raw fallback, while one with a selector is E10137. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-108 | Chapter 06, F018, master grammar | Function parameter and return fragments use the shared `value_type` production and do not define a stale competing `type_expr` alias. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-109 | Chapter 15, target appendices | A profile requires a target ID, supported CPU identity, positive selected clock, and load address. Reset-vector presence is determined by the packaging contract, and an explicit stack-warning threshold must fit usable capacity. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-110 | Chapters 08/14/15, F014, feature index, A7800 appendix | W10141 requires a nonzero uninitialized mutable array. W10143 measures one mutable array's RAM allocation at `>= warn_array_size`; const arrays do not trigger it. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-111 | Chapters 01/08/10/15, F004/F014/F021, master grammar | Twenty-nine intrinsic names are globally reserved, including both BCD operations and four target-encoding names. `main` is separately entry-reserved: only the exact entry declaration is legal, and E10020–E10023—not E10212—govern it. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-112 | F018 | The complete function error summary includes missing-path return, interrupt-sink ABI, local-address escape, and unbounded overlap/stack errors: 17 errors plus 2 warnings. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-113 | CX16 appendix | The default BASIC-loaded PRG controls `$0801`–`$9EFF`, exactly 38,655 bytes. Physical `$0800` is outside the load image and cannot satisfy code/data/BSS placement. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-114 | Chapter 15, A800XL appendix | XEX serializes ordered, non-overlapping emitted intervals and RUNAD, omits mutable/SFA BSS holes, and reports emitted container bytes separately from the full shared footprint. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-115 | Chapter 13, F015, target appendices, SC-034/SC-107 | Format-aware `embed()` always has a literal path; a selector is literal when present. Registered handlers resolve explicit/default selector dispatch or E10132. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-116 | Chapter 07, F011 | W10112 fires only when two mutable struct arguments at one call site are statically proven to designate the same base storage; may-alias uncertainty alone does not warn. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-117 | Chapter 04, F017 | W10170/W10171 fire exactly for selected software-helper calls. W10172 fires for a selected constant-multiply sequence containing a shift plus add/subtract; one power-of-two shift is excluded. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-118 | Chapters 13/14, F015, feature index | Identical canonical path/selector/representation outputs are one immutable object and address. W10151 reports the aliasing; bytes and fixed placement occur once. Different components are distinct. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-119 | Chapter 04, F006, master grammar, diagnostics/index | Address-of parses a unary operand so invalid forms receive semantic diagnostics. E10042 owns field/element forms; E10043 owns every other non-name literal/expression. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-120 | Chapters 03/04, F006/F019/F020 | Published 6502 byte/cycle examples use exact immediate, zero-page, and absolute instruction costs and distinguish intrinsic cost from argument/result materialization. Aggregate startup costs are selected-lowering totals, not a false fixed multiplier. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-121 | Chapter 11, A7800 appendix | A7800 code and const data share ROM `$8000`–`$FFEF`; default mutable/SFA storage uses the conservative `$2200`–`$27FF` general-allocation range. `$FFF0`–`$FFFF` is packaging-owned, with vectors at `$FFFA`–`$FFFF`. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-122 | Chapter 06, F018 | Call-cost examples separate the four-byte, twelve-cycle `JSR`/`RTS` core from parameter stores, result materialization, body work, and addressing-mode-dependent costs. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-123 | Chapter 04, F003, F017 | Multiplication and modulo examples use explicit operand widths, preserve the specified intermediate width, and do not rely on an accidental wider destination to change the operation. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
| SC-124 | F003, F019, F020 | Examples use the accepted `0b` binary-literal syntax and distinct declaration names, so they demonstrate the intended rule instead of triggering unrelated lexical or redeclaration errors. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; prior grades are historical; definitive requalification pending |
...
| SC-140 | F016 warning summary, Chapter 02 and Chapter 14 | W10160/W10161 apply when narrow runtime arithmetic is widened by assignment, argument or return binding, or another explicit semantic context. The summary no longer narrows the trigger to assignment only. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; earlier grades are historical; definitive requalification pending |
| SC-141 | Chapters 01/07/08/14, F011/F014 | Examples and optimization preconditions preserve source validity, typing, and full effective-offset proof: the word-valued color-ramp expression narrows explicitly to `byte`; array-of-struct byte indexing requires proof that the complete effective offset fits the selected byte form; and generated-code examples do not redeclare reserved intrinsic names. Declared aggregate size alone is insufficient. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; earlier grades are historical; definitive requalification pending |
| SC-142 | Chapters 04/05/07/08/12 and F009/F010/F011/F013/F014/F017/F020/F024 | Published cycle/ROM totals use one stated accounting boundary and exact 6502 instruction costs. Repairs cover scalar arithmetic and comparisons, multiply/shift sequences and helper ranges, struct operations, array accesses/fill, memory intrinsics, conditional selection, switch dispatch, and loop control. W10172 follows the selected lowering rather than every non-power-of-two constant. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; earlier grades are historical; definitive requalification pending |
| SC-143 | Chapters 02/04/05/12, F008/F009/F014/F016/F017, and historical SC-046 | Final cost and supersession audit makes branch-page assumptions explicit, separates instruction-core from complete-sequence totals, binds extension costs to actual materialization, and preserves inline-versus-helper selection for division/remainder. E10073 remains an error, and obsolete range-loop wording is historical only under SC-131. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; earlier grades are historical; definitive requalification pending |
| SC-144 | Chapters 02/04/07/08/12/14, F011/F014/F016/F020, diagnostics/index, migration, semantics and Q-L19 | `sizeof()` and `offsetof()` are stable compile-time `word` queries. Array extents are full-precision integers in `0..65535`; complete fixed array/struct sizes are in `0..65535` bytes. E10264/E10265 reject out-of-domain types, and E10266 rejects `sizeof(T[])`. Structs are not artificially capped at 255 bytes, and no runtime or wider source integer is added. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; earlier grades are historical; definitive requalification pending |
| SC-145 | Chapters 08 and F014, AR-P37, semantics/IL references, and Q-L19 | The direct index-ordinal context promotes before every unbarriered integer-producing operation: unary `~`/`-`, arithmetic, shifts, and bitwise operators. Existing signedness rules still apply; comparisons/logical operators yield invalid-index `boolean`. Explicit 8-bit casts, typed 8-bit assignments/compound assignments, and completed calls remain narrow barriers. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; earlier grades are historical; definitive requalification pending |
| SC-146 | Feature index; Chapters 04–06/08/10; F013/F016/F017/F018/F020/F024; future considerations; master grammar | Final impact repair removes stale switch/promotion costs, reserved-name examples, incomplete operator/helper accounting boundaries, unregistered advisories, untyped diagnostic/workaround examples, and obsolete conditional-expression prose. The derived grammar routes all 29 reserved built-ins through deterministic intrinsic-call lexeme dispatch while leaving ordinary identifiers disjoint in call position. No product behavior or diagnostic set changes. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; earlier grades are historical; definitive requalification pending |
| SC-147 | Chapter 08, F014, E10240, and Q-L19 | The examples now make the frozen boundary result explicit: ordinal 510 is valid for `shifted[600]` and is E10240 for `data[500]`. This is an example-completeness repair only; index semantics and the no-view array model are unchanged. | Content reconciled under BLEND65-SPEC-P3-4bf8a989; earlier grades are historical; definitive requalification pending |
| SC-148 | Chapter 00, Chapter 15, and A7800 appendix | The Atari 7800 summary now uses the governing 1.79 MHz nominal CPU clock and states the separate 1.19 MHz TIA/RIOT access rate plus MARIA DMA stalls. | AR-P42 accepted the detailed target-profile timing model; Q-L01/Q-L13 must reject a flat 1.19 MHz CPU-clock model |
| SC-149 | Feature index A4 and Chapter 02 TS-4/TS-14 | A4 now forbids hidden execution and unenumerated coercions while preserving the two explicitly specified implicit-conversion families: same-signedness mixed-width widening and enum-to-byte. | AR-P42 preserves the normative type rules; Q-L01/Q-L14/Q-L20 must accept only enumerated implicit conversions |
| SC-150 | Chapter 00, feature index, and future-considerations register | Related-document summaries use count-free `FUT-NNN` wording so a duplicated endpoint or resolved subset cannot silently drift from the live register. | AR-P42 accepted a durable navigation summary; Q-L01 must compare the live register rather than trust a copied count |
| SC-151 | F001 and semantic crosswalk | The F001 row is limited to supplied files, one binary, path-name irrelevance, and module-based cross-file references. It explicitly refuses unsupported discovery, module-to-file, duplicate-module, and diagnostic-stability inferences. | AR-P42 accepted the narrow evidence boundary; Q-L01 must flag any broader F001 claim as unsupported |
| SC-152 | Future register, function/array chapters, ABI references, migration, and Q-A08 | Current E10093/E10120 aggregate-return rejection is modern expressiveness debt, not an HLE. The required redesign direction is caller-owned hidden destination passing through SFA with direct construction/copy elision and full alias/lifetime/overlap/cost proof. | Current v3 behavior remains reportable; no hardware rationale may freeze it into the redesign |
| SC-153 | Array/variable diagnostics, migration, semantics, and Q-L23 | W10190 covers function-local maybe-read-before-first-assignment; W10141 independently covers every nonzero uninitialized mutable array, including module arrays. Both may apply to one function-local array because declaration state and a read path are distinct predicates. | HLE-005 documentation now preserves both warning scopes without blanket initialization |
| SC-154 | Future register and F018 function resource accounting | Static frame and optional stack-free return-home costs are per allocated frame/invocation instance, not per source function. Bounded simultaneous activations may require distinct homes and entry variants; no-recursion alone is insufficient. | SFA overlap analysis owns multiplicity; unbounded selection falls back to JSR/RTS or rejects an explicitly required stack-free mode |
| SC-155 | Future register, local-address provenance, migration, and C64 appendix | Manual base-plus-offset workarounds apply only to named addressable objects and lifetime-contained use; parameter fields need a separately passed address. Default C64 indexing follows HLE-003 modulo-address behavior, while checks remain explicit and default-off. | Removes universal workaround and missing-default-boundary claims without adding a new language concept |
...
Use citations such as `[BLEND65-SPEC-P3-4bf8a989, spec/06-functions.md §FN-10]`. A citation is an audit
