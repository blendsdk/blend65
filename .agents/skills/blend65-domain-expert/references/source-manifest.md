...
| Q-C13 | BLEND65-SPEC-d39ae45 `spec/04-expressions-operators.md:112-117` and `spec/02-type-system.md:429-446`; MOS-PGM-1976 Chapter 10 §§10.0–10.4 (`LSR`, `ASL`, `ROR`, `ROL`); CC65-2.19 `libsrc/runtime/asr.s` (comparative sign-propagating implementation only) | **Blocked conflict:** the product ruling preserves arithmetic sign extension for signed `>>`, including counts at least the width: negative operands produce `-1`, and non-negative operands produce `0`. Unsigned `>>` and `<<` produce `0` at those counts. The 6502 practice check confirms that signed shifts explicitly propagate the sign through carry; cc65 also confirms that its C rule leaves counts at least the width undefined, so it does not decide Blend65's deterministic wide-count result. The operator chapter still conflicts, so no wide-count lowering invariant freezes until a separately authorized spec commit reconciles it. |
...
| SRC-009 | `spec/04-expressions-operators.md:115-116` says a shift by at least the width is always zero and that signed right shift sign-extends; `spec/02-type-system.md:429-446` also requires sign extension. | The product ruling preserves arithmetic sign extension for signed `>>`: a wide count produces `-1` for a negative operand and `0` otherwise; unsigned `>>` and `<<` produce `0`. Keep SC-005 and Q-C13 blocked until a separately authorized spec commit reconciles the general rule. | The CPU shift effects and product intent are factual; the wide-count Blend65 oracle cannot freeze while the specification still contradicts itself. |
