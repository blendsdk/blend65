...
| SRC-010 | `spec/05-statements-control-flow.md:215-255,309-327` defines inclusive `to` and full-byte `0 to 255`; `spec/evaluations/F008-for-loop.md:244-258` defines exclusive `to` and allows `0 to 256`. | No semantic choice is made in skill construction. Record SC-006, block Q-C19, and require a product ruling plus reconciled spec commit before qualification. | The hardware wrap idiom remains factual; which Blend65 source form invokes it is blocked. |
