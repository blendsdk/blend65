...
- [ ] **FR-1** — `TokenKind` is a string-valued `const … as const` map with a derived
  `TokenKindValue` union, exported from `@blend65/core`. *(AR-L6; RD-02 §4.1)*
- [ ] **FR-2** — `TokenKind` contains exactly the 77 kinds of Ch 01 §12: 4 literals
  (`Number`, `String`, `Char`), 1 `Identifier`, 32 keywords, 29 operators, 10 punctuation,
  `Eof`. *(Ch 01 §12; RD-02 §4.1 note)*
- [ ] **FR-3** — `Token` is `{ readonly kind: TokenKindValue; readonly span: SourceSpan;
  readonly value?: number | string }`, exported from `@blend65/core`. *(AR-L3; RD-02 §4.2)*
- [ ] **FR-4** — `Token.value` is set for `Number` (parsed 0–65535), `String`/`Char` (raw
...
- [ ] **FR-5** — `KEYWORD_MAP` maps the 32 keyword lexemes to their `TokenKind`; lives in
  `@blend65/frontend`. *(AR-L4; RD-02 §4.3; Ch 01 §5.1)*
- [ ] **FR-6** — An identifier-shaped run (`[A-Za-z_][A-Za-z0-9_]*`) is looked up in
  `KEYWORD_MAP`: hit → keyword token, miss → `Identifier`. *(Ch 01 §4.1, §5.1; RD-02 R8/R9)*
- [ ] **FR-7** — `until`, `to`, `downto`, `step` are **not** in `KEYWORD_MAP`; they always
  produce `Identifier`. *(Ch 01 §5.1.1; RD-02 R10)*
- [ ] **FR-8** — `true`/`false` → `KwTrue`/`KwFalse`; `type` → `KwType` (parser later emits
  E10224 — the lexer just tokenizes it). *(Ch 01 §5.1, §5.2; RD-02 R11/R12)*
- [ ] **FR-9** — Fully case-sensitive: `break`→keyword, `Break`→identifier. *(Ch 01 §2.2; RD-02 R2)*
- [ ] **FR-10** — Reserved built-in identifiers (`peek`, `main`, …) are produced as plain
  `Identifier` — the lexer does **not** special-case them. *(Ch 01 §5.3; RD-02 R13)*

### Numeric literals

- [ ] **FR-11** — Decimal `digit { ["_"] digit }`; leading zeros allowed but emit
  **W10210**. *(Ch 01 §6.1; RD-02 R14)*
- [ ] **FR-12** — `$`hex requires ≥1 hex digit; bare `$` → **E10214**, recovery value `0`.
  *(Ch 01 §6.2; RD-02 R15)*
- [ ] **FR-13** — `0x`/`0X`hex requires ≥1 hex digit; bare `0x` → **E10214**, recovery `0`.
  *(Ch 01 §6.2; RD-02 R16)*
- [ ] **FR-14** — `0b`/`0B`binary requires ≥1 bin digit; bare `0b` → **E10215**, recovery `0`.
  *(Ch 01 §6.3; RD-02 R17)*
- [ ] **FR-15** — Underscores only **between** digits; leading/trailing/consecutive →
  **E10213**. Underscores stripped before parsing. *(Ch 01 §6.4; RD-02 R18)*
- [ ] **FR-16** — Parsed value capped at 65535; overflow → **E10216**, recovery value
  `65535` (saturated). *(Ch 01 §6.5; RD-02 R19)*

### String & character literals

- [ ] **FR-17** — `"…"`: single-line; literal newline → **E10217**; unterminated →
  **E10218**; `""` valid. `value` = raw text between quotes. *(Ch 01 §7.1; RD-02 R20)*
- [ ] **FR-18** — `'…'`: exactly one char/escape; empty → **E10221**; multi → **E10222**;
  unterminated → **E10223**. *(Ch 01 §8; RD-02 R21)*
- [ ] **FR-19** — Escape set is closed: `\\ \" \' \n \r \t \0 \xNN`. Unknown → **E10219**;
...
- [ ] **FR-20** — `//` line comment to EOL, fully discarded. *(Ch 01 §3.3; RD-02 R6)*
- [ ] **FR-21** — `/* */` block comment, non-nesting; unterminated → **E10211**. *(Ch 01 §3.4; RD-02 R7)*
- [ ] **FR-22** — Space/tab/CR/LF are whitespace, skipped, never significant; lines
  increment on LF, CRLF, bare CR. *(Ch 01 §3.1; RD-02 R3/R4)*
- [ ] **FR-23** — Comment detection (`//`, `/*`) precedes operator scanning; lone `/` →
  `Slash`. *(Ch 01 §11.3; RD-02 R27)*

### Operators, punctuation, disambiguation

- [ ] **FR-24** — Maximal munch: `<<=` is one token, `&&` is one token, etc. *(Ch 01 §11.1; RD-02 R23)*
- [ ] **FR-25** — `&` always `Ampersand`; `:` always `Colon`; `?` → `Question` (parser
  disambiguates meaning). *(Ch 01 §9.6/§9.7; RD-02 R24/R25/R26)*
- [ ] **FR-26** — Disambiguation order is exactly Ch 01 §11.2: string → char → `$`hex →
  `0x`hex → `0b`bin → decimal → identifier/keyword → operator → punctuation → E10210.
  *(Ch 01 §11.2; RD-02 R28)*

### Spans, line map, source input

- [ ] **FR-27** — Every token carries a correct `SourceSpan { sourceId, start, end }`
  (half-open byte offsets). *(AR-L3; RD-02 R29)*
- [ ] **FR-28** — `LineMap` is built once via `new LineMap(sourceId, text)` and returned in
  `LexResult`; line/col computed on demand. *(AR-L1/L2; RD-02 §4.8)*
- [ ] **FR-29** — Leading UTF-8 BOM (`EF BB BF`) is skipped before tokenizing; not part of
  any token span; no diagnostic. *(Ch 01 §2.1; RD-02 R1/§4.10)*
- [ ] **FR-30** — Non-ASCII outside strings/comments → **E10210**. *(Ch 01 §2.1; RD-02 R1)*
- [ ] **FR-31** — `lex()` receives `text` as a parameter; never reads disk directly.
  *(RD-02 R32; AR-40)*

### Error tolerance & determinism

- [ ] **FR-32** — `lex()` never throws; all errors append to the `DiagnosticBag`. *(RD-02 R30; AR-15/AR-73)*
- [ ] **FR-33** — The token stream is always non-empty and ends with exactly one `Eof`
  token at `{ start: pos, end: pos }`. *(RD-02 R30; Ch 01 §12)*
- [ ] **FR-34** — After any error the lexer advances `pos` by ≥1 byte (no infinite loop);
  every recovery path produces a well-defined token or skips exactly the bad byte. *(RD-02 R31; §4.5)*
- [ ] **FR-35** — Unterminated block comment (E10211) swallows the rest of the file — no
  further tokens/diagnostics after it (cascade suppression). *(RD-02 §4.5; AR-74)*
- [ ] **FR-36** — Deterministic: identical input → identical token stream **and** identical
  diagnostic set/order, every run. *(RD-02 R33; H5)*

### Packaging & export

- [ ] **FR-37** — Lexer diagnostic codes (E10210–E10224 except E10212; W10210) are added to
  `@blend65/core` `diagnostic-codes.ts` as named `DiagCode` members. *(AR-L5)*
- [ ] **FR-38** — `lex`, `LexResult` are exported from `@blend65/frontend`'s public entry;
  `TokenKind`, `TokenKindValue`, `Token` from `@blend65/core`. *(RD-02 §4.9, R34; AR-L4)*
- [ ] **FR-39** — R15/AR-20 boundary preserved: `frontend` imports `@blend65/core` only;
...
- [ ] **AC-1** — `lex()` exists in `@blend65/frontend`, exported in the public API.
- [ ] **AC-2** — Produces the complete set of 77 token kinds (Ch 01 §12).
- [ ] **AC-3** — All 32 keywords distinguished from identifiers via `KEYWORD_MAP`.
- [ ] **AC-4** — `until`/`to`/`downto`/`step` always produce `Identifier`.
- [ ] **AC-5** — Decimal, `$`hex, `0x`/`0X`hex, `0b`/`0B`binary tokenized with underscore
  rules (E10213) validated.
- [ ] **AC-6** — Numeric values parsed and capped at 65535 (E10216 on overflow); leading
  zeros emit W10210.
- [ ] **AC-7** — Strings and chars tokenized with escape validation; raw text in `value`;
  all 8 escapes validated (E10219 unknown, E10220 incomplete `\x`).
- [ ] **AC-8** — Line/block comments skipped; unterminated block → E10211.
- [ ] **AC-9** — Maximal munch: `<<=`, `&&` single tokens; `//` is a comment, not two
  `Slash`.
- [ ] **AC-10** — Every token carries a correct `{ sourceId, start, end }` span; `LineMap`
  `getLineCol()` returns correct 1-based line/column.
- [ ] **AC-11** — Error tolerance: never throws; invalid input appends diagnostic(s) and
  yields a complete stream ending in `Eof`.
- [ ] **AC-12** — All 14 error codes (E10210, E10211, E10213–E10224) and W10210 emitted for
  their conditions, with messages matching Ch 01 §14.
- [ ] **AC-13** — Determinism: identical input → identical tokens + diagnostics every run.
- [ ] **AC-14** — Unit tests cover every token kind, keyword, operator, error code, recovery
  path, underscore rules, BOM, contextual-keyword-as-identifier, empty input (`[Eof]`),
  comment-only input.
- [ ] **AC-15** — Golden-snapshot tests cover representative `.blend` files tokenized to a
  canonical token-list format (foundation for `--emit-tokens`).
- [ ] **AC-16** — Every decision traces to an `AR-L#`, an RD-02 `R#`, or a frozen Ch 01
  section.
